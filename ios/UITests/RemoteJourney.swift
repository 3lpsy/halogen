...
extension JourneyCase {
    func runRemoteJourney() {
        let suffix = String(UUID().uuidString.prefix(8)).lowercased()
        let createdUsername = "e2e-\(suffix)"
        let playlistName = "E2E \(suffix)"
        launch(autoconnect: "\(Self.serverBase)|dev|dev")
        requireSessionUp()
        let latestList = app.collectionViews["latest-episode-list"]
        let queueList = app.collectionViews["queue-episode-list"]
        let playlistList = app.collectionViews["playlist-episode-list"]
        snap("01-signed-in")
        ...
        require(
            latestList.buttons.matching(identifier: "row-play").firstMatch, "Latest episode rows",
            timeout: 60)
        snap("02-latest-rows")
        ...
        focusAndType(username, createdUsername, "new username")
        ...
        focusAndType(password, "password123", "new password")
        let confirm = app.textFields["Confirm password"]
        focusAndType(require(confirm, "confirm field", timeout: 5), "password123", "password confirmation")
        snap("04-create-user-form")
        scrollTo(app.buttons["Create user"], "create submit").tap()
        require(app.staticTexts[createdUsername], "created user row", timeout: 15)
        ...
        focusAndType(name, playlistName, "playlist name")
        require(app.buttons["Create"], "playlist create submit", timeout: 5).tap()
        require(app.staticTexts[playlistName], "created playlist row", timeout: 15)
        snap("06-playlist-created")

        app.tabBars.buttons["Latest"].tap()
        for cellIndex in 0..<2 {
            let cell = latestList.cells.element(boundBy: cellIndex)
            tapMenuTrigger(cell.buttons["row-menu"], "row menu (cell \(cellIndex))", timeout: 10)
            ...
            if !app.buttons[playlistName].waitForExistence(timeout: 4), addToPlaylist.exists {
                addToPlaylist.tap()
            }
            require(app.buttons[playlistName], "playlist choice", timeout: 5).tap()
        }
        ...
        for cellIndex in 0..<4 {
            ...
            tapMenuTrigger(cell.buttons["row-menu"], "row menu (queue \(cellIndex))", timeout: 10)
            // Restore an addable row when an earlier journey used this same server.
            if app.buttons["Remove from Queue"].waitForExistence(timeout: 2) {
                app.buttons["Remove from Queue"].tap()
                tapMenuTrigger(cell.buttons["row-menu"], "row menu after queue removal", timeout: 10)
            }
        ...
        let frontTitle = queueList.cells.element(boundBy: 0)
        ...
        tap(
            latestList.buttons.matching(identifier: "row-play").firstMatch,
        ...
        app.tabBars.buttons["Playlists"].tap()
        require(app.staticTexts[playlistName], "playlist row", timeout: 10).tap()
        require(
            playlistList.buttons.matching(identifier: "row-play").firstMatch, "playlist episodes",
            timeout: 15)
        snap("11-playlist-detail")

        tap(
            playlistList.buttons.matching(identifier: "row-play").firstMatch,
        ...
        snap("13-offline")
        let firstCell = latestList.cells.element(boundBy: 0)
        tapMenuTrigger(firstCell.buttons["row-menu"], "row menu offline", timeout: 10)
