...
use halogen_webui_component_icons::{MagnifyingGlass, XMark};
...
/// The type-appropriate control for one override row: a select (Bool), a date / text / number `<input>` per
/// [`InputKind`]. Unlike the shared [`InputField`] (two-way `Signal<String>` + a label wrapper), this is string-keyed,
/// it pushes each change up through `on_value` so the parent can update the working set by stable `key`, and renders
/// bare (the row already owns the label + error). The rendered markup/classes match the hand-written arms exactly.
#[component]
fn OverrideInput(
    label: String,
    kind: InputKind,
