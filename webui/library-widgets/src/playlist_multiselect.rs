//! Render a paged/searched playlist checkbox window while retaining selected rows; search shows all loaded matches.
//! Callers own search and submission, supply the scroll container, and receive on_change after toggles. This component
//! renders the matching sentinel.
...
/// Full-page chrome around [`PlaylistMultiselect`] shared by the two playlist picker screens (one episode, and a bulk
/// multiselect): a back + title + search header, the scrollable multiselect body, and a sticky Save footer with an
/// offline hint. The caller owns `selected` (it seeds/diffs it) and the save semantics (`save_label`, `save_disabled`,
/// `on_save`); everything else is here.
#[component]
