...
use halogen_webui_app_state::{DownloadState, PlaybackState, PlaylistState, PodcastState};
use halogen_webui_component_widgets::{PullToRefreshIndicator, use_quick_menu};
...
pub fn EpisodeList(
    ...
    swipe: halogen_webui_listview::SwipeConfig,
    ...
    // Bound ID-list growth by max(pool_count, id_total), including unresolved members. Bounding only resolved bodies
    // deadlocks paging because fetching more bodies itself requires a larger visible window.
    ...
    // Deduplicate body fetches across worker publications until missing IDs land in the store. Peek the guard so
    // bookkeeping cannot retrigger the effect; real state publications drive convergence.
    ...
    // Memoize selected_only separately so checkbox changes do not requery the whole list. Subscribe to the selection
    // set only while that filter is active; otherwise peek it.
    let selected_only = use_memo(move || ms.read().selected_only);

    // The shared pool plan handles cache-query fetches and skips in-memory/offline/ID-list modes. This component owns
    // scrolling to bound window growth. Never subscribe the plan to EpisodeState: its own cache publication would
    // refetch recursively; cache_version and effect A refresh displayed rows.
    ...
    // Subscribe ID-list fetching to membership state and request only missing bodies, stopping once pooled. Change
    // loading only for real requests to avoid spinner flicker on unrelated publications; bump cache_version after
    // success.
    use_effect(move || {
        if !paged {
            return;
    ...
    // Query filtered/sorted cached rows on window, cache version, and domain changes. Episode publications catch worker
    // upserts that land after the HTTP future completes; this effect only reads, so it cannot loop. Generation checks
    // prevent older async reads from overwriting newer windows.
    let render_gen = use_latest_wins();
    // For empty-cache first load, keep skeletons until queried rows actually commit. HTTP loading can clear before the
    // worker upsert lands; this extra flag prevents an intervening false empty state and is never read by the writing
    // effect.
    ...
    use_effect(move || {
        ...
        // Subscribe to the pool's first-fetch settle signal: `false` while an initial cold fetch is still in flight
        // (online), `true` the moment the cache is authoritative (offline / warm-fresh) or once that fetch lands. Only
        // consulted when the pool has NOTHING to paint (the empty-pool hold in the AllEpisodes branch), a non-empty
        // cache always paints at once.
        ...
        // Subscribe to episode writes, download membership, podcast names, playlist membership, and playback/history.
        // Clone only snapshots required by the chosen branch, avoiding full-pool copies on unrelated publications.
        ...
        // Derive the id-list predicates from `is_id_list()` (single source of truth for the variant set,
        // hand-enumerating it here is exactly how `ServerDownloads` shipped with a missing snapshot and panicked): the
        // downloads/history-style branch is every id-list source EXCEPT Playlist, which has its own arm with different
        // needs.
        ...
        spawn(async move {
            ...
            // Branch predicates guarantee their required snapshots. Commit row signals only when the window differs,
            // avoiding full-list rerenders on unrelated publications. Bind the peek comparison before set so its borrow
            // is released before mutation.
            ...
            match &source {
                ...
                // History / Downloads: resolve the WHOLE intrinsic membership, then filter + sort under the current
                // predicate (search, played chips, sort field) before windowing, mirroring the Playlist branch, so
                // these controls aren't silently ignored. The OnDevice facet trims the id list (to fully-downloaded)
                // before resolving.
                ListSource::ClientDownloads | ListSource::ServerDownloads | ListSource::History => {
                    ...
                    let rank: std::collections::HashMap<i32, usize> =
                        ids.iter().enumerate().map(|(i, id)| (*id, i)).collect();
                    // History's default sort is the non-selectable `UpdatedAt` sentinel meaning "natural order"; map it
                    // to the rank so recent-played-first stays the default (the episode body's `updated_at` isn't play
                    // recency). Downloads' default (Published) is a real, selectable sort, so it's left alone.
                ...
                ListSource::AllEpisodes => {
                    if on_device {
                        // OnDevice → render the local device set (downloaded only), scoped to this podcast if the list
                        // is. Search / Played / Unplayed still apply client-side over this small set. The device id set
                        // IS the OnDevice filter, so the token is a no-op inside `apply_filter_sort` (it ignores
                        // OnDevice).
                        ...
                        // Render cached filter/sort results immediately while the server revalidates. Worker
                        // publication reruns the query; commit only changed rows instead of holding skeletons on every
                        // stale navigation.
                        ...
                        // Only a genuinely cold load with an EMPTY pool still waits: there's nothing cached to paint,
                        // and committing the empty set would flash "No episodes found" under the skeletons while the
                        // initial fetch is in flight. `settling` keeps the skeletons up (see the render body); the
                        // `settled` flip / worker publish re-runs this effect to paint the fresh page.
                        ...
                        // Releasing a held-empty first paint: the fetch has resolved but its rows may still be
                        // upserting in the worker, settle briefly and re-read so the first paint is the FRESH page, not
                        // an empty flash. A publish inside the window supersedes this read (render_gen last-wins).
                        if rows.is_empty() && *first_paint_settling.peek() {
                            halogen_webui_platform::time::sleep_ms(FIRST_PAINT_SETTLE_MS).await;
                            if !render_gen.is_current(g) {
                                return;
                            }
    ...
    // Narrowed selector over the session token (`server_url`): the history reset-load below must re-fire when auth
    // lands, but `config.read()` would subscribe it to the WHOLE `ClientConfig`, so any unrelated mutation (offline
    // toggle, theme, prefs) would re-fire `load_history(reset=true)` and jump History back to page 0. A memo only
    // notifies when the token changes, exactly when a reset is wanted.
    let history_auth = use_memo(move || config.read().server_url.clone());

    // Reset history paging on mount, refresh, auth arrival, or restored connectivity. The worker merges page zero into
    // playback state; effect A derives its ordered history IDs.
    ...
    use_effect(move || {
        ...
        // Offline: don't issue the load OR set the in-flight guard, the worker's `load_history` early-returns without
        // advancing the cursor, which would latch `history_inflight` forever (effect 2 clears it only on a cursor
        // advance). Subscribing to the offline memo re-fires this exactly when connectivity returns, so a History page
        // mounted offline recovers on its own (the reset also re-arms `history_has_more` after a failed fetch).
        if is_offline() {
            return;
        }
        history_inflight.set(true);
        halogen_webui_commands::actions::load_history(&dispatch, true);
    });

    // Page ahead near the loaded history end only after initial reset completes and while more pages remain.
    // Deduplicate until the worker advances its cursor; peek guards so bookkeeping cannot retrigger itself.
    use_effect(move || {
        if !is_history {
            return;
        }
        ...
        drop(h);
        // Gate history page-ahead on unfiltered loaded playback IDs, not filter-reduced pool_count. Peek unrelated
        // pools so only history cursor/window changes drive this effect.
        ...
        {
            history_inflight.set(true);
            halogen_webui_commands::actions::load_history(&dispatch, false);
        }
    });
    ...
    // Pull-to-refresh. On a podcast-detail list the scope is the filter's `podcast_id` (poll that feed); elsewhere it's
    // `None` (poll all). The refresh resets the paging cursors + bumps the nonce (force a re-fetch even at page 0) and
    // asks the worker for a full pull so id-list memberships (queue/playlists) reconcile too.
    ...
    // Own infinite scrolling with a 250ms sentinel trigger. Stop at the available pool/membership bound unless another
    // server page remains, avoiding repeated cache queries at list end; in-memory mode reveals another 30 rows.
    let mut page = page;
    ...
    // Share one use_callback selection toggle, taking episode ID at invocation. Per-row Callback::new values defeat
    // prop memoization and rerender every row whenever the parent renders.
    ...
    // Manual reorder is available only on a playlist list in Custom order, and never while multiselect is active (the
    // grip is replaced by the checkbox) or while a filter/search narrows the rendered set. The reorder target is a
    // rendered-row index applied directly to the stored membership, so with rows hidden it would land at the wrong
    // stored position, disable it instead of corrupting the order (the user clears the filter to reorder).
    let playlist_id = match &source {
        ListSource::Playlist { id } => Some(*id),
        _ => None,
    };
    let list_len = episode_items.len();
    // Enable reorder only when all membership bodies resolve, preserving exact row-to-position indices and the
    // descending mirror length. A smaller visible prefix is safe; missing bodies anywhere disable reorder until
    // scrolling fetches them.
    ...
    rsx!(
        div { class: "flex flex-col h-full",
            ...
            div { class: "relative flex-1 min-h-0",
                ...
                // Append-more spinner: an ABSOLUTE overlay pinned to the bottom of the scroll area (out of flow), so it
                // appearing/vanishing as infinite-scroll pages load can't collapse the layout and shift content (a
                // scroll-time CLS). Cold load uses the in-flow skeletons above; this shows only when paging an
...
async fn load_page(
    ...
    // All data is already in memory; render the full filtered/sorted set and let the locked scroll container handle
    // scrolling. (Server-side paging/virtualization can be layered back in when lists outgrow memory.) Set the pool
    // size too (the reorder mirror axis + scroll gate read it); the paged path sets it, so the in-memory path must as
    // well, else `full_len` is 0.
