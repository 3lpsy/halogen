//! Bulk-action menu builder for the episode list's multiselect mode. Pure data construction (no reactive reads): given
//! the selected ids plus the resolved queue / current-playlist context, builds the [`QuickAction`] sections the shared
//! [`QuickContextMenu`](halogen_webui_component_widgets::QuickMenu) renders. Each action dispatches the matching bulk
//! worker command (or, for "Add to playlist", navigates to the bulk picker) for a snapshot of the ids.
...
/// Build the bulk-action menu sections for a multiselect over `ids`. `queue_id` is the default playlist (queue) id when
/// one exists; `current_playlist_id` is the playlist the list is showing (when it is a playlist/queue view), it gates
