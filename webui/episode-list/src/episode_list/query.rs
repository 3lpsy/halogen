//! Pure, unit-tested query helpers for the episode list: the id-list builders for id-list sources, the progress
//! fraction, and the in-memory filter + sort. Kept out of `list.rs` so the component there stays focused on rendering
//! and its load effects, while this logic, which has no Dioxus dependency, is testable in isolation.
...
use halogen_webui_app_state::{DownloadState, PlaybackState, PlaylistState};
use halogen_webui_listview::{
    EpisodeData, EpisodeFilter, FilterSpec, ItemVariant, ListSource, OrderDirection, SortField,
    SortSpec,
};

/// Return ordered membership IDs for playlists, device downloads, and history, or None for server cache-query sources.
/// Resolve a window from the local pool and fetch missing bodies by ID.
pub(super) fn id_list_for_source(
    ...
    match source {
        ...
        ListSource::ClientDownloads => {
            // Include active downloads but omit failed ones. Restrict rows to this account's cached episodes so shared
            // browser audio cannot expose another account's library on the Downloads page.
        ...
        ListSource::History => {
            ...
            // Sort history by playback recency, then descending episode ID. The explicit tie-break prevents HashMap
            // insertion order from reshuffling equal-time rows as pages arrive.
            rows.sort_by(|a, b| {
                b.updated_at
                    .cmp(&a.updated_at)
...
/// Downloaded-to-this-device episode ids, newest-first, optionally scoped to one podcast. Backs the `OnDevice` facet on
/// cache-query lists (`/latest`, podcast detail): when that chip is on, the list renders this local set instead of
/// paging the server (which can't express the device-download set). Unlike `ClientDownloads`, this excludes in-flight
/// `Downloading` items.
...
/// Derive fetch IDs with selected-only precedence, then playlist/download/history/OnDevice membership, else a cache
/// query. Rendering shares this classification but resolves each branch differently for windowing, ranks, and
...
mod tests {
    ...
    fn id_list_for_render_follows_the_taxonomy() {
        let s = EpisodeState {
            episodes_by_id: HashMap::from([(1, ep(1, "a", 7, 2021))]),
            ..Default::default()
        };
        let p = PlaylistState::default();
    ...
    fn id_list_for_device_downloaded_only_newest_first_and_podcast_scoped() {
        ...
        let s = EpisodeState {
            episodes_by_id: HashMap::from([(1, ep(1, "a", 7, 2021)), (3, ep(3, "c", 9, 2021))]),
            ..Default::default()
        };
