//! Small self-contained pieces of an episode row, shared by the list item and the episode detail page so the same
//! markup (classes / aria / glyph logic) lives in one place. Each takes minimal props derived from
//! [`EpisodeRowState`](super::EpisodeRowState) plus the per-scope handles (`player` / `dispatch`) the buttons fire
//! through.
...
/// The play/pause + length badge button. Disabled (greyed) when play is gated off (`play_disabled`); shows the spinner
/// while the player is preparing this episode, a pause glyph while playing it, else the play glyph. Toggling
/// pauses/resumes the current episode or requests a fresh play, with `playlist_id` (the list the row sits in, when it's
/// a playlist/queue) as the play context, so playback continues through that list.
#[component]
pub fn PlayBadge(
    episode_id: i32,
    is_current: bool,
...
/// The device-download toggle badge. Honest states: spinner while bytes are ACTUALLY being fetched to this device (also
/// covers waiting for the server's copy), trash once stored. `Failed` renders as the not-downloaded icon, clicking
/// retries. Disabled while in flight. The glyph is chosen by the pure [`download_badge`] helper so the phase transition
/// stays unit-testable.
#[component]
