//! Edit persisted navigation order and visibility shared by dock, sidebar, and menu. Settings stays visible and More
//! remains reachable; show admin destinations only to admins and preserve pins/unmanaged keys.
...
use halogen_webui_config::{BuiltinNav, NavConfig, NavKey};
use halogen_webui_hooks::{use_config, use_is_admin, use_toast};
...
/// Merge shown rows first, then unmanaged stored keys in their original relative order. Compute carry-through by set
/// difference, not the live admin flag, so a permission change cannot silently drop keys seeded under another flag.
...
pub fn ConfigureDock() -> Element {
    ...
    // `is_admin` resolves asynchronously after login; if it flips while this page is mounted, re-seed the working copy
    // so the admin-only rows appear (and get carried in `order` rather than depending on the apply-time carry-through).
    // Un-applied edits from the brief pre-flip window are reset, the flip happens once, right after login.
    ...
    let on_apply = move |_| {
        ...
        // The working `hidden` set is seeded from ALL of `config.nav.hidden` (`:96`), so it already IS the complete
        // desired hidden set, persist it directly. Merging with `stored.hidden` the way `order` does would re-add every
        // key the user un-hid (the only un-hide path is removing from the working set), making un-hiding impossible.
