//! Keep cache recovery outside auth/layout guards and call browser storage directly so it works when sync is broken.
//! Actions confirm inline and report locally; only close navigates away. Asset/service-worker changes reload this
//! route. Account state is needed only to remove affected local-library accounts.

use dioxus::prelude::*;

use halogen_webui_accounts::accounts::AccountsStore;
use halogen_webui_cache_purge as cache_purge;
use halogen_webui_component_icons::XMark;
use halogen_webui_config::ClientConfigStore;
use halogen_webui_hooks::{read_initial_query_param, use_accounts};
use halogen_webui_provider_local::embedded;
                        ...
                        ActionCard {
                            ...
                            on_confirm: move |_| {
                                ...
                                spawn(async move {
                                    ...
                                    if was_active {
                                        // Mirror ordinary sign-out: a surviving (remote) account becomes active rather
                                        // than dumping the user to first-boot setup. Its stored token may be stale, the
                                        // worker's auth-expiry path and RootGuard handle that as a normal re-login.
