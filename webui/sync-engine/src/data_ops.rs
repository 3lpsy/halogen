...
use halogen_webui_app_state::state::SyncStatus;
use halogen_webui_logging::{debug, error};
...
impl SyncService {
    /// Persist a server-paged batch of episodes and merge them into the in-memory indices. The worker is the single
    /// store writer, so server-paged views (e.g. `/latest`) hand their freshly-fetched pages here instead of writing
    /// the store themselves. Idempotent: upserts by id, and only appends ids not already grouped under their podcast.
    pub(super) async fn cache_episodes(&mut self, mut episodes: Vec<EpisodeData>) {
        if episodes.is_empty() {
            return;
        }
        // Preserve played/download facets while their corresponding operations remain queued. Server revalidation may
        // predate optimistic changes in either direction and must not overwrite them.
        ...
        // Chapters are loaded lazily (opt-in include) and the lists don't carry them, so keep an already-loaded set
        // sticky: a chapter-less body (`None` = "not requested") inherits the chapters we already have. `Some(_)`
        // (including an empty set) is authoritative and overwrites. Applied before the store write so the cache and
        // memory agree (chapters survive reload).
        for ep in episodes.iter_mut() {
            ...
            // Don't let a stale revalidation overwrite a locally-set playback or download facet while its outbox op is
            // still queued. The optimistic local value is authoritative in BOTH directions until the op drains, so keep
            // `prev` whenever the incoming server value differs (e.g. a queued mark-unplayed must survive a page that
            // still says Finished; a queued remove-download must survive a page that still says Downloaded).
    ...
    pub(super) async fn cache_podcasts(&mut self, podcasts: Vec<PodcastData>) {
        ...
        // Reject session-unsubscribed podcasts even after the outbox operation drains. A page fetched before
        // unsubscribe can arrive later and must not resurrect its tombstoned row.
    ...
    /// Upsert playlists into local state + store after a direct create/update. Replaces an existing row by id
    /// (preserving its already-loaded `episode_ids` if the incoming row doesn't carry them) or appends a new one. If an
    /// incoming playlist is the new default, clears `is_default` on the others locally so the UI never shows two
    /// defaults before the next `RefreshNow` pull reconciles.
    pub(super) async fn cache_playlists(&mut self, mut playlists: Vec<PlaylistData>) {
        if playlists.is_empty() {
            return;
        }
        // Preserve local playlist membership while add/remove/move/reorder operations are queued. Pre-drain server rows
        // would otherwise erase optimistic additions or restore removals.
        let mut membership_pending: HashSet<i32> = HashSet::new();
        let pending = match self.store.pending().await {
            Ok(pending) => pending,
            Err(error) => {
                error!(%error, "Cannot revalidate playlists while journal is unreadable");
                return;
            }
        };
        {
        ...
        for pl in playlists.iter_mut() {
            for (_, operation) in &pending {
                match operation {
                    OutboxOp::UpdatePlaylist { playlist_id, data } => {
                        if pl.id == *playlist_id {
                            data.apply_to(pl);
                        } else if data.is_default == Some(true) {
                            pl.is_default = false;
                        }
                    }
                    OutboxOp::MovePlaylist { .. } => {
                        if let Some(local) = self
                            ...
                            .playlists
                            .iter()
                            .find(|local| local.id == pl.id)
                        {
                            pl.position = local.position;
                        }
                    }
                    _ => {}
                }
        ...
        for pl in playlists {
            // When the row carries its membership (the list page requests `EpisodeIds`), mirror it into the id-list
            // index that the lazy detail/queue path reads (`episodes_by_playlist`), same invariant `hydrate_from_store`
            // and the default-playlist pull maintain. A row *without* `episode_ids` (an offline edit, a create) leaves
            // any existing membership untouched.
    ...
    /// Apply an offline podcast-config edit to the cached podcast. Mirrors the server's update semantics: a field set
    /// to `Some` is applied, `None` leaves it unchanged. The form always sends a full set, so this is effectively a
    /// replace; `None` handling keeps parity with the server regardless. Routes through `cache_podcasts`, the single
    /// podcast-pool writer.
    ...
    pub(super) async fn ensure_default_playlist(&mut self) {
        ...
        if self.pull_in_flight || self.recently_pulled() {
            // A pull is already fetching the default playlist, OR one completed (the queue page mounts right after the
            // SetAuth/WsConnected pulls fire at startup, sometimes a beat after the first finishes, so the in-flight
            // guard alone misses it). Reuse that refresh instead of a third redundant GET; resolve the queue from
            // cache, the pull published / will publish the authoritative result.
    ...
    /// Spawn a de-duped detached task keyed on `(kind, key)`: a no-op when manually offline, when `key` is already in
    /// flight for `kind`, or when there's no API client yet (dropping the guard so a later call retries). Otherwise
    /// builds the task with a fresh API handle + the internal channel and spawns it, dropping the guard if the spawn
    /// fails. The single shape behind every `ensure_*`.
    ...
    /// Fetch missing page podcasts in one IDs request. Share the in-flight set with per-row ensure_podcast calls so
    /// concurrent batch and row requests never duplicate IDs.
    ...
    pub(super) fn ensure_server_progress_poll(&mut self, episode_id: i32) {
        if let Some(polls) = self.deferred_server_polls.as_mut() {
            polls.push(episode_id);
            return;
        }
    ...
    /// Drop superseded SetCursor operations before enqueueing a newer cursor or MarkPlayed. Cursor writes are
    /// last-write-wins; the serial worker cannot be draining concurrently, so compaction avoids replaying every
    /// intermediate offline position safely.
    ...
    /// Insert-or-update the playback row for an episode, apply `f`, then persist it to the local store. Write-through
    /// (like `set_download_status_locally`) is required now that playbacks aren't bulk-pulled: this optimistic cursor
    /// is the only durable record until the outbox `SetCursor`/`MarkPlayed` reaches the server, so it must survive a
    /// reload, especially offline (boot `hydrate_from_store` rebuilds the overlay + History from these rows).
    ...
    pub(super) async fn remove_device_download(&mut self, episode_id: i32) {
        ...
        if let Some(removals) = self.deferred_audio_removals.as_mut() {
            removals.push(episode_id);
            return;
        }
        self.remove_device_audio(episode_id).await;
    }

    pub(crate) async fn remove_device_audio(&self, episode_id: i32) {
    ...
    /// Append an episode id to a playlist's membership if not already present. Insert an episode into a playlist's
    /// in-memory membership at `position` (`Some(i)` inserts at the clamped index, `None` appends). Mirrors the
    /// server's insert so a FIFO outbox replay reproduces this order. No-op if the episode is already a member.
    ...
    /// Optimistically reorder a playlist within the user's manual order: rewrite `position` across the loaded pool to
    /// the new contiguous order and persist the changed rows. Operates over the LOADED subset (lazy paging means the
    /// full set may not be in memory), the server rewrites the full 0..n on drain and the next pull reconciles. No-op
    /// if the playlist isn't loaded or `to` is unchanged.
    ...
    /// After confirmed deletion, remove the playlist, membership, and cached row. Recompute the queue as Absent only if
    /// the deleted playlist was its known default; a partial pool cannot otherwise prove absence.
    ...
    /// Remove all locally-cached data for one episode WITHOUT touching the server (see
    /// [`Command::RemoveLocalEpisodeData`]): the device audio blob, cached metadata, playback position, download state,
    /// and playlist/queue membership. Purely local recovery, no outbox op, so the episode re-syncs on the next pull.
    /// The outer loop publishes the resulting state.
    ...
    /// Remove all locally-cached data for a podcast and every one of its episodes WITHOUT unsubscribing on the server
    /// (see [`Command::RemoveLocalPodcastData`], unlike [`Command::Unsubscribe`]): device audio, cached metadata,
    /// playbacks, download state, and playlist/queue membership. Purely local recovery, no outbox op, so the podcast
    /// re-syncs on the next pull.
...
mod cache_playlists_tests {
    ...
    #[test]
    fn stale_playlist_metadata_keeps_latest_queued_edit() {
        let mut svc = test_service("playlist-metadata-guard");
        block_on(async {
            svc.cache_playlists(vec![playlist(1, Some(vec![7]))]).await;
            for name in ["First edit", "Second edit"] {
                svc.handle_command(crate::Command::UpdatePlaylist {
                    playlist_id: 1,
                    data: PlaylistUpdateData {
                        name: Some(name.into()),
            ...
            svc.cache_playlists(vec![playlist(1, Some(vec![7]))]).await;
            let row = &svc.playlists.playlists[0];
            assert_eq!(row.name, "Second edit");
            assert!(row.is_default);
            let stored = svc.store.list_playlists().await.unwrap();
            assert_eq!(stored[0].name, "Second edit");
