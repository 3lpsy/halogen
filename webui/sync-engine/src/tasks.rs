//! Detached one-shot/poll tasks the worker spawns (via `ensure_*` / the command router) and that report back through
//! the internal `Command` channel, kept out of the command-router module so all the fire-and-forget `!Send` fetches
//! live in one place. Co-located here: the device-download task lives in `download.rs` (its own larger concern).
...
/// Detached task: fetch a BATCH of podcasts by id in one `list_podcasts` request (with `PodcastConfig` included) and
/// report them back via [`Command::PodcastsFetched`] (spawned by `ensure_podcasts_batch`). Collapses the
/// per-episode-row `EnsurePodcast` N+1 into a single round trip. `None` on failure so the worker clears every requested
/// in-flight guard either way.
...
/// Poll server-download progress about once a second and emit only integer percentage changes. A 404 means not started
/// until progress has been seen, then completion; refresh durable status when finished.
pub(super) async fn server_download_poll_task(
    ...
    let mut seen = false;
    // Wall-clock seconds spent waiting on a 404/err before the download starts, and the current inter-poll delay. While
    // bytes flow we poll at 1 Hz for responsive progress; before the server starts (or during a blip) we back off
    // exponentially so a not-yet-started download isn't hammered once per second for the whole grace window (the 404
    // storm Lighthouse flagged).
    ...
    // Refresh the durable status (Downloaded / DownloadError / …) and clear the ring. Include Playback so re-caching
    // the body keeps the embedded cursor. Retry a few times: a transient offline blip at the very end would otherwise
    // hand back `None`, and the handler would leave the status stuck on the optimistic `Downloading` with no further
    // poll to reconcile it.
