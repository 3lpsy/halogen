//! Device-download pipeline: the detached task that fetches an episode's audio to the local media store, plus its
//! resumable/retrying chunked + streaming strategies and progress reporting. These are free functions (no `SyncService`
//! access), the worker spawns [`device_download_task`] and stays the single `EpisodeState` writer; the megabytes never
//! ride through the Debug-logged `Command` enum.
...
use halogen_webui_media::{MediaStore, MediaWriter};
#[cfg(target_arch = "wasm32")]
use halogen_webui_platform::weblock;
...
/// Poll for the server copy and hand fresh episode bodies back through CacheEpisodes. Then write downloaded bytes
/// directly to the media store and report outcomes through commands, preserving worker-only state writes and keeping
/// bytes out of logs.
#[allow(
    clippy::too_many_arguments,
    reason = "Task owns independent transfer handles and per-download settings"
)]
pub(super) async fn device_download_task(
    ...
    // Hold a per-origin episode Web Lock throughout download so tabs cannot interleave shared IndexedDB chunk
    // sequences. Tab exit releases it; unsupported browsers proceed unguarded. Native has one media-store owner.
    #[cfg(target_arch = "wasm32")]
    ...
    // Phase 2: resumable, retrying, chunked download with progress. On failure we KEEP the durable partial (it's
    // invisible to `audio_url`/`list_ids`, so no unplayable bytes are exposed) so the next attempt, a manual retry or
    // the boot-time `resume_partial_downloads`, continues from where this left off. The only purge is the
    // corruption-restart inside `download_audio`.
    if offline.get() {
        return send(Err("cancelled — you went offline".into()));
    }
...
/// A chunked-download failure. `OffsetMismatch` is kept apart from the plain message errors because it has a dedicated
/// recovery: the server (or a proxy in front of it) ignored the requested `Range` and served bytes from a different
/// offset, those bytes must never be written at the requested position, and the download restarts from scratch instead
/// (see [`download_audio_chunked`]).
#[derive(Clone, Debug)]
enum ChunkError {
...
) -> (u64, Result<halogen_apiclient::AudioChunk, ChunkError>) {
    (
        start,
...
/// Fetch the complete inclusive range, retrying short 206 responses for their remainder. Assembly advances by bytes
/// written, so incomplete ranges would leave gaps between reorder-buffer keys. Treat zero bytes before expected EOF as
/// an error.
async fn fetch_full_range(
...
) -> Result<halogen_apiclient::AudioChunk, ChunkError> {
    ...
    acc.truncate(want);
    Ok(halogen_apiclient::AudioChunk {
        bytes: acc,
        total,
...
/// Resume durable bytes, retry ranges with backoff, and buffer completed chunks until byte-order writes are possible.
/// Verify offsets and final length; changed totals restart at zero. A mis-offset response permits one clean restart,
/// then fails. Errors retain partials; chunk size may change across resumes.
async fn download_audio_chunked(
    ...
    .await
    {
        Err(ChunkError::OffsetMismatch {
            ...
            .await
            {
                // The from-scratch retry ALSO got mis-offset bytes: the server (or a proxy) never honors ranges, so
                // chunking can't work, fall back to the STREAMING strategy, which handles a 200-from-byte-0 correctly
                // with bounded memory (the chunked client caps each buffered body, so this path no longer succeeds by
                // accident via a whole-file buffer). Never commits stitched bytes and never restart-loops.
                Err(ChunkError::OffsetMismatch { .. }) => {
                    warn!(
                        episode_id,
...
async fn download_audio_chunked_attempt(
    ...
    // ── Phase A: the first chunk, fetched alone ────────────────────────────── Handles resume verification, lazy
    // writer-open, and learning `total` / content type before any parallel fan-out. Iterates at most twice (a second
    // pass only when a resume mismatch or an oversized staged partial restarts from byte 0).
    loop {
        if let Some(t) = total {
            if downloaded == t {
    ...
    // Without a reported total, fetch sequentially until short/empty EOF so a full first chunk cannot masquerade as a
    // complete file. Switch to the windowed phase if a later response supplies the total.
        ...
        && downloaded < t
    {
        ...
        while next_write < t {
            // Count in-flight and completed-but-unwritten chunks against the window to bound memory behind a slow
            // earliest chunk. Refill after draining; after failure, stop issuing requests and flush completed work.
            ...
            // Drain completions until the chunk we need to write next is buffered. Each completion moves a chunk
            // in-flight → buffer, leaving the outstanding total unchanged, so the next refill happens after a write
            // frees a slot (back at the top). `next_write`'s chunk is always issued before any later one, so while it's
            // still in flight `inflight` is non-empty and `.next()` yields.
            while !buffer.contains_key(&next_write) {
                ...
                match res {
                    // The server's copy changed size mid-download (a concurrent re-download): this chunk's bytes are
                    // from a different file, so don't trust them. Treat it like a failure, drain & write the consistent
                    // chunks below the gap, then bail; resume's Phase A total-check then discards the stale partial and
                    // restarts clean.
...
/// Stream one open-ended request to durable storage in STREAM_FLUSH_BYTES batches. Resume from the last flushed offset
/// without whole-file buffering or parallel reordering; verify the resumed total before streaming.
...
/// Fetch one chunk with bounded exponential-backoff retries; the error is the last attempt's message. Every response is
/// verified to start at the requested `start` offset, a server/proxy that ignored the `Range` (served from elsewhere,
/// usually byte 0) yields [`ChunkError::OffsetMismatch`] immediately (no retries: a peer that ignores ranges won't
/// honor the next one either), so mis-offset bytes can never be written at the requested position.
        ...
        match api.download_audio_range(episode_id, start, end).await {
            ...
            Err(e) => {
                ...
                if attempt < DEVICE_CHUNK_RETRIES {
                    halogen_webui_platform::time::sleep_ms(delay).await;
...
// Chunked-download behavior tests against a real (loopback) HTTP server whose `Range` handling is scripted per test,
// the point is the client's reaction to a server/proxy that ignores the requested byte offset, which no pure-function
// test can cover. Native-only: the download path runs identically on both targets, and the harness needs tokio + a TCP
// listener.
...
mod tests {
    ...
    use halogen_webui_media::PartialInfo;
    ...
    impl MediaStore for MemMediaStore {
        ...
        async fn audio_url(
            &self,
            _episode_id: i32,
        ) -> anyhow::Result<Option<halogen_webui_media::LocalAudio>> {
            Ok(self
                .0
                .committed
                .borrow()
                .is_some()
                .then(|| halogen_webui_media::LocalAudio {
    ...
    /// A server that NEVER honors nonzero ranges: after the one from-scratch restart also fails, the download falls
    /// back to the STREAMING strategy (bounded memory, handles 200-from-0 natively) and completes with byte-correct
    /// content. The invariants stand: no file stitched from mis-offset bytes is ever committed, and there is no restart
    /// loop.
    #[test]
