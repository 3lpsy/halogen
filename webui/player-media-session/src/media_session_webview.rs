//! Feature-detect native webview Media Session support and no-op when absent. One persistent eval channel carries
//! metadata/position updates and OS actions, avoiding per-tick query leaks. A scope-owned task routes actions through
//! the controller, including configurable next/previous seeking.
...
thread_local! {
    ...
    /// Bumped by each [`register`]; a guard tears its session down on drop only while it is still the current one. On a
    /// user switch Dioxus builds the new provider's session (a fresh `register`, bumping this) BEFORE dropping the old
    /// one, an unconditional teardown would detach the channel and clear the OS handlers the new session bound, killing
...
/// Lock-screen / notification metadata for the current episode. Same field convention as the wasm module: episode title
/// as the track title, podcast name as artist and album. `artwork_url` is the relative `/halogen-media/...` proxy URL,
/// the webview resolves it against the app origin, so the OS artwork fetch rides the authenticated proxy too.
...
pub fn register(
    ...
    spawn(async move {
        while let Ok(msg) = pump_chan.recv::<serde_json::Value>().await {
            ...
            match action {
                // Play/pause through the controller (not the raw element) so `now_playing` and cursor persistence stay
                // correct. `play` (not `resume`): the OS play action can arrive in any state, and `resume` is a no-op
                // outside `Paused`, which left this button dead after a track ended or a play failed, while `pause`
                // kept working.
