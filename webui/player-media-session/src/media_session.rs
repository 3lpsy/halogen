//! Route OS actions through the controller using live preferences, including optional next/previous seeking. ScopeBound
//! re-enters the runtime synchronously to preserve autoplay activation; the lifetime guard clears handlers on unmount.
//! Manual bindings avoid web-sys's unstable-API build flag.
...
use halogen_webui_player::{PlayerController, ScopeBound};
...
thread_local! {
    /// Bumped by each [`register`]; a guard clears the browser handlers on drop only while it is still the current
    /// session. On a user switch Dioxus builds the new provider's handlers (a fresh `register`, bumping this) BEFORE
    /// dropping the old one, so an unconditional clear in `Drop` would wipe the handlers the new session bound, leaving
    /// controls dead until reload.
    static SESSION_GEN: std::cell::Cell<u64> = const { std::cell::Cell::new(0) };
}

/// Owns the Media Session action-handler closures for the lifetime of one `PlayerProvider`. Replaces the old
/// `Closure::forget()` page-lifetime leak: `PlayerProvider` remounts per user-switch, so leaking a fresh closure set
/// (each cloning a `PlayerController`) on every remount accumulated forever. The provider now holds this guard and
/// drops it on unmount, which also clears the browser handlers so a stale (dropped-closure) handler can't linger.
...
pub fn register(
    ...
    let mut bind = |action: &str, f: Box<dyn FnMut()>| {
        ...
        let wrapped = move || {
            // Stale-registration no-op: once a newer `register` bumped the generation (user switch), this provider's
            // scope may already be unmounted, and re-entering a removed scope panics the moment a handler spawns.
            // Checked OUTSIDE the re-entry so a stale invocation never touches the dead scope at all.
    ...
    // Play/pause through the controller (not the raw element) so the `now_playing` state and cursor persistence stay
    // correct. `play` (not `resume`): the OS play action can arrive in any state, and `resume` is a no-op outside
    // `Paused`, which left this button dead after a track ended or a play failed, while `pause` kept working.
    {
