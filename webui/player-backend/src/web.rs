//! Web audio backend: an `HtmlAudioElement` driven from the UI thread. Control methods (load/play/pause/seek/rate)
//! operate directly on the element. `poll_events` snapshots the element's clock and drains any queued events. Media
//! Session API metadata/handlers (OS lock-screen controls) live in the sibling `media_session` module, not here.
...
/// Detach every playback source from the element: the `src` attribute AND any `<source>` children. Device-copy loads
/// attach a typed `<source>` child (see [`set_typed_source`]) while remote/primer loads set the `src` attribute, the
/// two must never coexist, because a present `src` attribute wins resource selection and silently masks the child.
...
/// Attach the device-copy source as a `<source src=… type=…>` child instead of assigning the `src` attribute. Two
/// WebKit reasons: blob media resolution is strict about the MIME type, and the iOS 17.4.1 regression class broke blob
/// URLs assigned via `src` while the typed `<source>` form kept working (the workaround an Apple media engineer
/// recommended). No-DOM hosts degrade to an empty element, whose load failure surfaces as a normal player error.
...
/// Audio backend backed by a single `<audio>` element. The element is created once up front. Creation only fails when
/// there's no DOM (e.g. a non-browser wasm host), in which case `audio` is `None`, every control is a no-op, and the
/// failure surfaces as a `PlayerEvent::Error` rather than panicking the render.
pub struct WebPlayerBackend {
    ...
    /// Retain native pause/playing listeners for the element lifetime. They capture OS audio-focus and route changes
    /// that polling could miss; dropping them detaches handlers. `None` without a browser DOM.
...
impl WebPlayerBackend {
    pub fn new() -> Self {
        ...
        // Observe the element's native pause/resume so an OS-initiated transition (headset swap, focus loss) is
        // reflected the moment it happens, not sampled, see the field docs. The `pause` listener skips the end-of-track
        // pause (the element flips `paused` true at `ended`; the `Ended` path owns that transition). Both skip the
        // silent primer entirely (see `priming`).
...
/// The element outlives the struct unless silenced here: an audibly-playing `HtmlAudioElement` is never collected (per
/// the HTML spec), so dropping the backend mid-playback, the account-switch/sign-out keyed remount is exactly that,
/// would leave a detached element playing with no surviving handle to stop it. The native listeners must also come off
/// before their `Closure`s drop, or the zombie element's later `pause`/`playing` events would invoke freed closures.
...
impl PlayerBackend for WebPlayerBackend {
    ...
    fn play(&self) {
        let Some(audio) = &self.audio else { return };
        match audio.play() {
            // `play()` resolves/rejects asynchronously. Autoplay-policy (`NotAllowedError`) and decode rejections do
            // NOT set the element's `error`, so await the promise to catch them, but ignore `AbortError`, the normal
            // rejection when a new load/pause supersedes this play (e.g. switching episodes quickly).
            Ok(promise) => {
                    ...
                    match wasm_bindgen_futures::JsFuture::from(promise).await {
                        ...
                        Err(err) => {
                            let name = js_sys::Reflect::get(&err, &"name".into())
                                ...
                                .unwrap_or_default();
                            if name != "AbortError" {
                                // Carry the DOMException name: NotAllowedError (autoplay policy) vs NotSupportedError
                                // (source failed to load) point at entirely different failures, and the device log is
                                // often all we get from an iOS report.
    ...
    fn stop(&self) {
        ...
        // Detach the sources and revoke the active blob URL so a teardown doesn't pin the device-copy bytes until the
        // next load. `stop` is a controller teardown (next play re-loads), so this is safe. Removing the sources +
        // `load()` is the spec-correct release, never set src="" (it resolves to the page URL and errors).
        clear_sources(audio);
        audio.load();
        self.replace_object_url(None);
        *self.pending_seek.borrow_mut() = None;
    ...
    fn poll_events(&mut self) -> Vec<PlayerEvent> {
        ...
        // Report time only after the resume seek lands and playback has current data with `!paused`. Early zero
        // snapshots would overwrite the resume cursor and falsely leave Loading, disabling its stall guard and
        // local-copy retry.
    ...
    /// Play a silent looping source inside the user gesture to unlock later autoplay after downloading. Real `load()`
    /// replaces it; priming suppresses events so the controller stays Preparing.
    fn prime(&self) {
        let Some(audio) = &self.audio else { return };
        if self.primed.get() {
            return;
