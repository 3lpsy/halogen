//! No-op audio backend for RENDERLESS native builds, the `--no-default-features` unit-test graph, which links no
//! webview renderer and so has nothing to play into. Real native playback lives in `webview.rs` (behind the
//! `desktop`/`mobile` renderer features): an `<audio>` element in the app webview. Wasm uses `WebPlayerBackend`.
