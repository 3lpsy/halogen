...
use halogen_webui_component_icons::{ClockRotateLeft, DocumentText, Download, Trash};
use halogen_webui_config::DeviceLogConfig;
...
use halogen_webui_logging::{self as logging, Level, LogLine};
...
pub fn Logs() -> Element {
    ...
    use_effect(move || {
        nav.replace("/logs/device");
    });
    rsx! {}
}

/// Server logs page (`/admin/logs`, admin-only). Fetches a tail of the server's log file via `GET /admin/server-logs`
/// and renders it newest-first in the same code-block style as the device-log viewer, with a manual refresh and a
/// client-side search filter. Online-only (the logs live on the server).
#[component]
...
/// (`halogen_webui_logging`), newest line first. Manual refresh (no live tail), a
