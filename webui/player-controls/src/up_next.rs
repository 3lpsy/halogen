...
/// Show the same continuation target as auto-advance and the list marker, or nothing at list end. Artwork opens details
/// and collapses the player; tapping text runs its marquee even when reduced motion normally pauses it.
#[component]
    ...
    rsx! {
        div { class: "flex items-center gap-2 w-full min-w-0",
            ...
            button {
                ...
                onclick: move |_| {
                    nav.push(format!("/episodes/{id}", id = id));
                    on_close.call(());
                },
                Artwork {
                    src: art_small,
