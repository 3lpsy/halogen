//! Online-only admin user list/create/edit/delete with direct requests and no cache or outbox. Disable mutations
//! offline; the server independently enforces admin access and self-delete rules.
...
use halogen_webui_view_user_edit::AccountDetailsForm;
            ...
            div { class: "flex items-center justify-between",
                ...
                div { class: "flex items-center gap-2",
                    ...
                    button {
                        ...
                        disabled: is_offline,
                        onclick: move |_| { nav.push("/admin/users/create"); },
                        "Create user"
                    }
                }
            }
                ...
                resource_list_view(
                    ...
                    |user: UserData| rsx! {
                        UserRow {
                            ...
                            is_offline,
                            on_edit: move |id| { nav.push(format!("/admin/users/{id}/edit", id = id)); },
...
/// Create users online with UserStoreData validation: username 3-64, password 8-256, matching confirmation. Show
/// local/server errors inline, then toast and return to the list on success. The server independently enforces admin
/// authorization.
#[component]
...
/// Admin edit page for another user, reuses [`AccountDetailsForm`] with the Is Admin toggle enabled. Online-only: the
/// target is fetched live via `GET /users/{id}`. There's no password field here (admins can't change another user's
/// password). The server enforces admin-only access independently; this `is_admin` check is a friendly guard for a deep
/// link.
#[component]
pub fn AdminUserEdit(id: i32) -> Element {
    ...
    // `use_resource` is a hook, keep it ABOVE the admin guard so the hook count is stable. `use_is_admin` resolves
    // asynchronously and can flip false→true while mounted; a hook after the early-returning guard would corrupt hook
    // indices on that flip. (A non-admin deep link triggers one harmless `get_user` the server rejects; the guard below
    // returns before the result is ever rendered.)
                ...
                resource_view(&*user.read_unchecked(), "user", |u: &UserData| {
                    rsx! {
                        AccountDetailsForm {
                            ...
                            on_saved: move |_| { nav.replace("/admin/users"); },
                        }
                    }
                })
            }
