...
use halogen_webui_media::LocalAudio;

// ============================================================================
// Player Types
// ============================================================================

/// Source for audio playback. Never the origin feed URL. The inner value is read by the web + webview backends; the
/// renderless native `NoopPlayerBackend` ignores it, so `dead_code` is allowed for that build.
#[derive(Debug, Clone)]
#[allow(dead_code)]
pub enum MediaSource {
    /// The device's stored copy, as resolved by the media store. Web: a `blob:` object URL + the blob's MIME type, the
    /// backend takes ownership of the URL (revoking it once displaced) and loads it through a `<source type=…>` child,
    /// since WebKit resolves `blob:` media strictly by type. Native: `url` is an absolute file path (the webview
    /// backend maps it to the `halogen-local-audio` loopback media-server URL).
    Local(LocalAudio),
...
/// Session-only playlist continuation context shared by auto-advance, transport, and next-up UI; None means queue. A
/// newtype gets its own Dioxus context slot and survives NowPlaying replacement between tracks.
...
/// Project only episode ID and coarse playback state into a PartialEq memo. Position ticks then recompute one
/// projection without invalidating every visible row's state memo.
#[derive(Debug, Clone, PartialEq)]
...
pub trait PlayerBackend {
    ...
    /// Unlock the backend for a `play()` that will happen OUTSIDE the current user gesture (the Download & Play path:
    /// the real play fires when the download lands, seconds after the click, where browser autoplay policy rejects it
    /// with `NotAllowedError` until the element has played once via a gesture). Must be called synchronously from the
    /// gesture's event handler. Default no-op: only backends with an autoplay policy (web) need it.
    fn prime(&self) {}
}
...
pub enum PlayerEvent {
    /// The playback clock, and only while playback is progressing (element not paused + data for the current position).
    /// This is a CONTRACT, not a courtesy: the controller flips `Loading → Playing` on the first `TimeUpdate` after a
    /// load, and its loading-stall guard and device-copy error retry both act only while `Loading`, a backend that
    /// ticks during resource selection fakes the start and disarms both.
    TimeUpdate(f64),
    DurationKnown(f64),
    Buffering(bool),
    /// The element entered the *paused* state on its own, i.e. the OS paused it, not the app. Mobile browsers do this
    /// on an audio-route change (a speaker / Bluetooth headset / wired headphones disconnect), an incoming call, or
    /// another app taking the audio focus. The controller reflects it so the UI and the OS lock-screen controls match
    /// what's audible, instead of staying stuck "Playing" while the element is silent.
    Paused,
