//! Edit the active user's username/password through direct online requests with inline errors. The route guards active
//! identity; the server independently enforces self-or-admin username edits and bearer-owned password changes.
...
use halogen_webui_accounts::accounts::AccountsStore;
        ...
        FormPage {
            ...
            div { class: "space-y-8",
                AccountDetailsForm {
                    ...
                    on_saved: move |_| { nav.replace("/settings"); },
                }
                PasswordForm {}
            }
        }
...
/// Edit username online; admin flows may also send another user's admin flag, hidden/omitted for self-service. After
/// success, update any matching device-registry name before on_saved navigation so account labels refresh immediately.
#[component]
pub fn AccountDetailsForm(
    ...
    let admin_unchanged = admin.is_none_or(|orig| orig == is_admin());
    ...
    let on_submit = move |evt: FormEvent| {
        ...
        form.spawn_submit(
            ...
            move |user| async move {
                // Rename only the matching (kind, id, server) registry slot, preserving reauth and other metadata.
                // Equal IDs on other servers must remain untouched; absent local accounts are a no-op.
