...
/// Shared `<select>` value mapping for the small string-keyed preference enums. Each enum supplies only its variant
/// table (`ALL`) and the stable serde token (`as_str`); the round-trip parse (`from_str_or_default`) is provided once
/// here, an unknown token falls back to the enum's `Default`. Callers that invoke `Enum::from_str_or_default(s)` must
/// have this trait in scope.
...
/// Generate a serde select enum, ALL variants, stable tokens, labels, and SelectEnum from one variant list. Mark its
/// default with #[default]; add specialized methods in a separate impl.
...
select_enum! {
    /// How the play button sources audio, the app is local-first, so the default never streams. Governs PLAY behavior
    /// only: the download badge and context-menu download actions always work manually, and a device copy plays offline
    /// in every mode.
...
select_enum! {
    /// Per-request byte size for device downloads. A device download pulls the server's audio copy in HTTP `Range`
    /// chunks so a dropped connection only re-fetches the chunk it lost; this picks how big each chunk is. `NoChunking`
    /// fetches the whole file in a single request (no `Range` windowing), simplest, but buffers the entire body in
    /// memory.
