//! Settings → Server (`/settings/server`): server identity (type/URL, the embedded server's live status + data
//! location), the admin database export/import (the server↔server / embedded↔server migration and full-backup story),
//! and the admin-only server surfaces (runtime config, polling history, server logs).
...
use halogen_webui_provider_local::embedded::{self, EmbeddedState};
    ...
    let export_db = move |_| {
        ...
        spawn(async move {
            let outcome = async {
                ...
                halogen_webui_logging::download_bytes(
        ...
        spawn(async move {
            ...
            match outcome {
                Ok(summary) => {
                    ...
                    {
                        halogen_webui_provider_local::embedded_session::align_imported_users(
                ...
                if is_admin() {
                    div { class: "space-y-2 pt-2",
                        ...
                        SettingsRow { label: "Server configuration",
                            button {
                                class: "btn btn-primary btn-sm",
                                onclick: move |_| { nav.push("/settings/config"); },
                                "View config"
                            }
                        }
                        SettingsRow { label: "Server Polling History",
                            button {
                                class: "btn btn-primary btn-sm",
                                onclick: move |_| { nav.push("/polling"); },
                                "View History"
                            }
                        }
                        ...
                        if !embedded_mode {
                            SettingsRow { label: "Server Logs",
                                button {
                                    class: "btn btn-primary btn-sm",
                                    onclick: move |_| { nav.push("/admin/logs"); },
                                    "View Server Logs"
                                }
                            }
                        }
                        SettingsRow { label: "Server Errors",
                            button {
                                class: "btn btn-primary btn-sm",
                                onclick: move |_| { nav.push("/admin/errors"); },
                                "View Errors"
                            }
                        }
                    }
