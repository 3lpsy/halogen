//! Read-only episode metadata page (`/episodes/:id/metadata`). Renders EVERY field of the [`EpisodeData`] row, grouped
//! into labelled sections (same shape as `pages/view_config`). The nested embeds (`podcast` / `playback` / `chapters`)
//! are summarized as one row each, not expanded. Reached from the episode kebab menu's "View metadata". The row comes
//! from the cached pool, with the usual deep-link fetch on a cold-store miss.
