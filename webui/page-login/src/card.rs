//! Shared shell for the unauthenticated screens (Login + EmbeddedServerSetup). Both are a centered "Halogen" card: a
//! title + subtitle over a `bg-sidebar` box holding a `space-y-4` form, an inline error line, and a full-width submit
//! button whose label flips while a request is in flight. Only the inputs and the submit handler differ, so those are
//! the caller's (`children` + `onsubmit`).
