//! Confirm local-only setup, provision the runtime, and sign in with stored local credentials. RootGuard also routes
//! local-account reconnection here; those accounts do not need a user-entered password.

use dioxus::prelude::*;

use super::card::AuthCard;
use crate::root_guard::PendingAuthRedirect;
use halogen_webui_hooks::use_accounts;
use halogen_webui_provider_local::{embedded, embedded_session};
    ...
    let onsubmit = move |e: FormEvent| {
        ...
        let _ = spawn(async move {
            ...
            match embedded_session::prepare_embedded_activation(accounts).await {
                Ok(reg) => {
                    // Same contract as Login: navigate first, set the registry last, the set schedules the keyed
                    // remount that tears this page down. Land back on the deep link the auth redirect displaced (falls
                    // back to Home).
