use dioxus::prelude::*;
use halogen_apiclient::{ApiClient, LoginData};

use super::card::AuthCard;
use super::{AuthErrorContext, auth_error_message};
use crate::root_guard::PendingAuthRedirect;
use halogen_webui_accounts::accounts::{AccountsStore, StoredAccount, jwt_sub};
use halogen_webui_component_toast::{ApiResultExt, ToastPolicy};
use halogen_webui_config::ClientConfigStore;
use halogen_webui_hooks::{use_accounts, use_config, use_toast};
use halogen_webui_logging::info;

/// Default value for the server URL input when no server is known yet. On wasm we dogfood the web app served from the
/// same origin as the API, so prefill the origin the app was pulled from. On native there's no meaningful origin, so
/// start empty.
...
pub fn Login() -> Element {
        ...
        let _ = spawn(async move {
            ...
            match result {
                Ok(token_data) => {
                    ...
                    // Save fresh auth into this server-scoped user namespace, preserving prior preferences. Use
                    // save_for before the ambient namespace switches so equal user IDs on different servers cannot
                    // share config or outbox.
                    let server = halogen_webui_config::server_hash(&server_url);
                    let key = halogen_webui_config::AccountKey::remote(id, server);
                    ...
                    cfg.server_kind = halogen_webui_config::ServerKind::Remote;
                    cfg.server_setup = true;
                    cfg.access_token = Some(token_data.token);
                    cfg.is_admin = is_admin;
                    ClientConfigStore::save_for(key, &cfg).await;

                    // Register + activate. Flipping `active_user_id` remounts the data subtree under this user's
                    // namespace (worker auths, stores reopen; the cookie was set by the login response). The `set` is
                    // last with no `.await` after it, so persistence + navigation finish before the remount tears this
                    // page down.
                    ...
                    reg.upsert(StoredAccount {
                        ...
                        username,
                        needs_reauth: false,
                        kind: halogen_webui_config::ServerKind::Remote,
                        server,
                    });
                    reg.set_active(Some(key));
                    AccountsStore::save(&reg).await;

                    // Reflect the new auth in the live config immediately. Re-login of the SAME active user doesn't
                    // change `active_user_id`, so the data subtree does NOT remount, without this the worker +
                    // RootGuard would keep the old (unauthenticated) config and loop. A *different* user instead
                    // remounts via `accounts.set` and reloads this from storage.
                    config_sig.set(cfg);
    ...
    let embedded_footer = halogen_webui_provider_local::embedded::available().then(|| {
        rsx! {
            ...
            button {
                ...
                onclick: move |_| {
                    let _ = nav.replace("/auth/embedded-setup");
                },
                "Use Embedded Server"
            }
        }
