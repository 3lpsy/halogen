//! Exercise the controller in a real VirtualDom using recording backend/media stores and a command-draining coroutine.
//! Thread-local handles let tests mutate within the runtime and pump until quiet; nextest isolates processes.
...
use halogen_webui_app_state::{ClientDownloadState, DownloadState, EpisodeState, PlaylistState};
use halogen_webui_commands::Command;
use halogen_webui_config::{ClientConfig, PlaybackPreference, ServerKind};
use halogen_webui_media::{LocalAudio, MediaStore, MediaWriter, PartialInfo};
...
enum Action {
    ...
    Play,
    Toggle,
    /// One controller poll tick. Routed through here (unlike the older bare-`in_runtime` tick tests) for paths where
    /// `tick` itself spawns, the local-error retry schedules its delayed re-resolve from `drain_events`, which needs a
    /// current scope, exactly like production's `use_future` tick loop provides.
    Tick,
}
...
fn Harness() -> Element {
    ...
    let config = use_signal(|| ClientConfig {
...
/// Mark the config as local-runtime mode (forces StreamOnly at the
...
/// Embedded-server mode forces StreamOnly at the controller's preference chokepoint regardless of the STORED preference
/// (which is preserved for a later switch back to remote): play streams from the built-in server and the device
/// pipeline never fires, even under DownloadOnly, whose remote-mode invariant is the exact opposite.
#[tokio::test]
...
// ════════════════════════════════════════════════════════════════════════════ Local-error retry: transient device-copy
// load failures (iOS WebKit's freshly-committed IndexedDB blob can fail to load, then read fine moments later, the
// controller re-resolves the source before surfacing an error)
// ════════════════════════════════════════════════════════════════════════════
...
// ── The OS media-session `play` action (PlayerController::play) ────────────── Regression cover for the lock-screen
// asymmetry: `pause` accepts `Playing`, but `resume` is a no-op outside `Paused`, so binding the OS play button
// straight to `resume` left it DEAD from `Ended`/`Error` while pause kept working. `play()` gives the OS action
// `toggle`'s state routing.
...
// Invoke ScopeBound handlers outside any runtime to verify device-copy playback can spawn safely; streaming never
// exercises that path. Browser-only closure wiring, stale-generation guards, and listener cleanup remain wasm
// compile/manual checks.
...
async fn scope_bound_nests_under_bare_in_runtime() {
    ...
    let c = controller();
    let handler = scope_bound().bind(move || c.play_next_episode());
