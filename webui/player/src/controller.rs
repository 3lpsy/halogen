//! The [`PlayerController`], drives a [`PlayerBackend`] and reflects playback into the dedicated `now_playing` signal.
//! The controller lives here; `lib.rs` is the crate shell, the player value types and the backend trait live in
//! `types`, and the no-op backend in `native`, so the playback state machine has its own home.
...
use halogen_webui_app_state::PlaybackState as PlaybackOverlay;
use halogen_webui_app_state::{DownloadState, EpisodeState, PlaylistState};
use halogen_webui_commands::Command;
use halogen_webui_config::{ClientConfig, PlaybackPreference};
use halogen_webui_logging::{debug, info, warn};
use halogen_webui_media::MediaStore;

/// Reflect backend playback into a separate `now_playing` signal so 250ms position writes do not rerender episode
/// subscribers. Episode state is read-only here; clones share UI-thread-only Rc handles.
#[derive(Clone)]
pub struct PlayerController {
    ...
    /// The playback continuation context (see [`PlayContext`]): set by the user-entry `*_in` play methods, preserved by
    /// the continuation paths (auto-advance, transport next/prev, stream fallback), cleared by [`Self::stop`]. Peeked
    /// by navigation/auto-advance; UI "up next" surfaces subscribe via their own hook.
    ...
    /// Count Preparing ticks without download progress. Reset on progress or state exit; this catches dropped commands
    /// and stuck downloads that the Loading stall guard never polls.
    ...
    /// A local-error retry is scheduled but hasn't fired: `tick` discards backend events (the aborted load's
    /// stragglers, a late play-promise rejection would otherwise burn a second retry) and freezes the state machine in
    /// `Loading` until the retry re-resolves the source. Cleared by the retry firing or by any new load
    /// (`bump_generation`).
...
/// Ticks (`TICK_INTERVAL_MS` apart) the player may sit in `Preparing` without the device download reaching a terminal
/// state before the stall guard surfaces an error. Generous (downloads legitimately take a while) but bounded, so a
/// dropped `DownloadToDevice` command or a stuck worker can't hang the spinner forever. ~5min at 250ms/tick.
const PREPARING_STALL_TICKS: u32 = 300_000 / TICK_INTERVAL_MS as u32;

/// Local-source retry delays in milliseconds; array length sets the budget. Re-resolving can recover freshly committed
/// WebKit blobs whose backing files have not settled, while persistent corruption eventually reports an error.
const LOCAL_RETRY_DELAYS_MS: [u32; 2] = [750, 2_000];

impl PlayerController {
    #[allow(
        clippy::too_many_arguments,
        reason = "Independent reactive handles preserve scoped subscriptions"
    )]
    pub fn new(
    ...
    /// The active playback preference (peeked, no subscription). Embedded-server mode forces `StreamOnly` at this
    /// single chokepoint: the "server" is this device, so a device copy would duplicate every byte for nothing,
    /// streaming from loopback IS local playback. The stored preference is left untouched so switching back to a remote
    /// account restores the user's real setting.
    ...
    fn update_now_playing(&self, f: impl FnOnce(&mut NowPlaying)) {
        // `peek()` avoids subscribing/notifying. `Signal::write()`/`set()` notify every subscriber on drop even when
        // nothing changed, and the web backend re-reports DurationKnown/Buffering each poll (~4×/sec); value-gate the
        // write so an unchanged poll (or a paused player) doesn't re-render the player UI. `now_playing` has dedicated
        // player-UI subscribers (not the shared EpisodeState), so gating it is safe.
    ...
    pub(super) fn saved_cursor(&self, episode_id: i32) -> f64 {
        self.playbacks
            ...
            .map(|p| p.cursor as f64)
            .unwrap_or(0.0)
    }

    /// Load `src` into the backend and reflect `Loading`.
    pub(super) fn load_and_play(&self, episode_id: i32, src: MediaSource, start_at: f64) {
        // Every real playback start funnels through here (device + stream, whether from the transport, the queue
        // auto-advance, the "Stream" menu/swipe, or the download-then-play watcher), so this is the one place to
        // auto-arm the sleep timer. Idempotent via the once-per-session latch, so auto-advance/replays don't re-arm it.
    ...
    /// Route a backend `PlayerEvent::Error`: device-copy loads get a bounded retry (fresh source resolution, a fresh
    /// IndexedDB read and a fresh object URL) before the error is surfaced, because a freshly-committed blob can
    /// transiently fail to load on iOS WebKit while a re-read moments later succeeds (see `LOCAL_RETRY_DELAYS_MS`).
    /// Stream errors and exhausted budgets latch `Error` exactly as before.
    ...
    /// Play/pause toggle, the in-app transport (mini-player, now-playing sheet, episode rows). Both directions delegate
    /// to [`Self::pause`] / [`Self::play`], so the "what does play mean from state X" routing lives in exactly ONE
    /// place and can't drift from the OS media-session handlers, which drive those two directional intents separately
    ...
    /// Handle directional play intent for OS controls. Unlike `resume`, this also restarts Ended/Error states; routing
    /// directly to resume would leave the OS play button inert after completion or failure.
    pub fn play(&self) {
    ...
    /// Resume a **paused** player, the low-level primitive behind [`Self::play`]. Guarded to `Paused` only: from any
    /// other state a flip to `Playing` is wrong (`Preparing` wedges the download handoff and zeroes the saved cursor;
    /// `Loading` is already headed to `Playing`; `Ended`/`Error` need a real re-play). Callers that mean "start
    /// playing" want [`Self::play`], which routes those states correctly instead of silently doing nothing.
    pub fn resume(&self) {
    ...
    /// Pause a **playing/loading** player. Guarded for the same reason as [`Self::resume`]: from `Preparing` a flip to
    /// `Paused` bypasses `persist_cursor`'s `Preparing|Error` skip and writes cursor 0, erasing the episode's saved
    /// resume position; `Paused`/`Ended`/`Error` have nothing to pause.
    pub fn pause(&self) {
    ...
    // ── Sleep timer ───────────────────────────────────────────────────────── The timer's state + rules live in
    // `sleep.rs`; these methods keep the public surface stable and forward to the `sleep::*` orchestration over the
    // `Signal<SleepState>` (the controller still owns the signal + the once-per-session auto-arm latch, and pauses on
    // the expiry edge `advance` reports).
    ...
    /// The transport Next target (delegates to [`navigation::next_episode`]): the playlist/queue continuation
    /// ([`PlaylistState::next_up_in`] with the play context) when there is one, else the podcast-order neighbor so you
    /// can keep bingeing a podcast whose episode isn't queued. Auto-advance (on episode end) uses `next_up_in`
    ...
    pub fn persist_cursor(&self) {
        if let Some(np) = self.now_playing.peek().clone() {
            // Persist only resumable states. Preparing/Error contain artificial zero positions; Ended already wrote
            // Finished with cursor zero, so saving its duration again would undo completion and replay at the end.
            if matches!(
                np.state,
    ...
    pub fn tick(&self) {
        ...
        // A local-error retry is pending: swallow the aborted load's straggler events (a late play-promise rejection or
        // the element's re-reported error would burn a second retry, and its TimeUpdate would fake a Loading→Playing
        // flip) and hold the state machine in `Loading` until the retry re-resolves the source. Bounded by
        // LOCAL_RETRY_DELAYS_MS.
    ...
    /// Poll only active states to avoid idle rerenders and repeated Ended/MarkPlayed events. `peek()` neither
    /// subscribes nor notifies.
    ...
    fn drain_events(&self) -> bool {
        ...
        for ev in events {
            match ev {
                ...
                // The OS paused the element on its own (audio-route change when a speaker/headset disconnects, an
                // incoming call, another app taking the audio focus). Only act on the Playing→Paused edge: our own
                // `pause()` already set Paused (so this is a no-op then), and a still-Loading element hasn't started
                // playing yet. Persist the cursor as a real pause would, so the position isn't lost.
                PlayerEvent::Paused => {
                    if matches!(
                        self.now_playing.peek().as_ref().map(|n| &n.state),
    ...
    /// Loading-stall guard: the controller only flips `Loading → Playing` on the first `TimeUpdate`. If a remote stream
    /// never reaches `HAVE_METADATA` (stalled network), that tick never comes and the player spins in `Loading`
    /// forever, surface an error after ~30s instead. Returns `true` when the stall fired (the rest of the tick should
    /// be skipped).
    ...
    /// Fail Preparing after `PREPARING_STALL_TICKS` without progress, covering dropped download commands or missing
    /// terminal reports. Return true while Preparing so tick skips backend polling.
    ...
    /// Debounced cursor persistence (~every 40th tick ≈ 10s), and only while playing, a paused player isn't advancing,
    /// so there's nothing new to save. Each persist is a `SetCursor` command that publishes `EpisodeState`, so keep it
    /// coarse to avoid re-rendering the episode lists. Returns `true` when the sleep timer expired on this tick (so
    /// `tick` can skip a coincident auto-advance).
    ...
    /// Mark the episode played and latch Ended, then optionally advance within the play context or queue. No podcast
    /// fallback: absent a next item, remain Ended.
    fn on_ended(&self, allow_advance: bool) {
        ...
        // Latch Ended synchronously before async next-source resolution. Otherwise the still-pollable backend repeats
        // ended and dispatches MarkPlayed/request_play until resolution sets Loading or Preparing.
