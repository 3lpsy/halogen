//! Pure client-side search + sort for the podcasts list. Podcast sets are small and held fully in the offline-first
//! pool (`EpisodeState::podcasts_by_id`), so we filter + sort in memory rather than round-tripping the server. Mirrors
//! `episode_list::list::apply_filter_sort`, but only the two facets that make sense for podcasts: a title/author search
//! and a Title / Added sort. The episode-only `FilterSpec` fields (chips, `podcast_id`, …) are intentionally ignored.
