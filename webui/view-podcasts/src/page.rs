...
use halogen_webui_component_icons::ChevronRight;
...
/// Podcasts page, subscribe to a feed and list every subscribed podcast, **offline-first and paged**: render a window
/// over the cached pool (`podcasts_by_id`, hydrated from the local store on boot), and revalidate it page-by-page from
/// the server. Rendering subscribes to the pool, so the worker's `CachePodcasts` upsert re-renders us, no store-read
/// race. Each card links to the podcast detail page.
#[component]
pub fn Podcasts() -> Element {
    ...
    // Offline-first paged pool: render a window over the cached podcast pool and revalidate it page-by-page (the worker
    // upserts each fetched page into `podcasts_by_id` + the store and publishes, re-rendering us). The fetch plan has
    // no reactive inputs beyond the cursor/config; sort + search are applied client-side over the pool below.
    ...
    rsx! {
    // Bare, full-width framing to match the episode-list pages (latest/queue/ history/downloads): no page padding or
    // in-page title (the nav names the page), the controls bar over a full-bleed, flush card list. The shared
    // `PagedListPage` owns the frame, scroll container, error/empty boilerplate and the bottom sentinel; this page
    // keeps the data wiring, pull-to-refresh, and the per-row card markup.
    PagedListPage {
        id_prefix: "podcast",
        error,
        loading,
        initial_settled: pool.initial_settled,
        ...
        for p in rows {
            {
                            ...
                            rsx! {
                                // Clickable card via a stretched Link (an <a>): keyboard-focusable + SPA nav, and its
                                // `after:inset-0` pseudo-element covers the whole `relative` card so a tap anywhere
                                // still navigates. The kebab is a sibling raised above the overlay (`relative z-10`) so
                                // it stays independently clickable, no nested interactive.
                                div {
                                    key: "{pid}",
                                    class: "card bg-base-100 shadow-sm border border-base-300 hover:bg-base-200 transition-colors cursor-pointer relative",
                                    // Skip offscreen layout while retaining DOM rows for scroll restoration. Match the
                                    // fixed card height in rem so font scaling cannot under-reserve space and cause
                                    // cumulative shifts; `auto` remembers measured height after first paint.
