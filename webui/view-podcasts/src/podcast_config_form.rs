//! Create requires online access to allocate and link a config ID; edit/remove can queue optimistic offline changes.
//! Always send all three whole-number overrides because updates cannot clear fields to NULL. Remove restores global
//! defaults.
    ...
    let on_submit = move |evt: FormEvent| {
        ...
        if is_offline {
            if let FormMode::Edit { config_id } = mode {
                ...
                nav.replace(format!("/podcasts/{id}", id = podcast_id));
            }
            return;
        }
