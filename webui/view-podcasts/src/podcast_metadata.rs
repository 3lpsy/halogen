//! Read-only podcast metadata page (`/podcasts/:id/metadata`). Renders EVERY field of the [`PodcastData`] row, grouped
//! into labelled sections (same shape as `pages/view_config`). Reached from the podcast kebab menu's "View metadata".
//! The row comes from the cached pool, with the usual deep-link fetch on a cold-store miss.
