//! Add a podcast by feed URL. Online-only. Reached from the Discover page's "Add by URL" action (moved here from the
//! Settings page). Validates the URL client-side, then subscribes through the worker, the same canonical
//! `Command::Subscribe` path used everywhere else.

use dioxus::prelude::*;

use crate::components::BackButton;
use halogen_webui_commands::actions as commands;
use halogen_webui_hooks::{use_dispatch, use_sync_status, use_toast};
