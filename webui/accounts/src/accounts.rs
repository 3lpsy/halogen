//! Device-global index of switchable accounts and the active identity; full config/auth/preferences stay namespaced per
//! account. Switching remounts the user subtree and refreshes the HttpOnly media cookie, which is never stored in this
//! registry.

use serde::{Deserialize, Serialize};

use halogen_webui_config::{AccountKey, ServerKind};

/// Switcher metadata for one server-scoped account; token/admin/preferences live in its namespaced config. Rejected
/// stored auth sets needs_reauth for prefilled login. Server kind defaults to Remote for legacy registries; identity
/// must distinguish equal user IDs across servers.
...
impl Accounts {
    ...
    pub fn active_account(&self) -> Option<&StoredAccount> {
        ...
        self.users.iter().find(|account| account.key() == key)
    }
...
/// Persistent storage backend for [`Accounts`]. Web: IndexedDB `halogen.accounts`, store `registry`, record
/// `"accounts"`. Native: JSON file at `<config root>/accounts.json` (see `halogen_webui_platform::paths::config_root`).
...
impl AccountsStore {
    ...
    pub async fn save(accounts: &Accounts) {
        ...
        halogen_webui_platform::kv_save!("", accounts_path(), accounts);
    }
    ...
    async fn load_raw() -> Option<Accounts> {
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            halogen_webui_platform::kv_load_opt!(Accounts, "", accounts_path())
        }
    }
}
...
fn accounts_path() -> std::path::PathBuf {
    halogen_webui_platform::paths::config_root().join("accounts.json")
}
...
mod tests {
    ...
    fn same_id_different_remote_server_are_distinct() {
        let sx = halogen_webui_config::server_hash("https://server-x.example");
        let sy = halogen_webui_config::server_hash("https://server-y.example");
