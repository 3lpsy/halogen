//! Native-only JSON persistence macros keep target-specific path expressions out of wasm compilation. Accept but
//! discard web_key without emitting it, so native code never references wasm-only constants.
...
/// `kv_try_load!(Type, web_key, native_path)` → `Result<Option<Type>, String>`: `Ok(None)` only for a genuinely ABSENT
/// file; a read failure or corrupt JSON is an `Err`. For values where "couldn't read" must not silently become "use
/// defaults", a defaulted auth config, later saved, permanently overwrites the real one (`kv_load_opt!` collapses all
/// three cases and is only safe for re-derivable preferences).
#[macro_export]
