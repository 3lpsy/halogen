//! Use origin-scoped Web Locks across tabs and workers to serialize outbox drains and audio writers. Locks release when
//! a context dies. Dynamic API lookup works in either global; missing or failed support returns Unsupported and callers
//! proceed unguarded.
...
async fn request(name: &str, if_available: bool) -> TryLock {
    ...
    // If the request promise REJECTS before the callback fired (pathological: bad name, API quirk), unblock the waiter
    // so callers proceed unguarded instead of hanging forever on `granted_rx`. Attached via `Reflect` like the rest of
    // this module (the typed `Promise::catch` wants a `ScopedClosure`, which doesn't fit a fire-once detached handler).
