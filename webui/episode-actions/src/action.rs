//! Resolve configured swipe toggles against live row flags and dispatch the same command/player/navigation primitives
//! as menus. Keep action mapping centralized.
...
use halogen_webui_listview::SwipeAction;
...
/// Perform one episode action against the row's live context, firing the same `commands::*` / player / nav primitives
/// the kebab menu uses. The single copy of the action→command mapping for the gesture path. Returns whether anything
/// was dispatched, the queue/list actions no-op when their target (`queue_id` / `playlist_id`) hasn't resolved or
