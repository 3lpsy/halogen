//! Customize-swipes screen (`/settings/configure-swipes`). Frontend-only editor for `ClientConfig.swipe_prefs`: per
//! list page, pick the action bound to each swipe direction. Persisted live to localStorage; the list pages read the
//! config and feed it straight into `EpisodeList` (each row resolves and performs the action itself, see
//! `EpisodeListItem`).

use dioxus::prelude::*;

use crate::components::{BackButton, SwipeAction};
use halogen_webui_config::SwipePage;
