//! Server Errors page (`/admin/errors`, admin-only). Renders the server's persisted failure histories from `GET
//! /admin/server-errors`: podcast RSS sync failures and episode media-download failures, newest first, each with the
//! failure reason the server recorded. Online-only (the histories live in the server DB).

use dioxus::prelude::*;
use halogen_wire::{EpisodeDownloadErrorData, PodcastSyncErrorData};

use crate::components::BackButton;
use halogen_webui_component_icons::ClockRotateLeft;
            ...
            for row in rows {
                div {
                    ...
                    div { class: "flex items-start justify-between gap-2",
                        Link {
                            to: format!("/podcasts/{id}", id = row.podcast_id),
        ...
        div { class: "space-y-2",
            ...
            for row in rows {
                div {
                    ...
                    div { class: "flex items-start justify-between gap-2",
                        Link {
                            to: format!("/episodes/{id}", id = row.episode_id),
                            class: "font-medium text-primary hover:underline truncate",
