...
/// Conservative lower bound on the serialized size of one NDJSON [`LogLine`] (the four required keys + braces/quotes
/// already exceed this even with empty strings). Used only to gate the prune read: a file smaller than `2×CAP` of these
/// can't hold `2×CAP` lines, so it can't need pruning. Under-estimating is the safe direction, it can only make the
/// cheap gate trip a little early, never skip a prune that's due.
...
fn log_path() -> Option<PathBuf> {
    ...
    Some(halogen_webui_platform::paths::data_root().join("device.log"))
}
