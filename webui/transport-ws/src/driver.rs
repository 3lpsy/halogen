//! Reconnect with backoff and use application ping/pong for RTT and silent-drop detection. Browser APIs expose no
//! protocol ping frames; the server echoes the client's timestamp.
...
/// Run until cancelled, retrying with backoff except a ticket-mint 401 that emits AuthExpired. The client snapshots
/// auth, so token changes cancel this driver and spawn one with fresh credentials.
...
async fn drive(api: ApiClient, emit: EventSink) {
    ...
    loop {
        ...
        // Whatever happened, we're now disconnected. Reset backoff ONLY when the connection was confirmed live (a first
        // pong) AND stayed up for at least `MIN_LIVE_SESSION_MS`, otherwise a web socket that "opens" against a down
        // server, or a confirm-then-drop server that pongs once and bails, would reset every cycle and hammer
...
/// Drive one live connection: ping on a timer, surface pong RTTs, and return when the socket closes/errors or the peer
/// goes silent past [`MAX_MISSED_PONGS`]. Returns `true` once at least one pong proved the link live (we emit
/// [`ConnectionEvent::Connected`] on that first pong, not on mere socket-open, so "online" always means a real
