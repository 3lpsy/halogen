...
pub fn PlaybackPrefsForm(
    ...
    // Local mirrors of the two skip fields' text, so a transient empty/partial edit (e.g. clearing the field before
    // typing a new number) isn't snapped back mid-keystroke. A parseable value is committed to `prefs`; an
    // empty/invalid one is held in the local string without fighting the user (B11). Seeded from the current prefs; the
    // bound `value` reads the local string from here on.
    ...
    // Memoize each text-mirrored preference independently and update only when its represented value differs. Async
    // resets then refresh inputs without unrelated commits overwriting partial or equivalent in-progress edits.
