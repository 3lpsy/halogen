//! Render merged local/server field messages under inputs and non-field failures below submit. FormErrors supplies the
//! shared mapping; local validation gates submission and touched-field feedback.

use dioxus::prelude::*;

// `FormErrors` lives in `halogen-webui-component-forms` (pure data, shared with the `use_form` hook in
// ui-state). Re-exported so the `crate::FormErrors` facade — and the
// form pages that use it — stay unchanged.
pub use halogen_webui_component_forms::FormErrors;
...
/// Shared labeled text/number/password/URL or textarea input with merged field errors. value binds two ways, on_input
/// clears stale feedback, and optional touched flips on blur to defer required errors.
#[component]
pub fn InputField(
    label: String,
...
/// A primary-styled checkbox toggle row, `[x] label` with an optional `hint` paragraph below, the shared shape for the
/// boolean toggles on the create/edit forms (`playlist_form`'s "make default", `podcast_config_form`'s "auto-download",
/// `user_edit`'s "Is Admin"). Distinct from [`CheckboxField`], the muted settings-style checkbox. `onchange` receives
/// the new checked state; `hint` renders only when non-empty.
#[component]
pub fn ToggleField(
    label: String,
    checked: bool,
