//! Root-hosted destructive confirmations share ConfirmModal and preserve caller callbacks. Purge actions clear only
//! local caches; delete actions mutate the server. See the crate README for usage examples.
...
/// What a pending confirmation targets. `on_deleted` is a scope-independent `Rc` closure, not a scope-owned
/// [`Callback`]: this target sits in a root-level signal while the modal is up, and the scope that opened it (a list
/// row, a detail page mid-navigation) can unmount underneath, a `Callback` slot would die with it and panic on fire.
#[derive(Clone)]
...
impl Confirm {
    ...
    /// Pre-bound closure form of [`purge_episode`](Self::purge_episode), what menu/kebab builders want (their actions
    /// are scope-independent `Rc` closures, see [`QuickAction`](crate::QuickAction)), so call sites write
    /// `confirm.purge_episode_callback(id)` instead of hand-rolling the closure each time. `Confirm` is `Copy` and
    /// context-backed, so the closure can never dangle.
