//! Visual indicator for the pull-to-refresh gesture. Rendered as the first child inside a list's scroll container; the
//! gesture itself is wired by [`use_pull_to_refresh`](halogen_webui_hooks::use_pull_to_refresh), which drives the
//! [`PullPhase`] this reads. The navbar and the sort/filter bar sit outside the scroll container, so they never move
//! with the pull.

use dioxus::prelude::*;

use halogen_webui_hooks::{PULL_THRESHOLD, PullPhase};
...
pub fn PullToRefreshIndicator(phase: Signal<PullPhase>, is_admin: bool) -> Element {
    ...
    // `absolute inset-x-0 top-0` overlays the bar on the top of the scroll region (its parent is the `relative` wrapper
    // around `#episode-scroll`) instead of taking an in-flow box. In flow, a background sync/poll flipping Idle→Polling
    // pushed every row down ~64px and back, a top CLS source. As an overlay the list never reflows; the bar briefly
    // covers the first row during a sync.
    rsx! {
        div {
