//! Share sort selection/direction with Escape/Space handling and revealable search with clear action between episode
//! and collection headers, keeping behavior and markup consistent.

use dioxus::prelude::*;

use crate::{FilterSpec, OrderDirection, SortField, SortSpec};
use halogen_webui_component_icons::{ArrowsUpDown, MagnifyingGlass, XMark};
...
/// The search toggle button (rightmost in the header bar), reveals the search input ([`SearchInput`]) on the row below.
/// Caller owns `show_search` so the toggle can sit in the header's button row while the input renders below it. Search
/// isn't persisted (unlike sort/filter), so closing the bar reverts the query rather than leaving a hidden active
/// search.
#[component]
pub fn SearchToggle(
