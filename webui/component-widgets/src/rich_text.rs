//! Safe renderer for untrusted feed HTML (episode/podcast descriptions). [`RichText`] parses the HTML into a
//! whitelisted AST ([`crate::html`]) and renders it through ordinary `rsx!`, text is escaped, only `p`/headings/
//! lists/bold and vetted links survive. Links are NOT navigated directly: they call back so the page can confirm via
//! [`ConfirmLinkModal`] first. On a parse error the raw string is shown as escaped text (tags visible).
