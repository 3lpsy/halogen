//! Web device-log persistence: IndexedDB `halogen.logs`, object store `logs`
//! with auto-increment keys. Each record is a JSON-encoded
//! [`LogLine`](super::super::LogLine) string. On append we prune the oldest
//! records once the store exceeds [`CAP`](super::super::CAP).
//!
//! The store is opened per call (device-log writes are infrequent and batched,
//! so a cached connection isn't worth the `Rc<RefCell>` dance the media store
//! needs).
