//! Remember a list's view state (sort / filter chips / search) across navigation
//! and restarts.
//!
//! Two mirrors of one state:
//! - **URL query** (web) — live, shareable, and restored by the browser
//!   back/forward + reload. Written with `history.replaceState` so it neither
//!   spams history nor triggers a router round-trip / re-render.
//! - **Sticky `ListViews[key]`** — its own persisted store (`ListViewStore`),
//!   kept separate from the auth-bearing `ClientConfig`. Survives fresh navigation
//!   (via the nav bar) and app restarts. Written debounced so rapid changes don't
//!   rewrite storage; the dedicated `Signal<ListViews>` has no reactive readers,
//!   so a write re-renders no `use_config` subscriber and never reaches the worker.
//!
//! Seeding precedence at mount: URL query → sticky config → page defaults.
//! `ConfigProvider` gates the whole app behind a splash until the config has
//! loaded, so the seed's non-reactive `peek` is always complete.
//!
//! Why a *captured* query, not the live one: the Dioxus router normalizes the
//! address bar to the canonical route path on init (a route like `Queue {}`
//! serializes to `/queue`, with no query), so it strips our query string *before*
//! any list mounts. [`capture_initial_query`] runs in `main()` — before
//! `dioxus::launch` builds the Router — to snapshot the pristine path + query; the
//! seed then consumes that (path-matched, once) instead of the now-stripped
//! `location.search`.

use dioxus::prelude::*;

use super::use_latest_wins::use_latest_wins;
use halogen_ui_config::{ListViewStore, ListViews, StoredListView};
...
/// Create the `sort` / `filter` signals for a list, seeded from remembered state,
/// and keep both mirrors in sync as they change.
///
/// `list_key` is the list *type* (`"latest"`, `"podcast"`, `"queue"`, …) — shared
/// across ids, so e.g. a podcast-episode sort preference applies to every podcast.
/// `base_filter` carries page-supplied scoping (`podcast_id`, `published_after`)
/// that is preserved and merged onto whatever is restored.
pub fn use_list_view_state(
    ...
    // Re-apply the route scope when it changes. On a same-variant, param-only nav
    // (PodcastDetail A → B) Dioxus reuses this component instance, so the one-shot
    // seed above leaves `filter`'s scope pinned to the first route. Sync the scope
    // ids — preserving the user's filters/search — but only when they actually
    // differ, so the write-back effect below converges instead of looping.
