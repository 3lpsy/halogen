//! Thin typed `use_context` accessors — the single way pages/components reach
//! shared context state. Each is a one-liner over `use_context::<T>()`; they live
//! together here rather than in a file apiece. Hooks with real logic (memo
//! slices, derived state) stay in their own modules (`use_episodes`,
//! `use_is_admin`, …).
...
use halogen_ui_svc_player::{NowPlaying, PlayContext, PlayerController, PlayingIdentity};
use halogen_ui_svc_store::StoreHandle;
use halogen_ui_toast::{ToastHandle, ToastQueue};

// Re-exported so components raising toasts can pick a level/timeout without a
// direct `halogen-ui-toast` dep (e.g. the episode-row swipe's brief info toast).
pub use halogen_ui_toast::ToastLevel;

/// Read the app-wide client config signal. The mutate-and-save *action* lives in
/// [`config_actions::persist_config`](halogen_ui_config::config_actions::persist_config), not here.
...
/// live in [`halogen_ui_accounts::account_actions`].
...
/// The current player state signal (provided by `PlayerProvider`). Lives outside
/// `EpisodeState` so player ticks don't re-render the rest of the app.
///
/// Reading this subscribes to `position_secs` too, so it re-runs ~4×/sec while
/// playing — fine for the player UI (it shows the position), but the episode rows
/// must NOT use it (one row × 30 × 4/sec recomputes). Rows use the gated identity
/// projection below instead.
pub fn use_now_playing() -> Signal<Option<NowPlaying>> {
    use_context::<Signal<Option<NowPlaying>>>()
}

/// The play-context signal (provided by `PlayerProvider`): the playlist the user
/// pressed play from (`PlayContext(None)` = queue semantics). Written by the
/// controller's `*_in` play entries; read by the "up next" surfaces (the
/// full-screen player preview, the list next-up marker, the transport buttons)
/// so they follow the playlist being listened through.
pub fn use_play_context() -> Signal<PlayContext> {
    use_context::<Signal<PlayContext>>()
}

/// The position-free `now_playing` projection (provided by `PlayerProvider`): a
/// `PartialEq`-gated [`Memo`] of `(episode_id, state)`. Episode rows read THIS to
/// derive current/playing/preparing without re-running on every ~4×/sec position
/// tick — the projection is unchanged by a tick, so its memo doesn't invalidate
...
/// The shared connectivity + sync-activity signal (provided by
/// `ConnectionStateProvider`). A separate signal from `EpisodeState`, so WS latency pongs
/// re-render only the navbar status dot. Prefer the `PartialEq`-gated memo slices
/// ([`use_sync_status`](crate::hooks::use_sync_status) /
/// [`use_is_offline`](crate::hooks::use_is_offline) /
/// [`use_connection_health`](crate::hooks::use_connection_health)) for narrow reads;
/// this is the whole-signal escape hatch.
pub fn use_connection() -> Signal<ConnectionState> {
    use_context::<Signal<ConnectionState>>()
}

/// The shared podcast-pool signal (provided by `PodcastStateProvider`), i.e. the
/// cached podcasts + per-podcast auto-playlist config. A separate signal from
/// `EpisodeState`, so a podcast cache write or auto-playlist edit re-renders only podcast
/// consumers (the podcasts list/detail, the row podcast-title lookup), not every
/// `EpisodeState` reader.
