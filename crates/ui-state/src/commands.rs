//! Worker-command façade — the single chokepoint for write intent.
//!
//! halogen has no Tauri `invoke` layer (unlike liftfg); instead, writes are
//! [`Command`]s sent to the background sync worker, which applies them
//! optimistically and durably (outbox). Reads come from `EpisodeState` via `hooks::*`.
//! Components call `commands::mark_played(&dispatch, id, true)` and never construct
//! a `Command` inline.
//!
//! Each function is a typed wrapper that builds a [`Command`] and sends it to the
//! worker coroutine.
...
use halogen_ui_commands::{Command, RedactedToken};
...
/// Reorder an episode within a playlist, given a target slot expressed in the
/// *displayed* list's coordinates. Custom order is stored ascending, so when the
/// list is shown descending (`reversed`, i.e. Custom + down arrow) a visual index
/// `v` maps to stored position `full_len - 1 - v`; ascending is identity. `full_len`
/// is the full (un-windowed) displayed row count. Without this translation a
/// descending drag/move lands at the mirrored-wrong position (see `handle_move`).
