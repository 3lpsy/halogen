//! Native (desktop/mobile) audio backend: an `<audio>` element inside the app's
//! webview, driven over the `document::eval` channel.
//!
//! The app IS a webview on every native target (wry: WebKitGTK on Linux,
//! WKWebView on iOS/macOS, the Android System WebView), so the OS media stack
//! the web build already targets is right there — including hardware decode and
//! pitch-preserving `playbackRate`, which a Rust-side decoder (rodio et al.)
//! would not give us. This backend mirrors `web.rs` semantics through one
//! long-lived eval channel:
//!
//! - **Rust → JS**: control messages (`load`/`play`/`pause`/`seek`/`rate`/
//!   `stop`/`shutdown`) via [`Eval::send`], applied by a JS loop that owns a
//!   single `Audio` element.
//! - **JS → Rust**: playback events (`time`/`duration`/`buffering`/`paused`/
//!   `playing`/`ended`/`error`) via `dioxus.send`, pumped by a spawned task
//!   into a queue that [`PlayerBackend::poll_events`] drains — same cadence
//!   contract as the web backend (a 250ms JS interval reports the clock, which
//!   is also what flips the controller `Loading → Playing`).
//!
//! Sources ([`MediaSource`]):
//! - `Local` — an absolute file path from the native media store; mapped to
//!   `{local media base}/halogen-local-audio/{file}` and served (with Range
//!   support) by the `WebviewMediaBridge` loopback media server in `ui-state`
//!   (plain http — the only transport webview media stacks can stream from).
//! - `Remote` — already an absolute loopback proxy URL
//!   (`{local media base}/halogen-media/...`, built by
//!   `ui-appstate::media_url` on native), served by the same bridge with the
//!   bearer token attached. Passed through untouched.
//!
//! No `prime()` implementation: wry disables the autoplay gesture policy on
//! every platform (`autoplay: true` webview attribute), so a programmatic
//! `play()` outside a user gesture is allowed — the web-only silent-WAV
//! priming dance isn't needed here.
...
/// The JS side: one `Audio` element + event listeners + the command loop. Runs
/// as an `AsyncFunction(dioxus)` so top-level `await dioxus.recv()` works. A
/// global teardown hook lets the *next* boot (provider remount on account
/// switch) silence a stale instance whose Rust side vanished without a
/// `shutdown` (the pending `recv()` promise would otherwise never settle).
const BOOT_JS: &str = r#"
...
/// The webview URL for a device-copy file path:
/// `{local media base}/halogen-local-audio/{file}` — absolute loopback http
/// served by the `WebviewMediaBridge` media server from the media store's
/// audio dir. Loopback http (not a custom-scheme asset URL) because webview
/// media stacks can only stream `<audio>` over http(s) — see
/// `ui-appstate::media_url::local_media_base`. `None` when the path has no
/// usable file name or the bridge hasn't bound (surfaces as a backend error,
/// not a silent hang).
fn local_audio_url(path: &str) -> Option<String> {
    let name = Path::new(path).file_name()?.to_str()?;
    let base = halogen_ui_appstate::media_url::local_media_base()?;
...
mod tests {
    ...
    fn local_audio_url_maps_file_name_under_local_base() {
        ...
        halogen_ui_appstate::media_url::set_local_media_base(
