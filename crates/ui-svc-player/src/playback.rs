//! The play-routing state machine for the
//! [`PlayerController`](super::PlayerController) — the "decide how to start
//! playback" cluster.
//!
//! The local-first source routing — device copy vs. server stream vs.
//! (re-)download — lives here in one place, while `controller.rs` keeps the
//! backend wiring, the `now_playing` reflection, and the `tick` poll loop. These
//! are `impl PlayerController` methods in a sibling file: they lean
//! on the controller's private wiring (the backend, the media store, the sync
//! handle, generation guard, and the `load_and_play`/`set_error` helpers), so a
//! sibling `impl` block keeps the move mechanical (the helpers it leans on are
//! marked `pub(super)` on the controller).

use dioxus::prelude::*;

use super::{MediaSource, NowPlaying, PlaybackState, PlayerController};
use halogen_ui_appstate::ClientDownloadState;
use halogen_ui_commands::Command;
use halogen_ui_config::PlaybackPreference;
use halogen_ui_logging::{info, warn};

impl PlayerController {
    /// Enter `Preparing` (spinner) while a device download runs; the
    /// `PlayerProvider` watcher plays the episode once its client state flips
    /// to `Downloaded`. Stops any current audio so its events don't bleed in.
    ///
    /// Reached synchronously from a user gesture (a Play / Download & Play
    /// tap), so this is where the backend gets primed: the actual `play()`
    /// fires when the download lands — seconds later, with no user activation
    /// left — and browser autoplay policy would reject it on a fresh page (the
    /// "freshly downloaded episode won't play until something else played"
    /// bug). See `PlayerBackend::prime`.
    ...
    /// React to `episode_id`'s device-download state landing while we're
    /// `Preparing` it (the `PlayerProvider` watches `client_downloads` and calls
    /// this): play it once the bytes are `Downloaded`, or surface an error if the
    /// download `Failed`. No-op unless we're currently preparing exactly this
    /// episode — so every write to `now_playing` stays inside the controller.
    ...
    /// Play an episode from the best available source, LOCAL-FIRST: the device
    /// copy via the media store; what happens when the bytes are unexpectedly
    /// missing (state said `Downloaded` but the store came back empty —
    /// eviction/corruption) depends on the playback preference: streaming
    /// modes fall back to the server copy, `DownloadOnly` re-downloads
    /// (spinner) and never streams.
    ///
    /// Source resolution is async (media store read), so this spawns; a
    /// generation guard drops stale resolutions when the user has already
    /// started a different load.
                ...
                match media.audio_url(episode_id).await {
                    Ok(Some(local)) => {
                        if this.load_generation.get() != generation {
                            ...
                            halogen_ui_svc_media::revoke_object_url(&local.url);
                            return;
                        }
    ...
    /// [`request_play`](Self::request_play) from a playlist list: sets the play
    /// context (the continuation playlist for auto-advance / "up next" /
    /// transport next-prev) before routing. `None` resets to queue semantics.
    /// The context-less methods below preserve the current context — they're
    /// also the continuation paths (auto-advance, transport next/prev, the
    /// stream fallback, the download-then-play watcher handoff).
    ...
    /// User intent to play, routed by the playback preference (the app is
    /// local-first; the default never streams):
    /// - `DownloadOnly`: play the device copy; otherwise download first
    ///   (spinner via `Preparing`, auto-plays when the bytes land).
    /// - `StreamFirstAndDownload`: stream now (unless a device copy already
    ///   exists — then play it) AND ensure a device download in the background.
    /// - `StreamFallback`: device copy if present, else stream. No downloads.
    /// - `StreamOnly`: always stream. No downloads.
    ///
    /// Preserves the current play context (this is also the auto-advance
    /// continuation path) — user-entry points with a list context call
    /// [`request_play_in`](Self::request_play_in) instead.
    pub fn request_play(&self, episode_id: i32) {
        // Persist the OUTGOING episode's position before we switch sources — a direct
        // play→play switch (tapping a new episode mid-playback) otherwise loses up to
        // the last cursor-debounce window (~10s). Gate on Playing/Paused so the
        // auto-advance path is excluded: there the just-ended track was already marked
        // played (cursor reset to 0) and latched to `Ended`, so re-saving its near-end
        // position would undo that.
        let outgoing = self
            .now_playing
            .peek()
            .as_ref()
    ...
    /// Explicit "Download & Play": force the fetch-to-device-then-play pipeline
    /// regardless of the playback preference (the detail page's action for an
    /// episode held neither on the server nor this device). Mirrors the
    /// `DownloadOnly` branch of [`request_play`](Self::request_play) — kick a
    /// device download (the worker chains server→device with progress) and enter
    /// `Preparing`; the `handle_preparing_download_state` watcher flips to play when the
    /// bytes land. A device copy plays immediately; a download already in flight
