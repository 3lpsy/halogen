//! Read a bounded tail of Config.log_file, never the whole file. With no file configured, return the in-memory ring of
//! the current process's formatted log lines.
...
/// GET /admin/server-logs — tail of the server logs. **Admin only.** File-backed when `log_file` is configured
/// (`path: Some`, full process history); otherwise served from the in-memory ring (`path: None`, current
/// process only). A configured-but-unreadable file (e.g. nothing written yet) yields empty lines rather than an
/// error.
#[axum::debug_handler]
pub async fn get(
    _admin: AdminUser,
