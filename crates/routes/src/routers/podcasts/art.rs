...
/// Serve full-resolution podcast art from the server cache, fetching origin art on first use. Accept media cookies or
/// API bearer tokens; absent artwork returns 204 for a quiet UI placeholder.
pub async fn art(
...
/// GET /podcasts/{id}/art/small Like [`art`] but serves a downscaled (~256px) thumbnail variant, generated and
/// cached on first request. Used by the podcast list — call sites that render at ~70px and must not pull the
