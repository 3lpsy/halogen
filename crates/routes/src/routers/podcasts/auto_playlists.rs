//! `GET` / `PUT /podcasts/{id}/auto-playlists` — read and replace the set of playlists a podcast auto-adds new
//! episodes to. `PUT` is a full set-replace (idempotent), so one endpoint covers both first-time configuration
//! and later edits — the form never needs a create vs update distinction. The RSS poller
//! (`halogen_rss::manager`) reads this set and appends each newly-ingested episode to every linked playlist.
