...
impl ApiError {
    /// Whether this failure means the request never reached a server — radio off,
    /// VPN/tunnel down, host unreachable, DNS gone — as opposed to something the
    /// server actually said.
    ///
    /// This is the one place that decides what "offline" means at the API layer.
    /// It matters because the two cases need opposite handling: unreachable means
    /// the cache is authoritative and the app's offline surface owns the message,
    /// while a real response means the cache being stale is NOT the explanation
    /// and the user needs to see what came back. Callers that re-derive this by
    /// matching `Transport` themselves drift — `ui-toast::classify` (toasts) and
    /// `ui-episode-list::paged` (list state) both route through here.
