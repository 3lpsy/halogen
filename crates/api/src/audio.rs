...
/// A raw authed media response for the native webview media proxy
/// ([`ApiClient::fetch_media_raw`]): the status plus the few headers the
/// webview needs, body fully buffered. Non-2xx statuses are RETURNED, not
/// errors, so the proxy forwards them verbatim (e.g. a 404 for missing artwork
/// drives the UI placeholder exactly like on web).
