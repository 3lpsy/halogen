//! Single source of truth for the web **IndexedDB** names: database-name prefixes,
//! per-database object-store names, and a couple of well-known record keys.
//!
//! These are referenced both by the producers — the metadata store
//! (`halogen-ui-svc-store`), the per-user config store (`halogen-ui-config`), and
//! the account registry (`halogen-ui-accounts`) — and by the `/cache-control`
//! failsafe (`halogen-ui-cache-purge`), which must name the same databases/stores
//! to purge them *without* depending on the producer crates. Keeping the names
//! here is what lets the failsafe and the stores agree.
//!
//! Per-user databases are suffixed with the namespace segment (`"u{id}"` /
//! `"anon"`, from [`crate::namespace`]); the account registry is device-global.
...
pub const CONTENT_STORES: [&str; 4] = [
    STORE_PODCASTS,
    STORE_EPISODES,
    STORE_PLAYLISTS,
    STORE_PLAYBACKS,
...
// ── Device media store database (per-user): `halogen.media.{segment}` ──────────
//
// Device-downloaded audio bytes. Per-user like the metadata/config stores, so a
// remote and an embedded account (whose per-server episode ids collide) never
// serve each other's audio. The failsafe names these to purge every segment.
