//! Native per-app storage roots (this module is compiled only for non-wasm
//! targets; web persists to browser storage — see `store_keys` /
//! `halogen-ui-idb`).
//!
//! The one authoritative answer to WHERE the app keeps its files, shared by
//! every native store: the SQLite metadata cache, the audio byte cache, the
//! config/accounts JSON, and the device log. Resolution order:
//!
//! 1. `HALOGEN_DATA_DIR` / `HALOGEN_CONFIG_DIR` env overrides (absolute paths
//!    only) — tests, portable installs, unusual hosts.
//! 2. `directories::ProjectDirs` ("org", "fgsec",
//!    "halogen"). Linux honors `XDG_DATA_HOME`/`XDG_CONFIG_HOME`, so a flatpak
//!    (which remaps those into `~/.var/app/<id>/`) needs nothing special; on
//!    macOS this is `Library/Application Support` inside `$HOME`.
//! 3. Fallbacks that never panic and never land in the CWD (which is `/` or
//!    read-only under flatpak): `$HOME/.halogen/{data,config}`, else a
//!    `halogen` dir under the OS temp dir (logged loudly — data won't survive
//!    a reboot, but the app still runs).
//!
//! Memoized per process (`OnceLock`): the roots can't change mid-run. Tests
//! rely on nextest's process-per-test model when they override the env vars.
...
/// Root for the embedded (in-process) server's data — its SQLite DB, media
/// downloads, overrides file, and secrets all live under this one directory
/// (`halogen-embedded-server` owns the layout). A single distinct name under
/// [`data_root`] so it can never collide with the UI's own claims (`u*`/`e*`/
/// `anon` namespaces, `audio/`, `webview/`, `device.log`) — and, because
/// `data_root == config_root` on macOS, also not with `accounts.json`.
