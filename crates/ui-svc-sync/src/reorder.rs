...
/// Sort `ids` in place by the chosen `field` + `direction`, resolving each id's
/// keys via `keys`. Absent ids and absent values (nulls) sort last regardless of
/// direction; ties break by id, so the order is stable and reproducible.
///
/// Mirrors the server's smart reorder (`playlist_reorder::handle_reorder`). The one
/// deliberate difference is [`PlaylistReorderField::Added`]: the server keys it off
/// the playlist pivot's `created_at` (not held client-side), which this approximates
/// with the episode's own `created_at` — a later sync reconciles any difference.
