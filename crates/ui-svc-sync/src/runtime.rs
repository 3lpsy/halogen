//! The background-runtime seam: the channel-based boundary between the sync worker
//! and the Dioxus main thread.
//!
//! The worker no longer holds Dioxus `Signal`s or a `ToastHandle`; instead it emits
//! [`WorkerEvent`]s through an `UnboundedSender`, and the provider drains them on the
//! main thread to write the signals + toast queue. This decouples the worker from the
//! renderer so Stage B can move it onto a Web Worker (`postMessage`) / native thread.
//!
//! Stage B serializes [`WorkerEvent`] (and the 8 state types it carries) with serde
//! so the events can cross the Web Worker `postMessage` boundary. The wire protocol
//! between the main thread and the worker is [`ToWorker`] / [`FromWorker`], exchanged
//! as `serde_json` strings.
...
use halogen_ui_toast::{ToastDecision, ToastLevel};

/// One published state mirror (or toast) emitted by the worker for the main thread
/// to apply. The worker sends these instead of writing Dioxus signals directly.
///
/// Serialized with serde so it can cross the Web Worker `postMessage` boundary as a
/// [`FromWorker::Event`] (the 8 state types it carries derive serde too).
...
/// Main-thread → worker messages exchanged over `postMessage` as `serde_json` strings.
///
/// The handshake: the main thread creates the Worker and immediately posts
/// [`ToWorker::Init`] carrying the active-user namespace `segment` (the worker has its
/// own globals and can't read the main thread's `namespace::segment()`). It then
/// queues [`ToWorker::Command`]s until the worker replies [`FromWorker::Ready`], and
/// flushes the queue.
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
pub enum ToWorker {
    /// Open the worker's own stores under this user namespace segment and start the
    /// sync loop. Always the first message.
    ///
    /// `enabled`/`level` seed the worker's device-log capture from the main thread's
    /// current setting so worker logs are captured at the same threshold. (Live
    /// toggling after init is a documented follow-up — there's no live-update channel
    /// today; the worker reads these once here.)
    Init {
        segment: String,
        enabled: bool,
...
pub enum FromWorker {
    ...
    /// [`ingest`](halogen_ui_logging::ingest) into the shared ring + `halogen.logs`
    ...
    Log(halogen_ui_logging::LogLine),
}

/// A toast emitter mirroring [`halogen_ui_toast::ToastHandle`]'s surface, but routed
...
mod tests {
    ...
    use halogen_ui_commands::RedactedToken;
    ...
    fn to_worker_round_trips() {
        round_trip(&ToWorker::Init {
            segment: "u42".into(),
            enabled: true,
            level: halogen_ui_logging::Level::Debug,
        });
        round_trip(&ToWorker::Command(sample_command()));
    }
    ...
    fn from_worker_log_round_trips() {
        round_trip(&FromWorker::Log(halogen_ui_logging::LogLine {
            ts_ms: 1_700_000_000_123,
            level: halogen_ui_logging::Level::Warn,
            target: "halogen_ui_svc_sync::runtime".into(),
