//! Connectivity: the WebSocket driver lifecycle + connection/latency status.
//!
//! A focused `impl SyncService` block — everything here is about *reachability*
//! (is the server up, how laggy), distinct from the data-sync command handling in
//! `worker.rs`.

use std::rc::Rc;

use super::{Command, DEGRADED_RTT_MS, RTT_WINDOW, SyncService, spawn_task};
use halogen_ui_appstate::state::{ConnectionHealth, SyncStatus};
use halogen_ui_logging::debug;
use halogen_ui_svc_ws::{ConnectionEvent, EventSink};
use halogen_ui_toast::ToastDecision;

impl SyncService {
    /// Spawn the connectivity-WebSocket driver (tearing down any existing one
    /// first). No-op when manually offline or before auth — there's nothing to
    /// connect to. Re-invoked on every `SetAuth` so the driver always mints tickets
    /// with the current token. The driver reports back via the internal channel as
    /// `Ws*` commands.
    pub(super) fn start_ws(&mut self) {
        ...
        let (cancel, driver) = halogen_ui_svc_ws::spawn_handle(api, emit);
