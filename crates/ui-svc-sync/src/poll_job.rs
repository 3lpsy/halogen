//! Server poll-job orchestration: start a poll job, poll its status to completion,
//! refresh the list, and toast a summary.
//!
//! The pull-to-refresh gesture hook (`halogen_ui_state`'s `use_pull_to_refresh`)
//! spawns [`run_poll_job`] when an admin holds the pull past the countdown.
//! Keeping the orchestration here keeps that hook focused on the gesture/visual
//! phase and lets the orchestration be tested/reused independently.
...
use halogen_ui_toast::{ToastDecision, ToastPolicy, classify};
