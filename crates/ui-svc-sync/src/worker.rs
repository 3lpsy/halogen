//! The sync worker: `SyncService` (owns the canonical `EpisodeState`, the periodic
//! pull/drain loop + command handling) and its worker-internal helpers
//! (`sleep_secs`/`spawn_task`, `InFlight`, the tuning consts). The rest of the
//! inherent impl is split across the sibling modules (connectivity/data_ops/
//! network); fields are `pub(crate)` so those impls reach them.
...
use halogen_ui_toast::{ToastPolicy, classify};
...
/// Spawn a `!Send` background task (device-download fetches). The worker loop is
/// sequential — a multi-megabyte download awaited inline would block every
/// command — so byte fetches run as detached tasks that report back through the
/// worker's internal command channel.
///
/// Native hosting order matters: dioxus desktop/mobile poll the app on a plain
/// **multi-thread** tokio runtime with NO `LocalSet` (`dioxus-desktop`
/// `launch_virtual_dom`), so `tokio::task::spawn_local` would always fail in
/// the real app. The native worker runs inside a `use_coroutine`, so a Dioxus
/// runtime is ambient there — detached tasks go on the Dioxus scheduler
/// (`spawn_forever`: root scope, so they outlive any one component, matching
/// the wasm `spawn_local` semantics). The tokio `spawn_local` arm remains for
/// the renderless `#[tokio::test]` harnesses, which run inside a `LocalSet`
/// and have no Dioxus runtime.
///
/// Returns `false` when neither host is available — callers must then fail the
/// download honestly instead of leaving it `Downloading` forever.
...
/// Background sync worker that owns the canonical data.
///
/// Responsibilities:
/// - Handle UI commands: apply an **optimistic** local change *and* enqueue a
///   durable outbox op, so the UI reflects the action immediately and it
///   survives a restart / offline period.
/// - Pull loop: periodically resolve the default playlist (queue) — the offline
///   probe. NOTHING is bulk-pulled: episodes/podcasts/playlists page lazily and
///   hand pages back via `CacheEpisodes`/`CachePodcasts`; playback cursors ride
///   episode pages (`EpisodeInclude::Playback`); History pages `/playbacks` on
///   demand (`load_history`).
/// - Outbox drain: send pending ops to the server (via [`OutboxOp::apply`]),
///   ack on success.
///
/// The worker is the **only writer** of the shared `Signal<EpisodeState>`; it writes
/// a working copy and publishes it after each change so the UI re-renders.
pub struct SyncService {
    ...
    /// True while a `do_pull` is mid-flight (it awaits a network round trip, so the
    /// cooperative worker can interleave the next command). At startup `SetAuth`,
    /// `WsConnected`, and the queue page's `EnsureDefaultPlaylist` each want a pull;
    /// without this they overlapped into 3-4 redundant `playlists/default` fetches.
    /// The guard collapses the burst to one — later periodic / visibility-driven
    /// pulls run normally once it clears.
    pub(crate) pull_in_flight: bool,
    /// Epoch-ms of the last pull that actually reached the server (`do_pull`
    /// success / `Ok(None)`), or `None` if none has completed. `pull_in_flight`
    /// only collapses *concurrent* triggers; at a fast cold boot the SetAuth /
    /// WsConnected / EnsureDefaultPlaylist triggers fire *sequentially* (each after
    /// the prior's round trip), so the guard never catches them. The recency check
    /// in [`SyncService::coalesced_pull`] / `ensure_default_playlist` skips those
    /// startup pulls when a successful one landed < [`PULL_COALESCE_MS`] ago — the
    /// explicit RefreshNow, the periodic tick, reconnect, and subscription-driven
    /// pulls bypass it and always run.
    ...
    /// Podcast ids the user unsubscribed this session. A page revalidation fetched
    /// before the queued `Unsubscribe` reached the server is STALE and must not
    /// re-add them to the pool — `cache_podcasts` filters against this. The tombstone
    /// outlives the outbox op (closing the window where the op drains before the
    /// stale page's cache write lands); cleared on `Subscribe` and on reload.
    ...
    /// "Add to the front of the queue" preference (mirrors
    /// `ClientConfig.playback_prefs.add_to_queue_front`, set via
    /// `SetAddToQueueFront`). When `true`, adding an episode to the *queue* (the
    /// default playlist) inserts it at position 0 instead of appending. Only the
    /// queue is affected — adds to any other playlist always append.
    ...
    /// Cancel handle for the running connectivity-WebSocket driver. `Some` while a
    /// driver task is live; dropping/sending stops it (and closes its socket). Set
    /// by `start_ws` on `SetAuth`/reconnect, cleared by `stop_ws` on manual-offline
    /// / sign-out / wipe. Re-created on each `SetAuth` so it always uses a fresh
    /// token.
    ...
    /// Shared mirror of `manual_offline` for DETACHED tasks (device downloads,
    /// server-progress polls): they hold their own `ApiClient` and outlive any
    /// one command, so "Go Offline" flips this and the tasks observe it between
    /// polls/chunks and stop transferring — instead of continuing to move bytes
    /// after the user turned networking off.
    pub(crate) offline_flag: std::rc::Rc<std::cell::Cell<bool>>,
    /// Account-segment scope for cross-tab Web Locks (web only): on web, every
    /// tab (and the sync worker inside each) shares one origin's IndexedDB, so
    /// the outbox drain and device downloads take `halogen.{what}.{scope}`
    /// locks to keep two tabs from double-applying ops or interleaving chunk
    /// writes. Set via [`SyncService::set_lock_scope`] by the web runtimes;
    /// empty (harmless — locks still scope per origin+name) on native, where
    /// one process owns the stores.
...
impl SyncService {
    ...
    /// Mirror the working state into the shared signal so the UI re-renders.
    ///
    /// Player state (`now_playing`) lives in a **separate** signal owned by the
    /// player, not in `EpisodeState`. The worker only ever writes `EpisodeState`, never
    /// `now_playing`, so the player's ~4×/sec ticks don't notify `EpisodeState`
    /// subscribers (navbar, episode lists) and a publish never clobbers the
    /// player. The two are fully independent.
    ///
    /// Pushes only the domains touched since the last publish: each [`Tracked`]
    /// cell reports + clears its dirty bit (`take_dirty`), and the matching signal
    /// is `set` only when dirty. A download-only command (e.g. `DeviceFetchProgress`)
    /// therefore re-renders only download consumers, not every `EpisodeState` subscriber.
    ///
    /// `Signal::set` has NO value-equality gate, and the dirty bit flips on mutable
    /// *access* not value change — so an `EpisodeState` touch always publishes, even
    /// when value-equal. Do **not** "fix" that by gating on `PartialEq`:
    /// `EpisodeList`'s paged effect A deliberately uses the `EpisodeState` publish as a
    /// *store-changed* signal, and `cache_episodes` can write the store while
    /// leaving `EpisodeState` value-equal (a revalidated page whose ids are already
    /// indexed).
    ...
    pub async fn run(mut self, commands: futures::channel::mpsc::UnboundedReceiver<Command>) {
        // Internal command channel: spawned device-download tasks send their
        // progress/completion back through here so the worker stays the single
        // EpisodeState writer. Merged with the UI command stream.
        //
        // The UI stream carries an explicit end SENTINEL (`None`) because the
        // merged stream alone never ends while any internal sender lives — and
        // one always does: `self.internal_tx` is held by this very service. In
        // the dedicated worker / native process that didn't matter (the whole
        // context dies with the app), but the web in-process FALLBACK runs this
        // loop as a page-lifetime `spawn_local`: without the sentinel it
        // outlived its provider on account switch and kept pulling + draining
        // against the old account's store and auth every 60s — a zombie second
        // writer.
        ...
        // The periodic-pull timer is created ONCE and carried across iterations: when
        // a command wins the `select`, the un-fired timer is handed back and reused —
        // so command traffic does NOT reset the interval. Without this, the timer was
        // rebuilt every loop, and the steady ~15s WS latency pongs merged into this
        // stream (always < the 60s interval) reset it before it could fire, starving
        // the periodic do_pull entirely whenever the connectivity socket was up.
    ...
    async fn handle_command(&mut self, cmd: Command) {
        ...
        match cmd {
            ...
            Command::DeletePlaylist { playlist_id } => {
                ...
                // Playlist CRUD is ONLINE-ONLY (the create/edit forms disable
                // offline), so this is a DIRECT call, never an outbox op: offline /
                // signed-out just toasts, and a failed call surfaces through the
                // shared classify → toast path. Local state is only touched after
                // the server confirms, so a failure leaves the playlist intact.
            ...
            Command::ServerDownloadComplete {
                ...
                if let Some(ep) = episode {
                    ...
                    // The poll's final status refresh failed (a transient offline blip
                    // at the tail, after its retries). Don't strand the optimistic
                    // `Downloading` badge as a forever-spinner: drop it to a neutral
                    // `NotDownloaded` so the next list/pull revalidation reconciles the
                    // true status (rearming a poll here could busy-loop while offline).
            ...
            Command::DownloadOnServer { episode_ids } => {
                ...
                // Reconcile every requested episode's optimistic `Downloading` badge
                // with a ≈1 Hz progress poll (deduped per id). A batch (n>1)
                // previously started no poll at all, so those badges never got
                // reconciled and could strand; the per-id dedup guard caps it at one
                // poll per episode. (A multiselect batch still summary-toasts below.)
                for &episode_id in &episode_ids {
            ...
            Command::UpdatePlaylist { playlist_id, data } => {
                ...
                self.enqueue(OutboxOp::UpdatePlaylist { playlist_id, data })
                    .await;
            }
            ...
            Command::SetOffline(offline) => {
                ...
                if offline {
                    ...
                    // Rebuild the connectivity socket, then flush queued ops BEFORE
                    // pulling so optimistic queue membership reaches the server
                    // before `cache_playlists` reads the default playlist back (its
                    // `episode_ids` would otherwise overwrite a not-yet-shipped
                    // offline add).
                    self.start_ws();
                    self.drain_outbox().await;
                    self.do_pull().await;
            ...
            Command::WsConnected => {
                ...
                self.publish();
                // Resync now that we're back — this is what replaces waiting for the
                // next 60s tick. Drain BEFORE pulling so optimistic queue membership
                // reaches the server before `cache_playlists` reads the default
                // playlist back (its `episode_ids` would otherwise overwrite a
                // not-yet-shipped offline add). The loop drains again after this
                // handler — a no-op once this drain emptied the outbox.
                // `coalesced_pull`: at cold boot WsConnected lands right after the
                // SetAuth pull, so it collapses onto it; a genuine reconnect (last
                // pull long ago) falls through and pulls.
                self.drain_outbox().await;
            ...
            Command::WipeLocal => {
                ...
                let _ = halogen_ui_logging::drain_pending();
    ...
    async fn hydrate_from_store(&mut self) -> Result<()> {
        ...
        // Resolve the queue from the locally-cached default so queue-dependent UI
        // (the "Add to queue" action, /queue) works offline on a cold start. Only
        // the server can assert there's genuinely no queue, so don't mark Absent
        // here — a missing local default just stays Unknown until the online
        // `ensure_default_playlist` confirms it.
...
mod tests {
    ...
    /// `run` must END when the UI command channel closes, even though the
    /// service holds its own `internal_tx` into the merged stream. Pre-fix the
    /// merged stream just went pending and the 60s timer kept the loop alive
    /// forever — on the web fallback that was a zombie second writer surviving
    /// account switch.
    #[tokio::test]
