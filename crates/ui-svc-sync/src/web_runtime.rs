//! The web side of the [`BackgroundRuntime`](crate::BackgroundRuntime) seam: a real
//! `web_sys::Worker` running the sync graph off the main thread, **supervised** so a
//! worker crash self-heals instead of freezing the app.
//!
//! dx (Dioxus 0.7) has no web-worker target (DioxusLabs/dioxus #3275), so the worker
//! is our own wasm-bindgen build (`halogen-ui-worker`) wired into the justfile and
//! bootstrapped by `/worker.js`. This type lives on the **main thread**: it creates
//! the Worker, performs the [`ToWorker::Init`] handshake (passing the active-user
//! namespace `segment`, since the worker has its own globals), queues commands until
//! the worker replies [`FromWorker::Ready`], then flushes; incoming
//! [`FromWorker::Event`]s are forwarded into the applier's [`WorkerEvent`] channel and
//! [`FromWorker::Log`]s into the shared device log.
//!
//! ## Resilience (a Worker CAN die — wasm trap, OOM, browser termination)
//! A wasm trap poisons the worker instance; left unhandled the UI silently freezes
//! (commands go to a dead worker, no events come back) until a page reload. So this
//! supervises the worker:
//! - **Detect** faults via `onerror`, `onmessageerror`, and a **ready-timeout** (no
//!   `Ready` within [`READY_TIMEOUT_MS`] of `Init`).
//! - **Respawn** a fresh Worker with exponential backoff, up to [`MAX_RESPAWNS`]; the
//!   new worker re-hydrates from IndexedDB and drains the durable outbox (we do NOT
//!   auto-replay arbitrary sent commands — non-idempotent ones like "add to playlist"
//!   would double-apply; re-hydrate-from-store is the safe path).
//! - **Fall back** to running [`SyncService`] IN-PROCESS (the [`BackgroundRuntime`]
//!   native path) after the respawn budget is spent, so the app degrades to
//!   main-thread sync instead of freezing.
//! - **Re-establish session state**: the config mirrors (`SetAuth`/`SetOffline`/prefs)
//!   only fire on mount/config-change, never on a respawn — so a fresh worker would
//!   come back with no auth (offline). We cache the latest *sticky* command of each
//!   kind and replay them to every fresh worker / the fallback service.
//! - **Notify** the user with a toast and log the fault to the console + device log.
//!
...
/// How long a worker must stay up AFTER `Ready` before the respawn budget
/// clears. `Ready` alone proves nothing — the worker posts it before hydrate /
/// the first pull, so a deterministic post-Ready trap used to clear the budget
/// every cycle and crash-loop forever (toast per ~1s) instead of ever reaching
...
fn post(worker: &Worker, msg: &ToWorker) {
    match serde_json::to_string(msg) {
        ...
        Err(e) => halogen_ui_logging::error!("worker: serialize ToWorker failed: {e}"),
    }
}
...
impl Supervisor {
    ...
    fn dispatch(&mut self, cmd: Command) {
        ...
        match &self.mode {
            Mode::Worker {
                worker,
                ready: true,
            } => post(worker, &ToWorker::Command(cmd)),
    ...
    fn on_ready(&mut self) {
        ...
        for cmd in &self.sticky {
            post(&worker, &ToWorker::Command(cmd.clone()));
        }
...
fn spawn_worker(sup: &Rc<RefCell<Supervisor>>) {
    let worker = match Worker::new("/worker.js") {
        Ok(w) => w,
        Err(e) => {
            halogen_ui_logging::error!("worker: failed to (re)create: {e:?}");
            start_fallback(sup);
            return;
        }
    };
    ...
    let onmessage = Closure::<dyn FnMut(MessageEvent)>::new(move |evt: MessageEvent| {
        ...
        match serde_json::from_str::<FromWorker>(&text) {
            ...
            Ok(FromWorker::Log(line)) => halogen_ui_logging::ingest(line),
            Err(e) => halogen_ui_logging::warn!("worker: malformed FromWorker dropped: {e}"),
        }
    });
    ...
    // thread's current device-log setting so worker capture matches).
    {
        ...
        post(
            ...
            &ToWorker::Init {
                ...
                enabled: halogen_ui_logging::enabled(),
                level: halogen_ui_logging::level(),
            },
        );
        s.mode = Mode::Worker {
            worker,
...
fn on_fault(sup: &Rc<RefCell<Supervisor>>, reason: String) {
    ...
    let next = {
        ...
        halogen_ui_logging::error!("sync web worker fault: {reason}");
...
fn start_fallback(sup: &Rc<RefCell<Supervisor>>) {
    ...
    let Some(store) = s.store.clone() else {
        halogen_ui_logging::error!("sync web worker: no local store — sync disabled");
        toast(
            &s.events_tx,
            ToastLevel::Error,
    ...
    halogen_ui_logging::warn!("sync running in-process (web worker fallback)");
...
/// Drives a supervised `web_sys::Worker` running the sync graph off the main thread,
/// implementing [`BackgroundRuntime`]. The UI dispatches [`Command`]s through
/// [`commands`](BackgroundRuntime::commands); a local pump routes them to whichever
/// backend is live (worker, or the in-process fallback), queued until `Ready`.
///
/// Keep this value alive for the worker's lifetime — dropping it drops the supervisor,
/// which terminates the current worker.
