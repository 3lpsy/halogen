//! Shared per-episode live row-state derivation.
//!
//! Both the list item ([`EpisodeListItem`](super::EpisodeListItem)) and the
//! episode detail page derive the SAME live state for one episode from
//! `EpisodeState` + the player's `now_playing`: device/server download status and
//! progress, queue membership, the current/playing/preparing flags, and the
//! local-first play-gating rule. The list item is the source of truth (its
//! comments note the detail page must match); this module lifts those formulas
//! into one place so the two can't drift.
//!
//! The list item additionally reads pieces that are intrinsically per-row and
//! not shared (the swipe `progress` prop fallback, the queue "up next" marker)
//! — those stay at the call site. What lives here is exactly the state that both
//! call sites compute identically and that feeds [`EpisodeMenuArgs`].
...
use halogen_ui_appstate::{ConnectionState, DownloadState, PlaylistState};
use halogen_ui_config::PlaybackPreference;
use halogen_ui_svc_player::{PlaybackState, PlayingIdentity};
...
impl EpisodeRowState {
    /// Resolve every shared field for `episode_id` from the live pool + player
    /// state. `is_current`/`is_playing`/`is_preparing` come from the player's
    /// `playing` identity (the position-free projection of `now_playing` — so a
    /// caller can subscribe to this without re-running on every position tick);
    /// the rest from `app_state`. This is the single copy of the formulas the
    /// list item and the detail page used to maintain by hand.
    pub fn resolve(
