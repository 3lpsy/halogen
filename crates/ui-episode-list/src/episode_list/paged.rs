//! Cache-aware, offline-first, server-paged loading for the episode list.
//!
//! The cache is a growing **pool** of episodes keyed by id. Rendering = a query
//! over the pool (`read_cache`: filter + sort + take `visible`). The server is a
//! paginated *filtered* fetch (`fetch_server`) that only tops the pool up — it
//! never owns what's shown. Toggling a filter never clears the pool: the view
//! re-queries it (instant) and the server cursor resets. Scrolling grows the
//! render window *and* advances the server cursor.
//!
//! Used by `/latest`. The played/download chips filter on both sides
//! (`playback_status`/`download_status`).
...
use halogen_ui_appstate::{PlaybackState, PodcastState};
use halogen_ui_config::api_client_from;
use halogen_ui_listview::{EpisodeFilter, FilterSpec, OrderDirection, SortField, SortSpec};
use halogen_ui_logging::{debug, warn};
use halogen_ui_svc_store::{EpisodeOrder, EpisodeQuery, EpisodeQueryFilter, LocalStore};
...
/// What [`fetch_server`] needs: where to send the fetched page (the worker) and
/// the session to talk to. The session (`server_url` + `token`) comes from
/// `ClientConfig`, **not** `EpisodeState` — the effect must not subscribe to
/// `EpisodeState`, or the `CacheEpisodes` it dispatches (which the worker publishes
/// back into `EpisodeState`) would retrigger it in a loop.
#[derive(Clone)]
pub(super) struct PagedDeps {
    pub dispatch: Coroutine<halogen_ui_commands::Command>,
...
/// Apply the optimistic-overlay rule to one row's cursor: a locally-written
/// playback (`app_state.playbacks`, e.g. the user just seeked) wins; otherwise
/// keep the cursor the server embedded on the body (via `EpisodeInclude::Playback`,
/// cached with it). Never blanks an embedded cursor on an overlay miss — that was
/// the bug when the overlay held the whole set; now it's session-local only.
...
/// Classify a failed page fetch.
///
/// An unreachable server ([`ApiError::is_offline`] — radio off, VPN/tunnel down,
/// host gone) is the `Offline` condition, NOT an error: the cached pool is all we
/// have, exactly as with the manual "Go Offline" toggle, and it's the same state
/// the navbar indicator already reports. Reporting it as `Error` instead put the
/// raw `reqwest` wording ("transport error: error sending request") on the page
/// as a red banner while the app's own offline surface sat unused.
///
/// Only a response the server actually produced (4xx/5xx, an undecodable body) is
/// a real error: the server IS reachable, so a stale cache isn't the explanation
/// and the user needs to see what it said.
...
pub(super) async fn fetch_server(
    ...
    match api.list_episodes(to_list_params(sort, filter, page)).await {
        Ok(result) => {
            ...
            halogen_ui_state::commands::cache_episodes(&deps.dispatch, fresh);
...
pub(super) async fn fetch_by_ids(deps: &PagedDeps, ids: Vec<i32>) -> ServerPage {
    ...
    match api.list_episodes(params).await {
        Ok(result) => {
            ...
            halogen_ui_state::commands::cache_episodes(&deps.dispatch, result.data);
