...
use halogen_ui_icons::{ChevronRight, CloudArrowDown, GripVertical};
use halogen_ui_listview::{EpisodeData, SwipeAction, SwipeConfig};
...
use halogen_ui_widgets::{Artwork, start_drag_reorder, use_confirm, use_quick_menu};
...
/// A rich episode card. Left: optional reorder grip (Custom order only) + art.
/// Right: title (2-line clamp), description (1-line clamp), then a controls row
/// (play/pause + length, device-download toggle, release date, kebab menu) and a
/// far-right chevron. Navigation to the episode page is via the title or the
/// chevron only — tapping the card body does NOT navigate or play. Swipe gestures
/// remain; when `reorder_enabled` the left grip drags to reorder.
#[component]
pub fn EpisodeListItem(
    ...
    // Per-row state is read through PartialEq-gated `use_memo`s (the `row`,
    // `is_next_up`, `playback_proj`, and `podcast_name` memos below), NOT directly
    // in the render body. A worker publish (bursty around sync) re-runs those memos
    // for every visible row — cheap pool lookups — but only re-renders a row whose
    // own projected slice actually changed, instead of re-rendering all rendered
    // rows on every publish. Keep any new worker-signal read inside a memo with a
    // PartialEq output; a raw `signal.read()` in the body re-subscribes the whole
    // row to that signal and brings the re-render storm back. (`config` is read
    // directly below — it publishes only on settings changes, not in sync bursts,
    // so it isn't a storm source.)
    ...
    // Parent podcast name, shown under the title. Read from the local pool
    // (episode lists no longer carry a nested podcast join); a stale embedded
    // copy is a last resort. When absent we lazily fetch it (below) and show a
    // '…' placeholder — podcasts are a small set that's almost always already
    // loaded after first use.
    // Memoized so a podcast-pool publish re-renders the row only when THIS
    // podcast's name changes, not on every pool update. `Option<String>` is
    // PartialEq, gating the row.
    ...
    let description = episode
        .description
        .as_deref()
        .map(halogen_ui_widgets::html::to_plain)
        .filter(|s| !s.is_empty());
    ...
    // Live per-episode state (download/queue/playback/play-gating), resolved once
    // by the SHARED builder so this row and the episode detail page can't drift —
    // the formulas (and the comments that flag the hand-synced invariants)
    // live in `row_state`. Reads the gated `playing` identity (NOT raw `now_playing`):
    // the player writes `now_playing` ~4×/sec for position ticks, but `playing` is a
    // `PartialEq` projection to `(episode_id, state)` that a tick leaves unchanged, so
    // this memo doesn't even recompute on a tick — it re-runs only on a real change
    // (episode/play-pause/prepare/download/queue). (`EpisodeRowState` is also
    // `PartialEq`, gating the downstream re-render for the other inputs.)
    ...
    // Whether this row is the "up next" episode auto-advance (and the player's
    // Up-next / Next button) will play next ([`PlaylistState::next_up_in`]). Only
    // meaningful inside the continuation playlist's own list — the play-context
    // playlist when one is active (with a known membership), else the queue — so
    // the arrow follows the playlist being listened through. A memo so the
    // player's ~4×/sec position writes don't re-render the row — it flips only
    // when the next-up episode actually changes.
    ...
    // Playback state — read LIVE from the overlay-wins pool (`playback_for`), NOT the
    // captured `episode` prop. The prop is built by the list query and goes stale, so
    // reading it would only update the played/finished marker after a re-fetch (i.e.
    // navigating away and back). The row already subscribes to EpisodeState, and the
    // worker republishes after MarkPlayed/SetCursor, so reading the pool here makes
    // the marker live (same reason the shared server status reads the pool). Falls
    // back to the prop's server-computed status when there's no local overlay.
    // Playback projection (finished / in-progress / progress %), memoized so the
    // bursty worker republishes — and the player's ~4×/sec position writes that
    // land in `playbacks` — re-render the row only when one of THESE derived values
    // changes, not on every publish. Reads LIVE playback from the overlay-wins pool
    // (`playback_for`); the `episode` prop's status/progress go stale until a
    // re-fetch, so they're only the fallback when there's no local overlay. The bar
    // and the finished marker both derive from the same `live_pb`, so they can't
    // drift (B2). `PlaybackProj` is PartialEq, gating the row's re-render. Mirrors
    // `compute_progress`: a bar only for real, mid-episode progress.
    ...
    // The quick-context-menu sections are built AT OPEN TIME inside the kebab's
    // onclick (below), not here in the render body: the builder allocates ~7
    // sections of `Rc` closures that are only consumed if the kebab actually
    // opens, and this row re-renders on every relevant publish (~1 Hz during a
    // download) — building them per render was pure waste on the hottest path.
    ...
    // Resolve a configured swipe action against this row's live state and perform
    // it through the SHARED dispatcher — the row holds every piece of context the
    // actions need (the toggle-resolving flags + the list's playlist id), so the
    // gesture is a thin adapter onto the same `commands::*` primitives the kebab
    // menu fires, instead of a third copy of the action→command mapping.
    // Embedded-server mode reshapes every download surface on the row (badge,
    // kebab, swipe remap): the server IS this device, so only the server-download
    // concept exists.
    ...
    let onpointermove = move |e: PointerEvent| {
        ...
        // No button held during a tracked drag → the pointer was released
        // OUTSIDE this row (a mouse-up off the element never fires our
        // `pointerup`), so the gesture is over. Without this the drag state
        // latched: a later plain hover kept matching `drag_start` and dragged
        // the card around under an unpressed cursor. Reset and stop tracking.
    ...
    rsx!(
        div {
            ...
            // Browser-native list virtualization: off-screen rows skip render + layout
            // (a big Style&Layout/paint win — the `tool-lighthouse` harness measured
            // blocking time ~6× lower WITH this on). `contain-intrinsic-size` = the
            // MEASURED row height (178px = 8.9rem at the 125% default scale; in rem so it
            // tracks the user font-scale). NOTE: removing this does NOT change the Latest
            // cold-load CLS (verified — CLS stayed 0.744 with it gone), so the CLS is a
            // whole-list re-render/reflow on cold load, tracked/fixed elsewhere; this
            // stays purely for the paint win.
            ...
            div {
                ...
                div { class: "card-body p-3",
                    div { class: "flex items-stretch gap-3",
                        ...
                        // Reorder grip (Custom order only) — press-and-hold, then
                        // drag to move the row. `touch-action: pan-y` lets a quick
                        // flick on the grip scroll the list normally; the reorder
                        // only engages after a short hold (see `start_drag_reorder`).
                        // `stop_propagation` keeps the card's swipe from firing so
                        // only the grip starts a vertical drag.
                        if reorder_enabled {
                            div {
                        ...
                        div { class: "flex-1 min-w-0",
                            ...
                            div { class: "flex items-start gap-3",
                                ...
                                div { class: "flex-1 min-w-0",
                                    // Title links to the episode — inert in multiselect (a
                                    // tap shouldn't leave the list mid-selection), so drop
                                    // the pointer cursor + navigation there.
                                    // Title as <h2> (was <h3>): a heading — good for screen
                                    // reader list nav, and an anchor the e2e suite selects via
                                    // `#episode-scroll h2`. Level 2, so it never skips: pages
                                    // wrapping the list in an <h1> (podcast/playlist detail)
                                    // read h1→h2, and heading-less list pages (latest/queue/…)
                                    // just start at h2 — both pass Lighthouse heading-order.
                                    // `min-h-[2.4rem]` reserves both clamped lines (2 ×
                                    // text-sm/leading-snug) so a 1-line title doesn't
                                    // render a shorter row than a 2-line one. With that,
                                    // every row is the same rem height — which is what
                                    // lets the `contain-intrinsic-size` estimate match
                                    // exactly and keeps the list's first-paint CLS at 0.
                                    h2 {
                            ...
                            // Description (left) + the Finished marker (right). Split
                            // into two columns so the check lands in the right-edge
                            // column between the chevron (above) and the kebab (below) —
                            // matching their `px-2` so the three stack vertically.
                            // ALWAYS rendered (reserving one text line via `min-h`) so a
                            // playback marker resolving AFTER first paint — the worker
                            // publishes `finished`/`in_progress`/`is_next_up` async — fills
                            // reserved space instead of growing the row (the big
                            // episode-row CLS). Description-less rows keep the same height.
                            div { class: "flex items-center gap-2 mt-1",
                                ...
                                // Playback marker. In the queue, the "up next" arrow
                                // takes priority — it's what plays when the current
                                // episode ends — over the finished check / in-progress
                                // half-circle. The thin progress bar below still shows
                                // position when in progress.
                                PlaybackMarker {
                            ...
                            div { class: "flex items-stretch gap-2 mt-1.5",
                              div { class: "flex-1 min-w-0",
                                ...
                                // Thin progress line — in-progress episodes only
                                // (played but not finished). Indicator only; no seek.
                                // The track height (`h-1 mt-1.5`) is ALWAYS reserved so an
                                // episode resolving its playback position AFTER first paint
                                // (the worker publishes `progress_pct` async) fills the
                                // reserved space instead of growing the row — the track is
                                // invisible (no background) until there's a percentage.
                                div {
...
/// Pick the badge glyph from the episode's device + server download state.
///
/// The key case is a *to-device* download: `device_downloading` is set the instant
/// you tap, so it would otherwise mask the server-fetch phase. Here, while the
/// device byte-pull hasn't started (`download_pct` is `None`) and the server
/// is still fetching its copy, we surface the **cloud** phase; once device bytes
/// land — or the server already had the file, so there's no cloud phase — we show
/// the plain device ring. The standalone `server_downloading` arm covers a
/// server-only download (no device pull queued).
