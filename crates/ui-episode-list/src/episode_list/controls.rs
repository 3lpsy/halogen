...
use halogen_ui_icons::{Funnel, Pencil, ViewfinderCircle, XMark};
use halogen_ui_listview::{
    EpisodeFilter, FilterSpec, ListSource, MultiSelectState, SortField, SortSpec,
};
use halogen_ui_widgets::{SearchInput, SearchToggle, SortDropdown, dropdown_keydown};
...
pub fn ListControls(
    ...
    // OnDevice (device-download set) is a client-only facet — offered on the
    // other id-list / cache-query sources, where it actually switches the list to
    // the device set. It's NOT offered on the Downloads page: those rows already
    // ARE the device set, so the chip would be a no-op the user could toggle to no
    // visible effect. The server-side Downloaded/Downloading chips are likewise
    // dropped there (server status is noise for device-defined rows), leaving only
    // the played-state facet.
    // Played-state facet, in enum order: never started → in progress → finished.
    // "In Progress" (not "Played") labels the middle state so the chip can't be
    // misread as "finished" — `PlaybackStatus::Played` means started, not complete.
    let filter_options: Vec<(EpisodeFilter, &'static str)> = match &source {
        ...
        // Every other source offers the full facet set. On the cache-query lists
        // (/latest, podcast detail) OnDevice can't be expressed server-side, so
        // picking it switches the list to the local device set (handled in
        // `EpisodeList`) rather than paging the server. Embedded mode drops the
        // OnDevice facet — there is no separate device set.
        _ => {
            let mut options = vec![
    ...
    rsx!(
        ...
        // Multiselect sub-bar — only while multiselect is active. Left: the
        // "Selected" review chip (restrict the list to the ticked set; selection
        // survives toggling it off) and "Select all" (ticks every loaded row).
        // Right: a pencil that opens the bulk-action menu (count badge), then an
        // exit (X) that leaves multiselect and clears.
        if ms_active {
            div { class: "flex items-center gap-2 p-2 bg-base-100 border-b border-base-200",
                button {
