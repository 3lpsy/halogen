//! Shared "episode actions" context-menu builder.
//!
//! The episode list-item kebab and the episode detail header kebab open the same
//! [`QuickMenu`](halogen_ui_widgets::QuickMenu) with the same actions — view
//! metadata, download / stream, view podcast/episode, reorder, add-to-playlist,
//! queue, and a local-data reset (the per-call-site `on_remove_local_data`
//! callback). This keeps the two call sites in lock-step (one place to
//! add/relabel an action).
//!
//! The detail page omits the "View episode" action (you're already on it) via
//! [`EpisodeMenuArgs::include_view_episode`], and passes no playlist context so
//! the reorder section is empty (and filtered out by the menu host).
...
/// Build the episode-actions menu sections. `player`/`dispatch`/`nav` are the
/// `Copy` handles the actions fire through — every action closure captures only
/// scope-independent handles, so the menu stays safe to use after the building
/// row unmounts (see `QuickAction`).
///
/// Section order (top→bottom; the bottom-anchored panel puts the last, most-used
/// section nearest the thumb): metadata, downloads, navigation, reorder,
/// add-to-playlist, queue, local-data recovery. Empty sections (no queue, no
/// reorder) are filtered out by the host.
