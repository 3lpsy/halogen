...
use halogen_ui_icons::User;
use halogen_ui_state::embedded_session::{sign_out_account_smart, switch_account_smart};
use halogen_ui_state::hooks::{use_accounts, use_dispatch, use_is_admin, use_toast};
            ...
            div { class: "space-y-4 p-3 bg-base-200 rounded-lg",
                    ...
                    button {
                        ...
                        onclick: move |_| {
                            ...
                            if accounts.peek().active_key().is_some_and(|k| k.is_embedded()) {
                                nav.push(Route::AddEmbeddedUser {});
                            } else {
                ...
                for (uid, uname, is_active, dom_key) in rows {
                    div {
                        ...
                        div {
                            class: "flex items-center gap-2",
                            // Edit (username + password) — only the active user, whose
                            // namespace + token are mounted, can self-edit. Left of Sign out.
                            // Hidden for embedded accounts: their credentials are managed
                            // internally (silent login) — changing them out from under the
                            // stored secrets would only trigger the recovery path.
                            if is_active && !uid.is_embedded() {
                                button {
                                    class: "btn btn-sm btn-primary",
                                    onclick: move |_| { nav.push(Route::UserEdit { id: uid.id }); },
                                    "Edit"
                                }
                            }
                            if !is_active {
