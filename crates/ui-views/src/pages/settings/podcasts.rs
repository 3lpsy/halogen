//! Settings → Podcasts (`/settings/podcasts`): admin-only OPML import/export,
//! moved verbatim from the old single-page Settings.
//!
//! Export fetches the server's subscriptions as OPML and triggers a file
//! download. Import reads a chosen `.opml` file, validates it client-side with
//! the shared parser (`halogen_utils::opml::parse_opml_str`) before upload, then
//! POSTs the XML; on success it asks the worker to refresh so the new podcasts
//! appear. File IO is web-only (Dioxus `FileEngine` / browser download).

use dioxus::prelude::*;

use super::SettingsSubpage;
use halogen_ui_state::commands;
use halogen_ui_state::hooks::{use_config, use_dispatch, use_is_admin, use_toast};
                            ...
                            button {
                                ...
                                onclick: move |_| async move {
                                    ...
                                    match client.export_opml().await {
                                        Ok(d) => {
                                            halogen_ui_logging::download_text("halogen-subscriptions.opml", &d.opml);
