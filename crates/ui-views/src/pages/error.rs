//! The two error-boundary fallbacks.
//!
//! - [`AppError`] — the **inner** boundary ([`crate::app::RootErrorBoundary`], a
//!   route layout *inside* the Router). It catches render errors in the route
//!   subtree, so it has router context and recovers via `navigator()` +
//!   [`ErrorContext::clear_errors`] (works in the native mobile webview, where a
//!   full-page `<a href>` would break the Rust↔webview bridge — there's no HTTP
//!   origin to navigate to). It renders **in place**, so the crashed URL is kept.
//!   Recovery order matters: navigate **off** the crashed route first, *then*
//!   clear — otherwise clearing re-renders the same subtree and re-crashes.
//!
//! - [`FatalError`] — the **outer**, true-root boundary ([`crate::app::App`]),
//!   wrapping `AppProviders`. It catches what the inner one can't reach: a render
//!   panic in the providers themselves (or in the inner fallback). It renders with
//!   **no context at all** — no router, no providers — so it must not use either;
//!   it can only offer a restart (reload on web, a "reopen" prompt on native,
//!   since a webview app can't reliably restart itself).
...
pub fn AppError(errors: ErrorContext) -> Element {
    ...
    // The fallback renders IN PLACE, so the crashed route is the current one.
    // "Go to Home" must not navigate straight back into it: `Home` immediately
    // redirects to `Queue`, so a crash on Queue (or Home itself) made the
    // recovery button re-enter the crashed subtree and bounce right back here.
    // For those, recover to Settings instead — a light, self-contained page.
