...
/// Map an [`ApiError`] to the single inline error string the auth card renders.
///
/// The page keeps one `Option<String>` error signal (not the form-wide
/// `FormErrors` map). Wording stays per-context because the steps differ:
/// login shows "Invalid credentials" for a 401 and prefixes unknown errors
/// with "Login failed:", while the health probe's transport message nudges
/// the user to check the server is running.
