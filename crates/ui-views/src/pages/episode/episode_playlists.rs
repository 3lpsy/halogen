//! Pick the playlists an episode belongs to.
//!
//! Opened from the episode context menu's "Add to playlist" entry. A multiselect
//! over the user's playlists (the queue/default included — `is_default` ignored),
//! **lazily loaded** like the podcasts/episodes lists: render a window over the
//! cached playlist pool (`EpisodeState.playlists`), revalidate it page-by-page from the
//! server in `position` (Custom) order, and let search drive server-side fetches.
//!
//! Pre-selection (which playlists the episode is already in) is resolved lazily via
//! `GET /episodes/{id}/playlists`: the member playlists are fetched + augmented into
//! the pool/local store so they render checked even when they're not on the first
//! page. Offline, membership falls back to the cached `episodes_by_playlist` index.
//! Submit **reconciles**: newly-checked → `AddToPlaylist`, unchecked-but-was-member
//! → `RemoveFromPlaylist`, both through the always-optimistic + outbox `commands`.
...
use halogen_ui_state::hooks::{use_config, use_connection, use_dispatch, use_playlists};
...
fn cached_membership(
    playlists: &Signal<halogen_ui_appstate::PlaylistState>,
    episode_id: i32,
) -> HashSet<i32> {
...
pub fn EpisodePlaylists(episode_id: i32) -> Element {
    ...
    use_effect(move || {
        ...
        if offline {
            ...
            // Don't clobber a selection the user made while this was resolving
            // (pre-init the only possible action is checking boxes, so a non-empty
            // `selected` means the user touched it) — but MERGE the seed in rather
            // than skip it: dropping the real memberships would make the on-save
            // diff emit removals for playlists the user never unchecked.
