...
use halogen_ui_icons::{CloudArrowDown, Download, Pause, Play};
...
/// Episode detail page — art, metadata, full description, playback + download
/// controls. Reached by tapping a list item, its chevron, or the kebab's
/// "View Episode".
///
/// Route target: a thin keyed wrapper. Dioxus reuses the same component instance
/// for a same-variant, param-only route change (`EpisodeDetail { A } → { B }` —
/// reachable from the full-screen player's Up-next tap), which would leave the
/// body's one-shot state (the deep-link fetch guard, `podcast_requested`, the
/// durable-status poll budget) armed for the OLD id. Keying the body on the id
/// remounts it with fresh per-id state instead. The key must be emitted through
/// a keyed list diff (a `for` over one item) — dioxus only honors `key` in keyed
/// children (see `AccountsProvider` for the long version).
#[component]
...
fn EpisodeDetailBody(id: i32) -> Element {
    ...
    let load_failed = use_deep_link_resource(
        ...
        move |client| async move {
            ...
            halogen_ui_state::commands::cache_episodes(&dispatch, vec![ep]);
            Ok(())
        },
    );

    // While the server copy is mid-download, revalidate the episode so the control
    // advances Downloading → "Remove server download" live, without the user having
    // to navigate away and back. The worker's periodic pull would get there
    // eventually; this just makes it prompt. `polling` guards against overlapping
    // loops; the loop stops as soon as the status leaves `Downloading` (or a cap).
    // NOTE: this is the coarse (~4s) DURABLE-STATUS poll only — the smooth ≈1 Hz
    // progress ring is a separate worker-side poll (`server_download_poll_task`)
    // against a different endpoint; the two are intentionally independent.
    ...
    // Lazily pull the parent podcast into the pool for the title link. Dispatch
    // AT MOST ONCE: this effect reads EpisodeState (so it re-runs on every worker
    // publish) and dispatching `EnsurePodcast` itself causes a publish — without
    // the guard that's an infinite publish→dispatch loop that starves the fetch.
    // Once the guard flips, the effect's only remaining dependency is the guard,
    // so it stops re-running on EpisodeState.
    ...
    let podcast_title = podcasts
        ...
        .unwrap_or_default();
    // Live per-episode state (download/queue/playback/play-gating) resolved by the
    // SHARED builder — the SAME formulas the list item uses, so the detail page
    // can't drift from it (the hand-synced "Same rule as the list item" invariants
    // live in `row_state`). `ep` was just cloned from this same pool, so the
    // resolver reading the pool for `id` sees identical server-download status.
    ...
    rsx! {
        ...
        div { class: "p-2",
            ...
            div { class: "flex gap-4 items-start",
                ...
                div { class: "min-w-0 flex-1",
                    ...
                    if !podcast_title.is_empty() {
                        Link {
                            to: Route::PodcastDetail { id: ep.podcast_id },
            ...
            // Play — the download/stream actions otherwise live in the back-row menu.
            // When the episode is held neither on the server nor this device (and
            // nothing's in flight), the actionable control is the explicit
            // "Download & Play" instead: it forces fetch→device→play with progress
            // regardless of the playback preference. The plain Play button is disabled
            // in that state anyway (nothing to stream, no local copy), so we swap it.
            div { class: "mt-4",
                if !server_downloaded && !downloaded_on_device
                    && !server_downloading && !device_downloading && !is_preparing
                {
                    // Embedded mode: a plain server download — the button flips to
                    // the Play state (with in-button progress) as the status
                    // streams in, and Play then streams from the built-in server.
                    // `download_and_play` must NOT be used here: it forces the
                    // device pipeline, which doesn't exist in embedded mode.
                    if embedded {
                        button {
