//! Edit a podcast's own fields (`/podcasts/:id/edit`) — title, description,
//! and feed URL. Modeled on `pages/playlists/playlist_form`.
//!
//! Online-only: unlike playlist edits there is no podcast-update outbox op, and
//! a feed-URL change should surface server validation immediately (the next
//! poll fetches the new URL). On success the updated row is cached into the
//! pool and the form navigates back to the podcast detail.
