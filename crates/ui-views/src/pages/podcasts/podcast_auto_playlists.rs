//! Configure the playlists a podcast auto-adds new episodes to.
//!
//! One set-based form — no create/edit split. It loads the podcast's current
//! auto-add playlists, renders the playlists as a checkbox list, and on submit
//! replaces the whole set via `PUT /podcasts/{id}/auto-playlists`. Online goes
//! direct (so the form can surface server errors); offline applies optimistically
//! (caching the set in `EpisodeState`) and queues a `SetPodcastAutoPlaylists` outbox op.
//!
//! The playlist list is **lazily loaded** (a window over the cached pool, paged +
//! searched server-side in `position` order, like the other lists). The submit
//! payload is built from the `selected` set directly — never by filtering the
//! rendered rows — so a playlist that's selected but not currently on-screen is
//! never dropped. Currently-selected playlists are fetched by id when missing from
//! the pool, so they always render checked.
...
pub fn PodcastAutoPlaylists(id: i32) -> Element {
    ...
    let on_submit = move |evt: FormEvent| {
        ...
        if is_offline {
            ...
            nav.replace(Route::PodcastDetail { id });
            return;
        }
