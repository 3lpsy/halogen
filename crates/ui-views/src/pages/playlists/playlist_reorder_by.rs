//! Smart-reorder screen (`/playlists/:id/reorder-by`).
//!
//! Pick a sort field + direction; on Apply the playlist's `Custom` (position)
//! order is rewritten to match (offline-capable optimistic reorder + outbox —
//! see [`commands::reorder_playlist`](halogen_ui_state::commands::reorder_playlist)). The
//! user can still drag to fine-tune afterwards.
