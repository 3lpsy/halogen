//! Create / edit a playlist — the app's first reactive form, and the reference
//! pattern for future ones.
//!
//! One [`PlaylistForm`] backs both flows (create + edit). It validates the typed
//! `*Data` locally via `validator` (gating the submit button and showing inline
//! field errors once a field is touched), then folds server errors into the same
//! [`FormErrors`] surface: field errors inline, everything else (database/data/
//! internal/transport) in the catch-all banner below the button.
//!
//! Submit paths: **create** goes direct to the server (it needs a real id back) and
//! is online-only. **Edit** goes direct when online (so the form can show server
//! errors), or — when offline — applies optimistically and queues an `UpdatePlaylist`
//! outbox op. The queue is the default playlist and may not exist; while it doesn't,
//! create locks "make default" on (the server enforces this too).
...
/// `Some(trimmed)` for a non-blank string, `None` otherwise — feeds the optional
/// `description` field.
fn desc_opt(s: &str) -> Option<String> {
    let t = s.trim();
...
fn validate_fields(is_create: bool, name: &str, description: &str, is_default: bool) -> FormErrors {
    let result = if is_create {
        PlaylistStoreData {
            name: name.to_string(),
            description: desc_opt(description),
...
fn PlaylistForm(mode: FormMode) -> Element {
    ...
    // Edit deep-link: playlists aren't bulk-held, so a hard refresh / deep link to
    // `/playlists/:id/edit` lands with a COLD pool — the prefill below would find
    // nothing, leaving the form blank, and a save would write defaults (notably
    // `is_default: false`, un-defaulting the queue) over the real playlist. Fetch +
    // cache it so the prefill can populate the form (mirrors `PlaylistDetail`). In
    // Create mode `present` is always true, so the hook no-ops.
    ...
    // Create needs a real server id, so it's online-only; edit works offline via the
    // outbox. So only create is disabled offline.
    //
    // Edit also can't submit until the existing values have prefilled (`initialized`):
    // on a cold-cache deep link a save before the fetch lands would write form
    // defaults (e.g. `is_default: false`) over the real playlist.
    ...
    let on_submit = move |evt: FormEvent| {
        ...
        let name_val = name();
        let desc_val = desc_opt(&description());
        ...
        if is_offline {
            if let FormMode::Edit { id } = mode {
                ...
                nav.replace(Route::PlaylistDetail { id });
            }
            return;
        }
