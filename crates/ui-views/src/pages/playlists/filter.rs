//! Pure client-side search + sort for the playlists list.
//!
//! Playlist sets are small and held in the offline-first pool, so we filter + sort
//! in memory for instant feedback (the server fetch fills in more matches as you
//! scroll). Sibling of `pages::podcasts::filter::apply_podcast_search_sort`; the
//! in-memory mirror of the server's `order_by`/direction mapping in
//! `pages::playlists` (`order_by_token`/`db_direction`).
