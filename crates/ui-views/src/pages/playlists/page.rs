...
/// Playlists page — **offline-first and paged**, mirroring the podcasts list:
/// render a window over the cached playlist pool and revalidate it page-by-page
/// from the server. Defaults to the manual "Custom" (`position`) order, which is
/// also the stable key the server pages by; Name/Created are selectable. Search is
/// server-backed (typing fetches matching playlists, not just filtering the pool).
/// Under Custom order the cards are drag-to-reorder.
#[component]
pub fn Playlists() -> Element {
    ...
    // Offline-first paged pool: render a window over the cached playlist pool and
    // revalidate it page-by-page. The plan reads `sort` + `search` synchronously, so
    // the fetch re-runs when the order/filter changes (server-side order + name
    // filter) and a search lazily pulls matching playlists. The worker upserts via
    // `CachePlaylists` and publishes, re-rendering us.
    ...
    // Reorder only in the unfiltered Custom-ascending view, where a row's display
    // index CAN equal its absolute `position` (the move target). Filtering or
    // another sort would make the dropped index meaningless. The final gate
    // (`reorderable`, below) also requires the visible rows to be a contiguous
    // position-prefix — several flows cache playlists by id out of paging order,
    // so a visible row's display index can otherwise diverge from its server
    // position and the drag would move a playlist to a slot the user never saw.
                                    ...
                                    div {
                                        ...
                                        div { class: "card-body p-3",
                                            div { class: "flex items-center justify-between gap-3",
                                                ...
                                                Link {
                                                    to: Route::PlaylistDetail { id: pid },
                                                ...
                                                // Kebab — playlist actions (reorder/configure/
                                                // delete; same builder as the detail-header
                                                // kebab). Sibling of the link so opening the
                                                // menu never navigates. Delete opens this
                                                // page's confirm.
                                                KebabButton {
