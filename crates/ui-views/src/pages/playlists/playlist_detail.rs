...
/// Playlist detail — header (name) + the playlist's episodes. The list reuses
/// `EpisodeList` in playlist-lazy mode (`Playlist { id }` + `paged`) — exactly the
/// same offline-first, id-list-driven path as the Queue (which is just playlist 1).
///
/// Route target: a thin keyed wrapper. A same-variant, param-only route change
/// (`PlaylistDetail { A } → { B }` — reachable by tapping a pinned playlist while
/// viewing another) reuses the component instance, leaving the deep-link fetch
/// guard (and the list's per-mount state) armed for the OLD id. Keying the body
/// on the id remounts it fresh; the key must go through a keyed list diff (a
/// `for` over one item) — see `EpisodeDetail`/`AccountsProvider`.
#[component]
                ...
                if let Some(pl) = playlist.as_ref() {
                    div { class: "flex items-center gap-1",
                        button {
                            ...
                            onclick: move |_| { nav.push(Route::PlaylistEdit { id }); },
