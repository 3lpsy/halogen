...
use halogen_ui_state::hooks::{use_config, use_dispatch, use_is_offline, use_playlists};
...
pub fn Queue() -> Element {
    ...
    // Lazily resolve the queue via the targeted endpoint when we don't know it yet
    // (robust even if the default sits outside a paged playlist list). ONE-SHOT per
    // online stretch: `peek` (not a reactive read) + a guard so it can't re-dispatch
    // on every EpisodeState publish — that would keep the worker busy and wedge the
    // render loop. The guard RE-ARMS while offline, so connectivity returning
    // re-nudges an unresolved queue instead of leaving it Unknown until the next
    // periodic pull. `do_pull` is the authoritative resolver; this is just a nudge.
