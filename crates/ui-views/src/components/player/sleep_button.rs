//! Sleep-timer button for the full-screen player (sits right of the auto-advance
//! toggle).
//!
//! - **Tap** toggles the timer: off → armed with the configured default; on → off.
//! - **Long-press** opens a small menu with `+X` / `−X` minute steps that adjust the
//!   remaining time *without closing*, so it can be tapped repeatedly. A backdrop
//!   closes it on an outside click.
//! - When active, the moon shows a badge with the remaining whole minutes.
//!
//! The menu is driven by local `open` state rather than the daisyUI `dropdown`
//! focus mechanism on purpose: a tap focuses the button, and `:focus-within` would
//! pop the menu open — which fights the tap-to-toggle gesture. We keep the daisy
//! `menu` look but own the open/close ourselves. All timer logic lives on the
//! [`PlayerController`](halogen_ui_svc_player::PlayerController); this component only reads
//! the state and forwards intents.

use dioxus::prelude::*;

use halogen_ui_icons::Moon;
use halogen_ui_platform::time::sleep_ms;
use halogen_ui_state::hooks::{use_config, use_player_controller};
