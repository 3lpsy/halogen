...
use crate::components::{Artwork, Marquee, UpNext};
use dioxus::prelude::*;
use halogen_ui_appstate::EpisodeState;
use halogen_ui_config::PLAYBACK_RATES;
...
/// Full-screen player overlay. Rendered by `AppLayout` when expanded; `on_close`
/// collapses it back to the mini player.
///
/// Layout: a single non-scrolling flex column. The playback controls are pinned
/// at the bottom (`shrink-0`) and the art + title fill the space above them
/// (`flex-1 min-h-0`), with the art shrinking to whatever's left — so a large UI
/// font scale just makes the art smaller instead of pushing the controls off the
/// bottom and forcing a scroll (the previous behavior). Dragging the upper zone
/// (header / art / title) down past `DRAG_DISMISS_PX` collapses to the mini player.
///
/// Prop-type note: `app_state` is a `Signal<EpisodeState>` (not `ReadSignal`), so
/// the derived props memoize compares it by pointer-eq, never by value, and
/// never marks the signal dirty (see `MiniPlayer` for the longer version).
#[component]
pub fn NowPlayingScreen(app_state: Signal<EpisodeState>, on_close: Callback<()>) -> Element {
    ...
    // Transport-button enablement walks the playlist/queue/podcast lists. This
    // component re-renders ~4×/sec (position), so memoize the scans to run on
    // track/list changes only: subscribe via `current_episode` + `app_state` +
    // `playlists` + `play_context`, read the (stable) controller with `peek`. The
    // PartialEq bool output gates re-render. `playlists` and `play_context` are
    // load-bearing: `has_next`/`has_previous` walk the continuation playlist
    // (`PlaylistState` + the play context) as well as podcast order, so a list or
    // context change with no track / EpisodeState change must still re-run these.
    ...
    // Scrub-in-progress guard for the seek bar. While playing, position ticks
    // re-render this component ~4×/sec; rendering the live position onto the
    // range input mid-drag would yank the thumb away from the finger, and the
    // release's `change` could then report the overwritten (live) value —
    // silently dropping the seek. Track the drag locally: `oninput` follows the
    // finger, the rendered value/label read the scrub position while dragging,
    // and the release seeks from the tracked position, never `e.value()`.
    ...
    // Drag-down-to-dismiss. Pressing anywhere in the upper zone (header / art /
    // title) and dragging down past the threshold collapses to the mini player; a
    // shorter tug snaps back. Move/up live on the root so the gesture keeps
    // tracking if the finger leaves the start element; the bottom controls are a
    // sibling of the start zone, so pressing them never begins a drag.
