...
use halogen_ui_state::providers::AppProviders;

#[component]
pub fn App() -> Element {
    rsx! {
        // Static <head> tags (favicon, viewport, theme-color, the apple-mobile-web-app-*
        // meta, manifest + apple-touch-icon links) live in the custom template
        // `crates/ui/index.html`, which dx merges into the generated index.html. Keeping
        // them there (not as runtime `document::Meta`/`Link` here) puts them in the raw
        // HTML pre-hydration, so crawlers / Lighthouse / the iOS install flow see them
        // immediately. Tailwind is still linked via Dioxus.toml `[web.resource] style`.
        //
        // True root boundary: catches render panics in `AppProviders` itself (above
        // the Router), which the inner `RootErrorBoundary` can't reach. Its fallback
        // [`FatalError`] uses NO context — no router, no providers — so it renders
        // even when provider setup is what failed, and only offers a restart. Route
        // render errors are caught lower down by `RootErrorBoundary` (nearest wins),
        // so they get the richer in-app `AppError` recovery instead.
        ErrorBoundary {
...
/// Outermost route layout: a cross-platform render-error boundary.
///
/// dioxus-core wraps every component render in `catch_unwind`, so a render-time
/// panic (or a `?`-propagated error) in any route below surfaces here; we log it
/// and show the [`AppError`] recovery page. It lives **inside** the Router (it's a
/// layout), so the fallback has router context and recovers via the router
/// (`navigator()` + `ErrorContext::clear_errors`) rather than a full-page `<a href>`
/// — which would break the Rust↔webview bridge in the native mobile webview, where
/// there's no HTTP origin to navigate to.
///
/// Scope: RENDER errors only. Panics in event handlers, spawned async tasks, or the
/// sync worker are NOT caught here (a global panic hook for those is a deliberate
/// follow-up), nor are provider-init panics above the Router.
#[component]
...
mod detail_route_render_tests {
    //! Render-loop guard for the wedged detail routes. In the browser,
    //! `/playlists/:id` and `/podcasts/:id` pin the wasm main thread at 100%
    //! before the first DOM flush (the lazy_lists / playlist_detail e2e
    //! timeouts); `/queue` renders the same `EpisodeList` fine. Route *parsing*
    //! is proven correct by `route_parse_tests`, so this drives the full app
    //! tree natively (MemoryHistory pinned at the route, sandboxed config/data
    //! dirs, authed client.json) and counts render passes: a settled tree goes
    //! quiet; a self-perpetuating dirty loop keeps producing work every pass.
    use super::*;
    ...
    /// Point the native config/data dirs (both resolved via `directories::
    /// ProjectDirs`, which honors `XDG_*` on Linux) at a per-test sandbox with
    /// an authed `client.json`, so RootGuard doesn't bounce to the login page and
    /// the worker's store opens inside the sandbox. nextest runs each test in
    /// its own process, so the env mutation can't race another test.
    ...
    /// Embedded-mode sandbox: an authed EMBEDDED account (registry + its
    /// `e{id}` config), so /downloads runs `ListSource::ServerDownloads`. No
    /// URL resolver is installed in this renderless build, so the persisted
    /// closed-port URL stands and the worker goes Offline fast (like the
    /// remote sandbox above).
    ...
    // Tip for the next render-loop hunt: dioxus-core/signals emit TRACE events in
    // debug builds — `mark_dirty` logs each dirtied context's creation site and
    // the props-memoize write guard logs the writing line. A throwaway test that
    // installs `tracing_subscriber::fmt().with_env_filter("dioxus_core=trace,
    // dioxus_signals=trace")` around `pump()` names the cycle directly.
