...
use crate::pages::discover::{Discover, DiscoverDetail};
...
pub enum Route {
    ...
    // 404 escape hatch. The catch-all redirect funnels every otherwise-unmatched
    // path (an unknown route, or a typed param like `/episodes/abc` that fails to
    // parse) to the canonical `/not-found`. The catch-all is least-specific, so
    // real routes still win.
    // The closure param must be named `segments` to match the `:..segments` catch-all
    // (the macro matches by exact name); `let _` keeps it from warning as unused.
...
mod route_parse_tests {
    //! Guards against the lazy-detail-routes wedge: in the browser, navigating to
    //! `/playlists/:id` or `/podcasts/:id` spins the wasm main thread at 100%
    //! before first mount (e2e `playlist_detail` / `lazy_lists` time out). These
    //! routes share a static prefix with a sibling route (`/playlists`,
    //! `/podcasts`), unlike `/episodes/:id` which parses fine. `Route::from_str`
    //! is pure, so the parse behavior is testable natively. A looping parse shows
    //! up here as a nextest timeout instead of a wedged browser.
    use super::*;
    use std::str::FromStr;
