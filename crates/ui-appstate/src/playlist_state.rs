//! Playlists + queue resolution — the worker-owned reactive slice.
//!
//! Its own root signal so a playlist mutation (add/remove/reorder, default
//! resolution) only re-renders playlist/queue consumers — the playlists list,
//! playlist detail, the queue page + its "up next" markers — not every component
//! that reads the main app state. The sync worker is the only writer; the UI reads
//! it reactively (see `halogen_ui_state::hooks::use_playlists`).
...
/// Resolution of "the queue" — which is just the default (`is_default`) playlist,
/// and may not exist. A tri-state because *absence* and *not-yet-known* must drive
/// different UI: `Unknown` keeps queue actions inert (we don't yet know), while
/// `Absent` actively disables them and prompts the user to create a queue. Only the
/// authoritative `GET /playlists/default` (or a local mutation) can assert `Absent`;
/// a partial playlist list going quiet must NOT be read as "no queue".
...
/// Cached playlists + queue resolution; the worker is the sole writer.
///
/// `PartialEq` is load-bearing (same reason as on `EpisodeState`): this is read through
/// a signal and sliced by memos, so a derive that silently degraded to "always
/// changed" would defeat the segmentation. The `playliststate_is_partialeq` canary
/// guards it.
#[derive(Debug, Clone, PartialEq, Default, serde::Serialize, serde::Deserialize)]
pub struct PlaylistState {
    /// Cached playlists pool (including the default "Queue") — a PARTIAL set, not
    /// the whole list. Like `PodcastState::podcasts_by_id`, it's filled lazily: the
    /// paged playlists list / picker / auto-form page the server and augment it via
    /// `CachePlaylists`, the default is resolved via `GET /playlists/default`, and
    /// boot hydrates whatever the local store cached. Do not assume completeness;
    /// the queue is resolved through `queue`/`queue_id()`, not by scanning this.
...
impl PlaylistState {
    ...
    /// The episode that should play next, given the `current` episode (if any)
    /// and the play `context` — the playlist the user pressed play from
    /// (`None` = the queue). The single source of truth shared by auto-advance
    /// (on episode end), the player's "Up next", and the list next-up marker —
    /// so all three always agree.
    ///
    /// The continuation playlist is the context playlist when its membership is
    /// known locally, else the queue (a deleted/unknown context degrades to queue
    /// semantics). Playlists are ordered by their custom `position` (the
    /// membership Vec is stored in that order). Rules:
    /// - playlist empty / absent → `None`.
    /// - `current` is in the playlist → its position-neighbor (`None` past the
    ///   end — playback stops at the tail, no queue fallback).
    /// - otherwise (current not in it, or nothing playing) → the first item.
