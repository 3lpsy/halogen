//! Per-episode device/server download state — the worker-owned reactive slice.
//!
//! Lives in its own root signal so a download-progress byte update (which can
//! arrive ~1 Hz per in-flight download) only re-renders download consumers — the
//! episode-row badges and the Downloads list — not every component that reads the
//! main app state. The sync worker is the only writer; the UI reads it reactively
//! (see `halogen_ui_state::hooks::use_downloads`).

use std::collections::HashMap;

use crate::state::ClientDownloadState;

/// Device + server download state for episodes, keyed by episode id; the worker is
/// the sole writer.
///
/// `PartialEq` is load-bearing here for the same reason as on `EpisodeState`: this is
/// read through a signal and sliced by memos, so a derive that silently degraded
/// to "always changed" would defeat the segmentation. The `downloadstate_is_partialeq`
/// canary guards it.
