//! Podcast pool + per-podcast auto-playlist config — the worker-owned reactive
//! slice, a separate slice from [`EpisodeState`](crate::state::EpisodeState).
//!
//! Its own root signal so a podcast cache write (the paged podcasts list / on-demand
//! detail fetch) or an auto-playlist edit only re-renders podcast consumers — the
//! podcasts list, podcast detail/config pages, the row podcast-title lookup — not
//! every component that reads the main app state. The sync worker is the only
//! writer; the UI reads it reactively (see
//! `halogen_ui_state::hooks::use_podcasts`).

use std::collections::HashMap;

use halogen_wire::PodcastData;

/// Cached podcasts + their auto-playlist config. Its own slice, separate from
/// `EpisodeState`; the worker is the sole writer.
///
...
/// changed" would defeat the segmentation. The `podcaststate_is_partialeq` canary
/// guards it.
