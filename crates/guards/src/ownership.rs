//! Ownership guards run after validation and before handlers, using the authenticated Actor and a minimal resource
//! lookup. Unauthorized ownership returns 403; missing or hidden read resources return 404. Admins bypass ownership
//! checks.
...
/// Trigger/remove a server-side download of an episode: gated on being subscribed to the episode's podcast (or
/// owning it / admin) — the same access read gives. Resolves the episode's `podcast_id`, then defers to
/// [`require_subscribed`]. Returns **404** for a missing episode (and for non-subscribers, since
...
/// Resolve the admin flag for a media-token user ID so subscription checks match normal Actor authentication. Unknown
/// IDs become non-admin actors and fail the subscription guard with 404.
...
/// Read/modify a user record: the actor must be that same user, or an admin. Unlike the resource guards this
/// needs no DB lookup — identity is already in the [`Actor`]. A non-self, non-admin actor gets a **403** (the
/// actor knows their own id, so refusing another id leaks nothing about whether it exists).
