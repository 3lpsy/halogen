...
impl ToastQueue {
    /// Push a toast, returning its id.
    ///
    /// Deduplicates: an existing toast with the same level+message is dropped
    /// first, so an identical message re-raised (e.g. the worker retrying a
    /// failing op every 60s, or repeated 500s) refreshes the toast in place
...
/// Funnel for foreground (synchronous) API calls.
///
/// Classify the error per `policy`, raise a toast for the cases that warrant one,
/// and return the original `Result` untouched so the caller can still match
/// specific variants (e.g. `Validation`/`Transport`) for inline form handling.
