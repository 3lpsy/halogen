//! Generic per-user namespaced JSON store — the shared load/save/clear backend
//! behind [`ClientConfigStore`](super::ClientConfigStore) and
//! [`ListViewStore`](super::ListViewStore).
//!
//! The `let seg = namespace::…` + per-target persistence dance for every
//! load/save/clear variant lives here once, so each concrete store is a thin
//! wrapper that picks a record `suffix` (the IndexedDB record key on web) + a
//! native filename and delegates to a `NamespacedStore<T>`. Web persists through
//! `crate::web` (IndexedDB `halogen.config.{seg}`); native through the `kv_*`
//! file macros.
...
impl<T: Serialize + DeserializeOwned + Default> NamespacedStore<T> {
    ...
    /// Like `load_segment`, but a backend/parse failure is an `Err` — only a
    /// genuinely absent record defaults. The auth-bearing config store loads
    /// through this: collapsing "couldn't read" into `Default` fakes a
    /// signed-out state, and the next save then permanently overwrites the
    /// real record.
    async fn try_load_segment(&self, seg: &str) -> Result<T, String> {
        ...
        {
            halogen_ui_platform::kv_try_load!(T, "", super::ns_path(seg, self.file))
    ...
    async fn save_segment(&self, seg: &str, value: &T) {
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            halogen_ui_platform::kv_save!("", super::ns_path(seg, self.file), value);
        }
    }
    ...
    async fn clear_segment(&self, seg: &str) {
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            halogen_ui_platform::kv_delete!("", super::ns_path(seg, self.file));
        }
    }
