//! Write actions over the app-wide [`ClientConfig`] signal.
//!
//! The read side (`use_config`, a hook in halogen-ui-state) reads the signal; the
//! mutate-and-persist *action* lives here alongside the other service actions
//! `account_actions` (in halogen-ui-accounts) rather than in a
//! `use_*` file, since it's a plain function, not a hook.
