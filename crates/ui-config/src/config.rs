...
/// Device-log capture configuration. Held in [`ClientConfig`] so it persists,
/// and mirrored into the logging runtime globals so the `/logs/device` capture
/// gate is live (see `halogen_ui_logging`).
///
/// `enabled` defaults to on in dev + tests (`debug_assertions || cfg!(test)`)
/// and off in release/prod.
...
impl ClientConfig {
    ...
    /// Build a read-only [`ApiClient`] for the configured server, with the access
    /// token applied. `None` when there's no server URL, it doesn't parse, its
    /// scheme isn't allowed, OR the user toggled "Go Offline" — manual offline means
    /// "the server doesn't exist", so this central builder returns `None` and every
    /// caller's existing `let Some(client) = … else { … }` path treats it as offline
    /// (no request). Artwork `<img src>` loads bypass this and still hit `/art`.
    /// The single place the read side (pages/hooks) builds a throwaway client.
...
/// Build a read-only [`ApiClient`] from a server URL + token, validating the
/// scheme against [`ALLOWED_SERVER_URL_SCHEMES`]. `None` when the URL is missing,
/// unparseable, or uses a disallowed scheme. Prefer [`ClientConfig::api_client`];
/// this free form exists for callers that hold the URL/token apart from a full
...
/// Persistent storage backend for [`ClientConfig`], namespaced per user.
///
/// Every user's full config (server URL, setup flag, token, admin flag, prefs)
/// lives at its own namespaced key — there is no shared "active" blob to swap.
/// [`load`](Self::load)/[`save`](Self::save) target the *ambient* active user
/// (set by `AccountsProvider`); [`load_for`](Self::load_for)/
/// [`save_for`](Self::save_for) target an explicit id (used by login + switch,
/// which run before the ambient flips). Pre-login work lives in the `anon`
/// namespace.
///
/// Web: `localStorage` key `halogen.{segment}.client_config`.
/// Native: `<platform config dir>/halogen/{segment}/client.json` (the `halogen`
/// segment is part of `project_config_dir()`, via `ProjectDirs`).
...
/// Latched when any config LOAD fails at the backend (not "absent" — a real
/// read/parse failure), cleared when a boot-path [`ClientConfigStore::try_load`]
/// succeeds. While set, every config SAVE is refused: the in-memory config that
/// would be written descends from a `Default` the load error faked, and saving
/// it would permanently overwrite the real record (auth token included) that is
/// still sitting in storage. The session runs degraded (signed-out-looking,
/// nothing persists) until a reload/boot where the load succeeds.
...
/// Rewrite the runtime-only parts of a *loaded* config. For `Embedded`
/// configs: substitute the live loopback base URL (the embedded server's port
/// is OS-assigned per process — the persisted URL is only what was live at the
/// last save) and force `manual_offline` off ("Go Offline" against an
/// in-process server would just black-hole the app against itself).
///
/// This runs at the single choke point every load path shares (ambient +
/// `load_for`), so the ~20 by-value consumers of `server_url` stay untouched.
/// The URL comes from a host-installed resolver rather than a direct
/// dependency: this foundation crate can't depend on the embedded-server
/// graph, and on web / feature-off builds no resolver exists — the config then
/// loads as persisted, which downstream treats as an unreachable server (the
/// safe degraded state).
...
/// it at boot when built with the `embedded-server` feature). A plain fn
/// pointer: the resolver reads process-global supervisor state, no capture
/// needed.
#[cfg(not(target_arch = "wasm32"))]
static EMBEDDED_URL_RESOLVER: std::sync::OnceLock<fn() -> Option<String>> =
    std::sync::OnceLock::new();

/// Install the embedded-server URL resolver (first install wins; later calls
/// are no-ops — there is only one supervisor per process).
#[cfg(not(target_arch = "wasm32"))]
pub fn set_embedded_url_resolver(resolver: fn() -> Option<String>) {
    let _ = EMBEDDED_URL_RESOLVER.set(resolver);
}

/// Native path for a per-user store: `<config root>/{segment}/{file}`, where the
/// root comes from `halogen_ui_platform::paths::config_root()` (XDG on Linux,
/// `Library/Application Support` on macOS/iOS, the app files dir on Android —
/// never a panic, never the CWD). (Web uses IndexedDB — see `crate::web` — and
/// needs no path helper.)
#[cfg(not(target_arch = "wasm32"))]
...
mod tests {
    ...
    fn runtime_overlay_rewrites_embedded_only() {
        #[cfg(not(target_arch = "wasm32"))]
        {
            fn test_resolver() -> Option<String> {
...
mod degraded_storage_tests {
    ...
    fn load_failure_latches_and_refuses_saves_until_a_load_succeeds() {
        ...
        cfg.server_url = Some("https://example.com".into());
