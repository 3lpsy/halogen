//! Web persistence backend for the per-user config stores: IndexedDB database
//! `halogen.config.{segment}`, a single `kv` object store holding one record per
//! suffix (`"client_config"`, `"list_views"`).
//!
//! The store is `Durable` — never wiped on a schema bump (config is not
//! re-fetchable). Connections are cached per database (single-threaded wasm), so
//! the frequent settled config writes don't reopen each time.
...
use halogen_ui_platform::store_keys::{CONFIG_DB_PREFIX, CONFIG_STORE};
