...
impl NavConfig {
    /// Self-heal a persisted order that lost builtin keys (a historical
    /// configure-dock bug could drop admin-only keys on Apply): append any
    /// missing [`default_order`] builtins at the end. A key absent from `order`
    /// is unreachable everywhere — the dock, sidebar, `/menu`, and the
    /// configure-dock screen all render only keys present in `order`.
