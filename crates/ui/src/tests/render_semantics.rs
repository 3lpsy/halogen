//! Executable documentation of the Dioxus state/re-render semantics this app
//! relies on — each test pins one behavior that the state architecture (single
//! worker-owned `Signal<EpisodeState>`, memo slices, `use_callback` props) depends
//! on. If a dioxus upgrade changes any of these, the matching test fails and
//! names the assumption that broke.
//!
//! Driving pattern: a root component stashes its `Signal` (signals are `Copy`)
//! into a `thread_local` so the test can write it via `vdom.in_runtime(..)`,
//! then pump renders with `render_immediate` / a bounded `wait_for_work` loop.
//! nextest runs one process per test, so the thread_locals can't bleed.
...
use halogen_ui_appstate::{ConnectionState, EpisodeState, SyncStatus};
...
async fn unchanged_set_renotifies_subscribers() {
    ...
    assert!(
        ...
        "Signal::set gained a PartialEq gate?! The worker's publish() and the \
         EpisodeList store-freshness contract (see crates/ui-svc-sync/src/worker.rs \
         publish) \
...
async fn memo_slice_gates_unchanged_set() {
    ...
    let mut unchanged = ConnectionState::default();
    unchanged.last_error = Some("noise the slice doesn't read".into());
    ...
    let mut changed = ConnectionState::default();
    changed.sync_status = SyncStatus::Online;
...
// ── Regression: the /playlists/:id 100%-CPU loop ────────────────────────────
// Shape of the original bug: a parent that SUBSCRIBES to a signal and passes
// it as a `ReadSignal<T>` prop alongside a fresh-identity `Callback::new`.
// The derived props memoize value-compares the signal prop; with
// `EpisodeState: PartialEq` the values compare equal and nothing is dirtied. The
// companion test below shows the loop is real when T is not PartialEq.
