//! Unified API error handling. Provides a single `ApiError` type that implements `IntoResponse`, replacing the
//! ad-hoc `{"error":"..."}` JSON responses scattered across auth, polling, and middleware routes.

use crate::response::{ResponsableData, ResponseData};
use axum::{
    http::StatusCode,
