//! Tracing init. One global subscriber: a stdout fmt layer always, an in-memory ring layer always (backs `GET
//! /admin/server-logs` when there's no log file — stdout, once printed, is unrecoverable from inside the
//! process), plus an append-only file layer when `log_file` is set. `OnceLock`-guarded so a restart's re-`init`
//! is a no-op (the subscriber can only be set once).
...
/// Explicit RUST_LOG replaces all defaults. Otherwise combine log_level with QUIET_TARGETS caps, never raising a target
