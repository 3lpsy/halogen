//! Native `MediaStore`: one file per episode under the platform data dir
//! (`audio_dir()` = `<data root>/audio/<episode_id>.<ext>`). The extension is
//! derived from the MIME type so the player-facing path (and the webview
//! media server's content type) stays sensible; `list_ids` parses file stems.
//!
//! An in-progress download streams to a `<id>.partial` temp file alongside a
//! tiny `<id>.partial.meta` sidecar (content type + total size). The temp file
//! is durable across a restart, so an interrupted download is *resumed* by
//! re-opening it in append mode (`partial` reports the byte offset). `commit`
//! atomically `rename`s the temp onto the real path; the partials are never
//! returned by `find`/`list_ids`, so a partial never looks playable or done.
//!
//! Byte fetches authenticate with the normal bearer token (the audio endpoint
//! accepts bearer OR the streaming cookie). Playback: `audio_url` returns the
//! absolute file path, which the webview player backend maps to the
//! `halogen-local-audio` loopback media-server URL (`ui-state`'s
//! `WebviewMediaBridge` serves the bytes over 127.0.0.1, with Range support,
//! from the same `audio_dir()`).
...
/// Streams chunks to a `<id>.partial` temp file, then atomically `rename`s it onto
/// the real path on `commit`. A writer dropped before committing *keeps* the temp
/// file (and its sidecar), so an interrupted download resumes from the bytes on
/// disk; readers (`find`/`list_ids`) still never see a partial. The partial is
/// purged only by `commit`, `remove_audio`, or `clear`.
...
impl MediaWriter for NativeMediaWriter {
    ...
    async fn commit(&mut self) -> Result<()> {
        ...
        file.flush()?;
        // fsync BEFORE the rename: a crash/power loss can otherwise persist the
        // rename (directory metadata) while the data pages never hit disk — a
        // truncated file permanently marked Downloaded. With the sync, the bad
        // orderings collapse to "rename lost", which just leaves the durable
        // partial to resume. One fsync per completed download — negligible.
        file.sync_all()?;
        ...
        halogen_ui_logging::debug!(episode_id = self.episode_id, "Committed device audio");
        Ok(())
    }
}
