...
/// Map a SeaORM error to the standard validation-error envelope, logging the real cause (with `context`)
/// server-side. Use as `.map_err(db_error("fetching podcast"))?`. The mapping itself is delegated to
/// [`DbValidationErrors`] — the single source of truth in `halogen-wire`: a UNIQUE violation becomes a friendly
/// per-field 409, everything else a generic 500. The raw driver message is logged but NEVER sent to the client.
...
/// A 404 "<thing> not found" validation error, keyed `id`/`exists` — the same shape the guards and
/// `EntityHelpers::by_id_or_err` produce. Collapses the `.ok_or_else(|| verrors(VALIDATION_ID_FIELD,
/// VALIDATION_EXISTS_CODE, msg.to_string()))` that the read/update/delete handlers repeat, while keeping each
/// handler's specific message (which several tests assert on verbatim).
