//! Chapter-marker lookups for the `EpisodeInclude::Chapters` embed. Chapters live in the `episode_chapters`
//! table (read-only; written only by the feed-sync path). The episode read handlers fill `EpisodeData.chapters`
//! with the episode's ordered markers when the include is requested. Unlike `user_playback`, chapters are not
//! user-scoped — every caller sees the same set. Mirrors the batched/single shape of `user_playback.rs`.
