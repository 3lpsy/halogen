...
pub async fn handle_store(
    ...
    let data: EpisodePlaylistData = match position {
        ...
        Some(idx) => {
            ...
            // Insert the new row at a free slot past the end (no transient position clash) and renumber every
            // row — including the new one — to its contiguous 0..n position, all in ONE transaction. Doing the
            // insert and the renumber in the same transaction is what keeps a renumber failure from leaving the
            // row committed at the transient position.
...
pub async fn server_delete_flag(
    dbc: &sea_orm::DatabaseConnection,
    playlist_id: i32,
...
pub async fn handle_delete(
    ...
    // Per-playlist cleanup (`on_remove_delete_file_server`): drop the server-side download once NO playlist
    // holds the episode — the file is one global per-episode copy shared across users, so another playlist's
    // membership keeps it alive. Best-effort: the membership removal above already succeeded and the bulk route
    // is deliberately lenient per-id, so cleanup never fails the request.
