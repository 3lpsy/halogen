...
/// Create or subscribe-to a podcast. `feed_url` is globally unique — there is one shared podcast row per feed.
/// If a podcast with this feed already exists, the caller is simply **subscribed to it as a non-owner** (the
/// `owner_id` stays the original creator) and the existing row is returned. Otherwise a new podcast is created
/// owned by the caller and the caller is auto-subscribed. Idempotent and transactional.
pub async fn handle(
