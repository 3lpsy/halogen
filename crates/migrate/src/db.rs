...
/// Explicit SQLite journal profile for a server-owned open. sqlx deliberately
/// leaves `journal_mode` unset (switching it needs an exclusive lock that
/// `busy_timeout` can't wait on), so an implicit open inherits whatever the
/// file already runs — `delete` for a fresh file.
///
/// - `Wal`: readers don't block the writer (poll-tick writes vs audio/list
///   reads), two pools briefly overlap safely across the embedded server's
///   in-process restart, and replication tooling requires it. `-wal`/`-shm`
///   sidecars appear next to the DB.
/// - `Delete`: actively pins the rollback journal. This exists because WAL is
///   STICKY on the file — the `--db-no-wal` escape hatch (network-filesystem
///   storage, where WAL's shared-memory file is unsafe) must convert an
///   existing WAL DB back, not just stop asking for WAL.
...
/// [`connect_and_migrate_with`] pinned to WAL (the embedded server's profile).
...
mod tests {
    ...
    fn temp_dir(name: &str) -> std::path::PathBuf {
        let dir =
            std::env::temp_dir().join(format!("halogen_migrate_{name}_{}", std::process::id()));
        std::fs::create_dir_all(&dir).expect("create temp dir");
        dir
    }

    /// The WAL profile is what the embedded server's restart-overlap safety
    /// rests on: WAL journaling (readers don't block on writers, two pools can
    /// briefly coexist) plus a nonzero busy_timeout.
    #[tokio::test]
