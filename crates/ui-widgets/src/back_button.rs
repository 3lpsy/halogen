...
/// A history "back" button (`navigator.go_back()`). Used on detail pages
/// (episode / podcast) so they return to wherever the user came from — e.g.
/// Episode-from-Queue goes back to Queue, Episode-from-Podcast goes back to the
/// podcast. The main nav pages (Queue, Latest, History, Playlists, Downloads)
