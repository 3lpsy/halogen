//! Minimal, **untrusted-HTML** parser for podcast/episode descriptions.
//!
//! Feed-supplied descriptions are hostile input. We NEVER render them as raw
//! HTML (no `dangerous_inner_html`). Instead we tokenize, keep only a small
//! whitelist of structural tags, decode entities to plain text, and hand back a
//! typed AST ([`Block`]/[`Inline`]) that the UI renders through normal `rsx!`
//! (which escapes all text). Anything not whitelisted — `script`, `img`,
//! `iframe`, event handlers, unknown tags — is dropped, keeping only inner text.
//!
//! Whitelisted: `p`, `div`, `br`, `h1`–`h6` (rendered bold), `ul`/`ol`, `li`,
//! `a` (only `http(s)`/`mailto` hrefs stay clickable), `b`/`strong` (bold),
//! `i`/`em` (kept as text).
...
fn extract_attr(tag: &str, attr: &str) -> Option<String> {
    ...
    while let Some(pos) = lower[from..].find(attr) {
        let at = from + pos;
        // It's a *whole* attribute name only when both token boundaries are clean:
        // preceded by space/start, AND immediately (after optional spaces) followed
        // by `=`. The after-check stops a name that is a PREFIX of a longer one
        // (`hreflang`, `data-href`) from matching `href` — its next char would be a
        // name char, not `=`, so the run is rejected and we leave the text literal.
        ...
        match tok {
            Tok::Text(raw) => {
                ...
                if text.trim().is_empty() {
                    // A whitespace-only run *between* inline elements still
                    // separates them (`<b>a</b> <b>b</b>` must render "a b", not
                    // "ab"). Preserve a single space — but only after existing
                    // content (never leading), and never doubling a trailing
                    // space/newline.
                    if !text.is_empty() {
