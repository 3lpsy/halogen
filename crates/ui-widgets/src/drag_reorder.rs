//! Shared pointer-drag reorder gesture, used by every drag-to-reorder list
//! (episode list, playlists list, dock config).
//!
//! The whole gesture runs in JS so it survives the pointer leaving the grip.
//!
//! **Press-and-hold to engage.** The grip allows normal vertical scrolling
//! (`touch-action: pan-y`), so a quick flick that starts on the grip scrolls the
//! list instead of reordering. A reorder only *engages* after the finger is held
//! roughly still on the grip for [`HOLD_MS`]; moving more than [`MOVE_CANCEL_PX`]
//! before then (or the browser claiming the gesture for a scroll, which fires
//! `pointercancel`) abandons it as a scroll. This keeps reorder from firing while
//! scrolling.
//!
//! Once engaged, the row "lifts" and the other rows slide out of the way to open
//! a gap at the live drop target (a preview of where the row will land); on
//! release the original index of the row under the pointer is reported back.
//! Geometry is snapshotted at *engage* time (not pointer-down) so any scrolling
//! during the hold is already accounted for, and a non-passive `touchmove`
//! `preventDefault` locks scrolling for the duration of the drag so the snapshot
//! stays valid. The reported target is the original `index_attr` value of the row
//! under the pointer, which the callers' move dispatch expects
//! (remove-then-insert-at-`to`, clamped to the list).
...
/// Run a press-and-hold drag-to-reorder gesture for one row.
///
/// `container_id` is the scroll container's dom id; `index_attr` is the
/// data-attribute each row carries with its index (e.g. `"data-ep-index"`);
/// `card_id` is the dragged row's dom id; `dragged_index` / `start_y` come from
/// the pointer-down event. `on_drop(target)` fires only when the row actually
/// moved (the reported index is valid and differs from `dragged_index`).
