//! Reusable right slide-in "quick context menu".
//!
//! A single overlay instance lives at the app root ([`QuickContextMenuHost`] in
//! `AppLayout`) and is driven by a context controller ([`QuickMenu`]). Any
//! component opens it with a title + [`QuickAction`]s grouped into sections
//! (rendered with divider lines between them):
//!
//! ```ignore
...
//! ```
//!
//! Actions are plain `Rc<dyn Fn()>` closures — NOT scope-owned
//! [`Callback`](dioxus::prelude::Callback)s. The menu outlives the row that
//! opened it (a background publish can unmount the row while the panel is up),
//! and a `Callback::new` slot dies with its scope: tapping it afterwards panics
//! (`Callback::call` → dropped generational slot). An `Rc` closure over `Copy`
//! handles (`Coroutine`, `Signal`, `Navigator`) can never dangle.
//!
//! Anchoring is responsive. On mobile the panel is bottom-anchored: sections
//! stack down toward the title/close header at the very bottom, so the most-used
//! actions (last section) sit in thumb reach. On desktop (`md:`) it flips to
//! top-anchored — header on the first line, then the sections below it in the
//! same most-used-first order — via `md:flex-col-reverse` on both the panel and
//! the sections column (a clean vertical mirror, no reordering of the data).
//!
//! Rendering at the root (rather than inside the triggering row) is deliberate:
//! list rows live inside a `transform`ed, `overflow-hidden` swipe card, which
//! would trap/clip an in-flow dropdown. A root-level `fixed` panel escapes that.
    ...
    rsx! {
        div { class: "{container_class}",
            ...
            aside { class: "{panel_class}",
                // Scroll anchoring (chat-log pattern): `flex-col-reverse` makes
                // `scrollTop = 0` rest at the VISUAL BOTTOM, so when a tall menu
                // overflows it's the TOP (least-used downloads) that scrolls off and
                // the bottom-anchored, most-used actions (queue / add-to-playlist)
                // stay on screen. Sections are rendered in REVERSE so the reversal of
                // the column restores the authored top→bottom visual order. The
                // previous `flex-1` spacer bottom-anchored only when it FIT — on
                // overflow the spacer collapsed and the bottom section was clipped.
                // Desktop (`md:flex-col`) is the normal top-anchored scroll.
