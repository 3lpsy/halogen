//! Episode-list view state.
//!
//! The data DTOs (`EpisodeData`, `PlaybackData`, …) live in `halogen_wire`
//! (wasm-safe, no `db` feature); `EpisodeData` is re-exported here so the component
//! and the rest of the UI share one definition. Everything else below is *view
//! state* — sort/filter/swipe UI controls — which is intentionally UI-only.
...
/// Filter `rows` by the `keep` predicate, then sort by the Ascending comparator
/// `cmp`, reversing the whole order for `Desc`.
///
/// The shared skeleton behind the small in-memory list filters
/// (`pages::podcasts::filter` / `pages::playlists::filter`): each call site
/// supplies only the entity-specific search predicate and the field comparator,
/// while the `Asc`/`Desc` reversal lives here once. (The episode list sorts ids
/// with a render-time `reversed` flag instead, so it doesn't go through here.)
...
impl FilterSpec {
    /// Whether this filter currently HIDES rows from the full set — any active
    /// search, chip, date bound, or podcast restriction narrows what's rendered.
    ///
    /// Manual playlist reorder keys off rendered-row indices and translates them
    /// directly into stored-membership positions, so it must be disabled while this
    /// is true: a hidden row above the drop target would make the move land at the
    /// wrong stored position (and the descending mirror axis would be wrong too).
...
pub enum ListSource {
    ...
    /// page's source in embedded-server mode, where the server's library *is*
    /// this device's library and the device-download concept doesn't exist.
    ServerDownloads,
}

impl ListSource {
    /// Sources whose membership is an id list resolved from LOCAL state (no
    /// sparse server paging): playlists, both downloads sets, history. EXTEND
    /// THIS when adding a variant — the list component's mode gate reads it,
    /// and a variant missing here silently takes the server-paged path while
    /// the id-list branch's snapshot gates go inconsistent (the exact bug that
    /// shipped with `ServerDownloads` and panicked the Downloads page).
...
/// Multiselect (bulk-action) view state for one `EpisodeList`. Page-local — it
/// lives only while the list is mounted and is never persisted: selection is a
/// transient interaction, not saved/shared state.
///
/// `selected` survives filtering and sorting: an episode hidden by a filter stays
/// in the set (and reappears selected when the filter is removed). `selected_only`
/// is the "Selected" review chip — it restricts the list to the selected set.
#[derive(Debug, Clone, Default, PartialEq)]
