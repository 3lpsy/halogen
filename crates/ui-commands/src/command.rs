...
/// A bearer token whose `Debug` prints `"***"`, never the secret.
///
/// `Command` derives `Debug` and the worker logs `debug!(command = ?cmd)`, which
/// — with device logging enabled — persists to the on-disk/IndexedDB log ring.
/// Wrapping the token makes the redaction structural: no call site, and no future
/// `Command` variant, can leak the JWT into the logs by accident.
...
pub enum Command {
    ...
    /// Download episodes' audio bytes to THIS device: per id marks the client
    /// state `Downloading`, triggers a server download (the device pulls from the
    /// server, never the RSS origin), and spawns the byte-fetch task that stores
    /// the file in the media store and reports `DeviceFetchComplete`. One id (a
    /// row action) downloads quietly — the row spinner is the feedback; multiple
    /// (multiselect) get a summary toast. Lenient: already-downloaded/in-flight
    /// ids are skipped, and an offline batch toasts once, not per id.
    ...
    /// Set the playlists a podcast auto-adds new episodes to (offline-capable):
    /// optimistically cache the set and queue a `SetPodcastAutoPlaylists` outbox
    /// op. Online edits go direct, then dispatch `CacheAutoPlaylists`.
    /// `add_to_start` is the podcast's insert-position override (`Some(true)` =
    /// start of the playlists, `Some(false)` = end, `None` = server default).
    ...
    /// INTERNAL — a server-download poll task reporting the SERVER's fetch progress
    /// (percent 0–100) for an episode whose copy the server is downloading, so the
    /// download controls render a filling ring like the device flow. Polled ≈1 Hz
    /// off the server's progress tracker; applied only while the episode's
    /// `download_status` is still `Downloading`. Throttled to integer-percent
    /// changes by the task.
    ...
    /// INTERNAL — a batch of lazily-fetched podcasts reported back by the
    /// pool-prime task (one `list_podcasts(filter.ids=…)` collapsing the per-row
    /// `EnsurePodcast` N+1). `podcast_ids` is the requested set (so every in-flight
    /// guard is cleared even for ids the server omitted); `podcasts` is `None` on
    /// fetch failure.
    PodcastsFetched {
        podcast_ids: Vec<i32>,
    ...
    /// Mirror the device-download preferences into the worker: `chunk_bytes` is the
    /// per-request chunk size (`None` = no chunking, fetch the whole file in one
    /// request) and `parallelism` is how many chunks are fetched concurrently within
    /// a single download. Persisted in `ClientConfig.download_prefs`; dispatched on
    /// boot and whenever the setting changes (like [`Command::SetOffline`]).
    SetDownloadPrefs {
    ...
    /// Remove ALL locally-cached data for one episode — cached metadata, playback
    /// position/history, the device-downloaded audio blob, and its membership in
    /// local playlists/queue — WITHOUT touching the server. A local cache prune for
    /// recovery, NOT a delete: it never enqueues an outbox op, and the episode
    /// re-syncs on the next server pull / pull-to-refresh.
...
impl Command {
    /// Does this command apply an optimistic, user-initiated change to `EpisodeState`
    /// that should be visible the instant it's handled — *before* the (possibly
    /// slow) outbox drain?
    ///
    /// The run loop publishes after `drain_outbox().await`, which makes a network
    /// round-trip. On a throttled link an in-flight download can stall that drain,
    /// so a freshly-clicked action's optimistic state (e.g. a download spinner)
    /// wouldn't render until the drain returned. For the commands below the loop
    /// publishes *early*, so feedback is immediate regardless of link speed.
    ///
    /// Returns `false` for commands that either publish internally already
    /// (cache/ensure/load/connectivity), carry no user-visible optimistic state,
    /// or are high-frequency internal task reports — an extra publish there is
    /// wasted work, since `publish()` has no value-equality gate and every call
    /// re-renders all `EpisodeState` subscribers (notably the paged-list and
    /// download-progress hot paths). `true` is the safe default for a new
