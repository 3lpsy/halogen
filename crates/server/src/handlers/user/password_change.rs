...
///
/// Re-verifies `current_password` against the stored bcrypt hash, then stores a
/// fresh hash of the new password. The new password's length and the confirmation
/// match are enforced up-front by the `Body<PasswordChangeData>` extractor in the
/// router, so the DTO arrives here already valid.
pub async fn handle(
