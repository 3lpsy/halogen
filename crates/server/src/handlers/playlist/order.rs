//! Shared position-ordering helpers for the playlist handlers.
//!
//! Both the playlist-level move (`playlist_move`) and the episode-within-playlist
//! move/reorder (`episode_playlist`, `playlist_reorder`) maintain a contiguous
//! `position` column the same way: compute the target order in memory, then
//! rewrite every row's `position` to its new 0..n index in one transaction. These
//! two helpers hold that logic in one place so the only per-entity difference is
//! how an `ActiveModel` is built.
...
/// Move the element at index `from` to index `to` (clamped into range) within
/// `ids`, returning the resulting order. Returns `None` when it would be a no-op:
/// an empty list, a `from` out of range, or a clamped `to` that equals `from`.
///
/// Pure (no DB) so the index math is unit-testable and identical for playlists and
/// playlist membership.
...
/// Rewrite a set of rows' `position` to exactly match `ordered_ids` (contiguous
/// 0..n) in a single transaction. `build(id, new_position)` produces the
/// `ActiveModel` for one row — typically the primary key `Unchanged`, `position`
/// `Set`, `updated_at` `Set`, everything else `NotSet`. Self-heals any holes left
/// by deletes.
...
/// Like [`rewrite_in_txn`] but inserts `new_row` first, in the SAME transaction as
/// the renumber. Use when adding a row at a specific index: a renumber failure
/// must not leave the freshly-inserted row committed at a transient position.
/// `ordered_ids` must already include the new row's id at its target index (the
/// renumber then places it, and everyone, at its final 0..n slot).
