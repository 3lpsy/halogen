...
/// Delete everything that hangs off a set of episodes — playlist memberships,
/// playback history, per-user listen state, and chapters — within `txn`. The
/// shared core of the episode cleanup `episode_delete` (one episode) and
/// `podcast_delete` (all of a podcast's episodes) both perform; FKs would cascade
/// these, but we delete them explicitly for deterministic ordering. No-op on an
/// empty slice (an empty `IN ()` is degenerate SQL).
