//! Per-user episode listen-state lookups.
//!
//! Per-user listen state lives in the `user_episode_status` table (not a global
//! `episode.playback_status` column), so the episode read handlers
//! (`list`/`get`/`update`) overwrite the `EpisodeData.playback_status` (which
//! `From<Model>` defaults to Unplayed) with the caller's row. Absence of a row
//! means UNPLAYED.
