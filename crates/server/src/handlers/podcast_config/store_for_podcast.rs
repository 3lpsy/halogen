...
/// Create a podcast config AND link it to the podcast, atomically.
///
/// Backs `POST /podcasts/{id}/config`: one transaction inserts the config row and
/// sets `podcast.podcast_config_id`, so a partial failure can't leave an orphan
/// config — this nested route is the only way to create a config row (there is no
/// standalone create endpoint). Refuses to clobber an existing config — the UI
/// only routes here when the podcast has none.
pub async fn handle(
