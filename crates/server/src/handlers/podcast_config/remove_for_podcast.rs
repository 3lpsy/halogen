...
/// Unlink and delete a podcast's config, atomically (revert to global defaults).
///
/// Backs `DELETE /podcasts/{id}/config`: one transaction nulls
/// `podcast.podcast_config_id` then deletes the config row, so no podcast is ever
/// left pointing at a deleted config — this nested route is the only way to delete
/// a config row, and it always nulls the FK first. Idempotent: a podcast with no
/// config is a no-op success.
pub async fn handle(
