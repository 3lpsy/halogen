...
///
/// Read-gated on being subscribed to the episode's podcast (or owning it / admin),
/// the same access the list route grants — a non-subscriber gets 404 so existence
/// doesn't leak. `playback_status` is per-user, so the caller's id is also threaded
/// into the handler.
pub async fn get(
