...
/// Fetch episodes with pagination, ordering, and includes.
///
/// Episodes are scoped to the caller's subscriptions (the handler filters by
/// `user_podcast`), and `playback_status` is per-user, so we thread the caller's
/// id into the handler.
pub async fn list(
