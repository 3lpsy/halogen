...
use crate::tests::harness::{episode_model, podcast_model, seed_admin_and_user, subscribe};
use halogen_fixture::test_support::TestRoot;

pub use crate::tests::harness::{
    build_test_router, build_test_router_with_media_root, generate_jwt_token,
    generate_media_jwt_token,
};

/// Create a test SQLite database with seeded episodes and podcasts using SeaORM entities.
pub async fn setup_test_db() -> (TestRoot, sea_orm::DatabaseConnection, serde_json::Value) {
    let (mut root, dbc) = crate::tests::harness::new_test_db("test_episodes").await;

    let admin_id = i32::MAX;
    let user_id = i32::MAX - 1;
    seed_admin_and_user(&dbc, admin_id, user_id).await;

    // The acting (regular) user owns the podcast so the ownership-guarded episode
    // write routes (store/update/delete) succeed.
    ...
    // Subscribe the regular user to the podcast (parity with the podcasts harness;
    // the owner can also read its episodes via any subscription-gated route).
    subscribe(&dbc, user_id, podcast_id).await;
