...
mod tests {
    ...
    /// A param-less list comes back in PIVOT POSITION order, not id order.
    /// Regression: `params.order.unwrap_or_default()` used to fill
    /// `order_by = "id"`, dead-coding the position default — so a front-add
    /// (position 0) showed up at the END for clients that fetch without an
    /// explicit order (the iOS queue). Also covers position Desc, which was
    /// silently returned ascending.
    #[tokio::test]
