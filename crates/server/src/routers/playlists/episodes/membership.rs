...
pub async fn store(
    ...
    // The episode must also be one the caller may access (subscribed / owner /
    // admin), matching the bulk-add path — otherwise a user could add an episode
    // from a podcast they can't see into their own playlist and read its metadata
    // back via the playlist listing. (Remove/move don't gate on subscription: the
    // caller must always be able to manage what's already in their own playlist.)
