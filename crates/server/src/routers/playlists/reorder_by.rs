...
mod tests {
    ...
    /// Seed a fresh playlist owned by `user_id` with three episodes whose sortable
    /// fields are deliberately scrambled vs. each other, so each sort field yields a
    /// distinct order. Returns `(playlist_id, [ep_a, ep_b, ep_c])`, inserted at
    /// positions a,b,c and added (pivot `created_at`) in the order a < b < c.
    ///
    /// |  ep | title     | published | duration | added |
    /// |-----|-----------|-----------|----------|-------|
    /// |  a  | "Charlie" | 1000      | 300      | 1st   |
    /// |  b  | "alpha"   | 3000      | 100      | 2nd   |
    /// |  c  | "Bravo"   | 2000      | 200      | 3rd   |
