...
pub async fn refresh(
    ...
    // Only a normal full-access API token may be refreshed. A media-only credential
    // OR a WebSocket ticket (both scope-locked to their own transport — and the WS
    // ticket rides in a URL query string, where it's prone to leak) must never be
    // upgradable into a full-access token. Checking `is_api()` rather than just
