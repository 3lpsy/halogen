...
///
/// Both branches set `allow_credentials(true)` so the browser sends the
/// `auth_media` cookie cross-origin (dev: UI and API on different ports) and
/// accepts the credentialed audio response. Credentials forbid a `*` origin, so
/// the dev branch reflects the request origin rather than being wildcard. A
/// non-empty list restricts `Access-Control-Allow-Origin` to those exact origins.
