//! Shared HTTP-test support for the crate's `#[cfg(test)]` router suites. One copy
//! of `build_test_router`, `generate_jwt_token`, and the `setup_test_db` factories
//! that every `routers/<resource>/tests.rs` re-exports, so the per-resource suites
//! stay consistent (same `Duration` aliases, ids, and `JwtClaims` shape):
//!
//! - [`build_test_router`] / [`generate_jwt_token`] / [`TEST_JWT_SECRET`] — one copy,
//!   re-exported by each resource's `tests.rs` so the per-verb `mod tests`
//!   imports keep working unchanged.
//! - `*_model` factories — return an `ActiveModel` with the verbose all-`None`
//!   columns pre-filled and timestamps set; the caller overrides the handful of
//!   fields it cares about, then `.insert()`s.
//! - [`authed`] / [`authed_json`] / [`json`] / [`json_body`] / [`field_error`] —
//!   collapse the `Request::builder()…Bearer…oneshot` + `to_bytes` +
//!   `serde_json` + `json["errors"][field][0]["message"]` plumbing every test
//!   repeats.
...
/// Spin up a fresh migrated SQLite database under a temp [`TestRoot`].
///
/// The returned `root` is NOT yet marked successful — the caller seeds it and
/// calls `root.mark_success()` once seeding succeeds, preserving the
...
// ----- entity factories -------------------------------------------------------
//
// Each returns a fully-populated `ActiveModel` (timestamps set, optional columns
// `None`) carrying sensible defaults. Callers override the fields a given test
// cares about before inserting, e.g.
//
//     let mut p = podcast_model(id, owner_id);
//     p.title = Set("Second Podcast".into());
//     p.insert(&dbc).await.unwrap();
...
//
// The request/response plumbing every per-verb `mod tests` uses. A handful of
// tests still build requests inline where these don't fit — malformed/custom
// `Authorization` headers, `auth_media` cookies, CORS preflight, and helpers that
// take an optional token — which is expected.
