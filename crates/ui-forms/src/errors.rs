//! Reactive-form error plumbing — the shared `FormErrors` type.
//!
...
//!
//! Pure data (no renderer) so both the `use_form` hook (in `ui-state`) and the
//! form field components (in `ui-widgets`) can share it; it lived in
//! `components/form.rs` before the UI crate split.
...
impl FormErrors {
    ...
    /// The standard inline merge every form renders under an input: this (local
    /// `validator`) field error — only when `show_local` (the field is touched or
    /// the form was submitted, so an untouched-empty field isn't nagged) — followed
    /// by any `server` field error. Call on the local error set, passing the
    /// last-submit server errors (cleared on edit, so they never linger past a fix).
